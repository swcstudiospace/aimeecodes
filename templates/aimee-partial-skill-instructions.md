...
**CRITICAL**: Before acting in the loop (research, plan, or implement), ALWAYS check if a skill exists for it in the available_skills list below. Skills are specialized workflows that must be loaded when their trigger conditions match. Do not improvise a parallel procedure.
