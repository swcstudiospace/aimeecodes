...
pub struct TodoWriteOutput {
    ...
    pub todos: Vec<aimee_domain::Todo>,
}
...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: aimee_domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: aimee_domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &aimee_domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<aimee_domain::Effort>>;
    ...
    /// all configuration changes; use [`aimee_domain::ConfigOperation`]
...
pub trait WorkspaceService: Send + Sync {
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> anyhow::Result<aimee_stream::MpscStream<anyhow::Result<SyncProgress>>>;
...
pub trait FsPatchService: Send + Sync {
    ...
    async fn multi_patch(
        &self,
        path: String,
        edits: Vec<aimee_domain::PatchEdit>,
    ) -> anyhow::Result<PatchOutput>;
}
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<aimee_domain::Image>;
}
...
pub trait FsSearchService: Send + Sync {
    ...
    /// * `Err(_)` - Search error
    async fn search(&self, params: aimee_domain::FSSearch) -> anyhow::Result<Option<SearchResult>>;
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<aimee_domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    /// Load all command definitions from the aimee/commands directory
    async fn get_commands(&self) -> anyhow::Result<Vec<aimee_domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &aimee_domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<aimee_domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<aimee_domain::Skill>>;
}
