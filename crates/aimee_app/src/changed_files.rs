...
use aimee_domain::{Agent, ContextMessage, Conversation, Role, TextMessage};
...
impl<S: FsReadService + EnvironmentInfra<Config = aimee_config::AimeeConfig>> ChangedFiles<S> {
...
mod tests {
            ...
            self.files
                ...
                .map(|content| {
                    ...
                    ReadOutput {
                        ...
                        info: aimee_domain::FileInfo::new(1, 1, 1, hash),
                    }
                })
    ...
    impl EnvironmentInfra for TestServices {
        ...
        fn get_config(&self) -> anyhow::Result<aimee_config::AimeeConfig> {
            Ok(aimee_config::AimeeConfig { max_parallel_file_reads: 4, ..Default::default() })
        }
