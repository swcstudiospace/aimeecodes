    ...
    fn compress_single_sequence(
        ...
        if let Some(reasoning) = reasoning_details
            && let Some(ContextMessage::Text(msg)) = context
                .messages
                .iter_mut()
                .find(|msg| msg.has_role(aimee_domain::Role::Assistant))
                .map(|msg| &mut **msg)
            && msg
                .reasoning_details
                .as_ref()
...
mod tests {
    ...
    fn render_template(data: &serde_json::Value) -> String {
        TemplateEngine::default()
            .render("aimee-partial-summary-frame.md", data)
            .unwrap()
    }

    #[test]
    fn test_template_engine_renders_summary_frame() {
        use aimee_domain::{ContextSummary, Role, SummaryBlock, SummaryMessage, SummaryToolCall};
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                Role::Assistant,
                vec![
                    SummaryToolCall::codebase_search(vec![aimee_domain::SearchQuery::new(
    ...
    fn test_template_engine_renders_todo_write() {
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                ...
                vec![
                    SummaryToolCall {
                        id: Some(aimee_domain::ToolCallId::new("call_1")),
    ...
    async fn test_render_summary_frame_snapshot() {
        ...
        let fixture_json = aimee_test_kit::fixture!("/src/fixtures/conversation.json").await;

        let conversation: aimee_domain::Conversation =
        ...
        let environment = test_environment().cwd(PathBuf::from(
            "/Users/tushar/Documents/Projects/aimee-codes-workspace/aimee-codes",
        ));
        let compactor = Compactor::new(Compact::new(), environment);
    ...
    fn test_compaction_preserves_usage_information() {
        use aimee_domain::{TokenCount, Usage};
    ...
    fn ctx(pattern: &str) -> Context {
        aimee_domain::MessagePattern::new(pattern).build()
    }
    ...
    fn test_safe_threshold_triggers_earlier_than_unsafe_threshold() {
        use aimee_domain::{ContextMessage, ToolCallId, ToolName, ToolResult};
    ...
    fn create_large_context(token_count: usize) -> Context {
        use aimee_domain::ContextMessage;
