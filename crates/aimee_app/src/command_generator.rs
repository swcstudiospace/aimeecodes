...
where
    S: EnvironmentInfra<Config = aimee_config::AimeeConfig>
        ...
        + AppConfigService,
{
    ...
    ///
    /// Terminal context is read automatically from the `_AIMEE_TERM_COMMANDS`,
    /// `_AIMEE_TERM_EXIT_CODES`, and `_AIMEE_TERM_TIMESTAMPS` environment
...
mod tests {
    ...
    impl MockServices {
        ...
        fn with_terminal_context(
            ...
            env_vars.insert("_AIMEE_TERM_COMMANDS".to_string(), commands.to_string());
            env_vars.insert("_AIMEE_TERM_EXIT_CODES".to_string(), exit_codes.to_string());
            env_vars.insert("_AIMEE_TERM_TIMESTAMPS".to_string(), timestamps.to_string());
            Arc::new(Self {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<aimee_domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<aimee_domain::AnyProvider>> {
            Ok(vec![])
        }
    ...
    impl AppConfigService for MockServices {
        async fn get_session_config(&self) -> Option<aimee_domain::ModelConfig> {
            Some(aimee_domain::ModelConfig::new(
                ProviderId::OPENAI,
                ModelId::new("test-model"),
            ))
        }

        async fn get_commit_config(&self) -> Result<Option<aimee_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_suggest_config(&self) -> Result<Option<aimee_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_reasoning_effort(&self) -> Result<Option<aimee_domain::Effort>> {
            Ok(None)
        }

        async fn update_config(&self, _ops: Vec<aimee_domain::ConfigOperation>) -> Result<()> {
            Ok(())
        }
    }
