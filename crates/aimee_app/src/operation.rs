...
pub enum ToolOperation {
    ...
    Skill {
        output: aimee_domain::Skill,
    },
    TodoWrite {
        before: Vec<aimee_domain::Todo>,
        after: Vec<aimee_domain::Todo>,
    },
    TodoRead {
        output: Vec<aimee_domain::Todo>,
    },
}
...
fn create_validation_warning(path: &str, errors: &[aimee_domain::SyntaxError]) -> Element {
    Element::new("warning")
...
impl ToolOperation {
    /// Converts this tool operation into a [`aimee_domain::ToolOutput`].
    ///
    /// # Arguments
        ...
        match self {
            ToolOperation::FsRead { input, output } => {
                ...
                if let Some(image) = output.content.as_image() {
                    ...
                    *metrics = metrics.clone().insert(
                    ...
                    return aimee_domain::ToolOutput::image(image.clone());
                }
                ...
                *metrics = metrics.clone().insert(
                ...
                aimee_domain::ToolOutput::text(elm)
            }
            ...
            ToolOperation::FsRemove { input, output } => {
                ...
                *metrics = metrics.clone().insert(
                ...
                aimee_domain::ToolOutput::text(elem)
            }
            ...
            ToolOperation::CodebaseSearch { output } => {
                ...
                if output.queries.is_empty() || total_results == 0 {
                    ...
                    for query_result in &output.queries {
                        ...
                        for data in &query_result.results {
                            if let aimee_domain::NodeData::FileChunk(file_chunk) = &data.node {
                ...
                aimee_domain::ToolOutput::text(root)
            }
            ...
            ToolOperation::Shell { output } => {
                ...
                aimee_domain::ToolOutput::text(parent_elem)
            }
            ...
            ToolOperation::TodoWrite { before, after } => {
                ...
                let before_map: std::collections::HashMap<&str, &aimee_domain::Todo> =
...
mod tests {
    ...
    use aimee_domain::{FSRead, FSReadRange, FileInfo, ToolValue};
    ...
    fn to_value(output: aimee_domain::ToolOutput) -> String {
    ...
    fn test_syntax_errors(errors: Vec<(u32, u32, &str)>) -> Vec<aimee_domain::SyntaxError> {
        use aimee_domain::SyntaxError;
    ...
    mod sem_search_helpers {
        use aimee_domain::{CodebaseQueryResult, CodebaseSearchResults, FileChunk, Node, NodeData};
        use fake::{Fake, Faker};
    ...
    fn test_shell_output_no_truncation() {
        let fixture = ToolOperation::Shell {
            output: ShellOutput {
                output: aimee_domain::CommandOutput {
    ...
    fn test_fs_remove_success() {
        let fixture = ToolOperation::FsRemove {
            input: aimee_domain::FSRemove { path: "/home/user/file_to_delete.txt".to_string() },
    ...
    fn test_fs_search_files_with_matches_mode() {
        let fixture = ToolOperation::FsSearch {
            input: aimee_domain::FSSearch {
                ...
                output_mode: Some(aimee_domain::OutputMode::FilesWithMatches),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_search_count_mode() {
        let fixture = ToolOperation::FsSearch {
            input: aimee_domain::FSSearch {
                path: Some("/home/user/project".to_string()),
                pattern: "test".to_string(),
                output_mode: Some(aimee_domain::OutputMode::Count),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_undo_no_changes() {
        let fixture = ToolOperation::FsUndo {
            input: aimee_domain::FSUndo { path: "/home/user/unchanged_file.txt".to_string() },
    ...
    fn test_fs_undo_file_created() {
        let fixture = ToolOperation::FsUndo {
            input: aimee_domain::FSUndo { path: "/home/user/new_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: None,
    ...
    fn test_fs_undo_file_removed() {
        let fixture = ToolOperation::FsUndo {
            input: aimee_domain::FSUndo { path: "/home/user/deleted_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: Some(
    ...
    fn test_fs_undo_file_restored() {
        let fixture = ToolOperation::FsUndo {
            input: aimee_domain::FSUndo { path: "/home/user/restored_file.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_fs_undo_success() {
        let fixture = ToolOperation::FsUndo {
            input: aimee_domain::FSUndo { path: "/home/user/test.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_net_fetch_truncated() {
        ...
        let truncation_path =
            TempContentFiles::default().stdout(PathBuf::from("/tmp/aimee_fetch_abc123.txt"));
    ...
    fn test_skill_operation() {
        let fixture = ToolOperation::Skill {
            output: aimee_domain::Skill::new(
                "test-skill",
                "This is a test skill command with instructions",
                "A test skill for demonstration",
            )
            .path("/home/user/.aimee/skills/test-skill")
            .resources(vec![
                PathBuf::from("/home/user/.aimee/skills/test-skill/resource1.txt"),
                PathBuf::from("/home/user/.aimee/skills/test-skill/resource2.md"),
            ]),
        };
    ...
    fn test_fs_read_image_with_vision_model() {
        ...
        match &actual.values[0] {
            aimee_domain::ToolValue::Image(_) => (), // Expected
