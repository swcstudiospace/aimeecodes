...
impl<S: Services + EnvironmentInfra<Config = aimee_config::AimeeConfig>> ToolRegistry<S> {
    ...
    async fn call_inner(
        ...
        // First, try to call a Aimee tool
        if ToolCatalog::contains(&input.name) {
            ...
            if let ToolCatalog::Task(task_input) = tool_input {
                ...
                let conversation_id = session_id
                    .map(|id| aimee_domain::ConversationId::parse(&id))
                    .transpose()
                    .ok()
                    .flatten();
            ...
            let approval = aimee_domain::ApprovalMode::current();
            if is_restricted
                && !approval.skips_permission_check()
                && !(approval.skips_prompts())
                && self.check_tool_permission(&tool_input, context).await?
            {
                // Send formatted output message for policy denial
                context
                    .send(aimee_domain::TitleFormat::error("Permission Denied"))
                    .await?;
    ...
    pub async fn tools_overview(&self) -> anyhow::Result<ToolsOverview> {
        ...
        // Build TemplateConfig from AimeeConfig for tool description templates
...
#[cfg(test)]
fn create_test_agents() -> Vec<aimee_domain::Agent> {
    use aimee_domain::{Agent, AgentId, ModelId, ProviderId, ToolName};
...
fn create_test_model(
    id: &str,
    modalities: Vec<aimee_domain::InputModality>,
) -> aimee_domain::Model {
    use aimee_domain::{Model, ModelId};
...
fn test_validate_tool_modality_with_image_file_and_vision_model() {
    use aimee_domain::{InputModality, ToolCatalog};

    let vision_model = create_test_model("gpt-4o", vec![InputModality::Text, InputModality::Image]);
    let tool_input = ToolCatalog::Read(aimee_domain::FSRead {
...
fn test_validate_tool_modality_with_non_read_tool() {
    ...
    let text_only_model = create_test_model("gpt-3.5-turbo", vec![InputModality::Text]);
    let tool_input = ToolCatalog::Write(aimee_domain::FSWrite {
        file_path: "/home/user/test.png".to_string(),
