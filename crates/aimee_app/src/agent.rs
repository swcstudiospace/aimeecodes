...
#[async_trait::async_trait]
impl<T: Services + EnvironmentInfra<Config = aimee_config::AimeeConfig>> AgentService for T {
    async fn chat_agent(
        &self,
        id: &ModelId,
        context: Context,
...
/// from [`AimeeConfig`] and applied to domain agents at runtime.
pub trait AgentExt {
    ...
    ///
    /// # Arguments
    /// * `config` - The top-level Aimee configuration.
    fn apply_config(self, config: &AimeeConfig) -> Agent;
}
...
impl AgentExt for Agent {
    fn apply_config(self, config: &AimeeConfig) -> Agent {
        ...
        if let Some(ref workflow_compact) = config.compact {
            // Convert aimee_config::Compact to aimee_domain::Compact, then merge.
        ...
        if let Some(ref config_reasoning) = config.reasoning {
            use aimee_config::Effort as ConfigEffort;
...
#[cfg(test)]
mod tests {
    use aimee_config::{Effort as ConfigEffort, ReasoningConfig as ConfigReasoningConfig};
    use aimee_domain::{AgentId, Effort, ModelId, ProviderId, ReasoningConfig};
    ...
    fn test_reasoning_applied_from_config_when_agent_has_none() {
        let config = AimeeConfig::default().reasoning(
    ...
    /// workflow settings. This means user's .aimee.toml compact settings
    ...
    fn test_compact_agent_settings_take_priority_over_workflow_config() {
        use aimee_config::Percentage;

        // Workflow config with custom compact settings (from .aimee.toml)
        let workflow_compact = aimee_config::Compact::default()
        ...
        let config = AimeeConfig::default().compact(workflow_compact);
    ...
    fn test_compact_partial_agent_settings_override_workflow_values() {
        use aimee_config::Percentage;
        use aimee_domain::Compact as DomainCompact;
