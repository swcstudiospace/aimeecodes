...
async fn test_empty_responses() {
    ...
    ctx.config.retry = Some(aimee_config::RetryConfig {
...
async fn test_no_tool_call_start_end_responses_for_agent_tools() {
    // Call an agent tool (using "aimee" which is configured as an agent in the
    // default workflow)
    let agent_tool_call = ToolCallFull::new("aimee")
        .arguments(ToolCallArguments::from(json!({"tasks": ["analyze code"]})));
    let agent_tool_result =
        ToolResult::new("aimee").output(Ok(ToolOutput::text("analysis complete")));
...
async fn test_reasoning_should_be_in_context() {
    ...
    ctx.agent = ctx
        .agent
        .reasoning(ReasoningConfig::default().effort(aimee_domain::Effort::High));
...
async fn test_reasoning_not_supported_when_disabled() {
    ...
    ctx.agent = ctx.agent.reasoning(
        ReasoningConfig::default()
            .effort(aimee_domain::Effort::High)
...
async fn test_todo_enforcement_injects_reminder() {
    ...
    use aimee_domain::{Metrics, Todo, TodoStatus};
    ...
    let reminder = messages
        ...
        .find(|content| content.contains("pending loop items"));
