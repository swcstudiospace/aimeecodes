...
pub struct Runner {
    ...
    config: aimee_config::AimeeConfig,
    env: Environment,
}
...
impl Runner {
    fn new(setup: &TestContext) -> Self {
        ...
        aimee_embed::register_templates(&mut hb, &TEMPLATE_DIR);
...
impl AgentService for Runner {
    async fn chat_agent(
        &self,
        _id: &aimee_domain::ModelId,
        context: aimee_domain::Context,
        _provider_id: Option<ProviderId>,
    ) -> aimee_domain::ResultStream<ChatCompletionMessage, anyhow::Error> {
    ...
    async fn call(
        &self,
        _: &aimee_domain::Agent,
        _: &aimee_domain::ToolCallContext,
        test_call: aimee_domain::ToolCallFull,
    ) -> aimee_domain::ToolResult {
...
impl TemplateService for Runner {
    ...
    async fn render_template<V: serde::Serialize + Send + Sync>(
        &self,
        template: aimee_domain::Template<V>,
        object: &V,
...
impl AttachmentService for Runner {
    async fn attachments(&self, _url: &str) -> anyhow::Result<Vec<aimee_domain::Attachment>> {
...
impl SkillFetchService for Runner {
    async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<aimee_domain::Skill> {
