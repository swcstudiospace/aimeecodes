...
impl<S: Services + EnvironmentInfra<Config = aimee_config::AimeeConfig>> AgentExecutor<S> {
    ...
    pub async fn execute(
        ...
        ctx.send_tool_input(
            TitleFormat::agent(agent_id.as_str().to_case(Case::UpperSnake))
        ...
        let conversation = if let Some(conversation_id) = conversation_id {
            ...
            let context = aimee_domain::Context::default().initiator("agent".to_string());
        ...
        // Execute the request through the AimeeApp
        let app = crate::AimeeApp::new(self.services.clone());
