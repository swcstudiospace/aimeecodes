...
pub struct Model {
    ...
    /// OpenRouter-style object. xAI Imagine models send an array of
    /// `{quality, resolution, price_per_image}` — ignore anything that is not
    /// the OpenRouter shape so SuperGrok `/models` still loads.
    #[serde(default, deserialize_with = "deserialize_optional_pricing")]
...
/// Accepts OpenRouter `{prompt, completion}` objects and ignores xAI Imagine
/// arrays (or any other non-object pricing payload).
fn deserialize_optional_pricing<'de, D>(deserializer: D) -> Result<Option<Pricing>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    fn from(value: Model) -> Self {
        ...
        let input_modalities = value
            ...
            .map(|modalities| {
                modalities
                    .iter()
                    .filter_map(|s| s.parse::<aimee_domain::InputModality>().ok())
                    .collect::<Vec<_>>()
            })
            .unwrap_or_else(|| vec![aimee_domain::InputModality::Text]);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_xai_oauth_models_response_deserializes() {
        let fixture = load_fixture("xai_oauth_models.json").await;

        let actual = serde_json::from_value::<ListModelResponse>(fixture).unwrap();

        assert!(actual.data.iter().any(|m| m.id.as_str() == "grok-4.6"));
        let grok = actual
            .data
            .iter()
            .find(|m| m.id.as_str() == "grok-4.6")
            .unwrap();
        assert!(grok.context_length.is_some());
    }
