use aimee_domain::{ContextMessage, Image};
...
impl TryFrom<aimee_domain::Context> for Request {
    type Error = anyhow::Error;
    fn try_from(request: aimee_domain::Context) -> std::result::Result<Self, Self::Error> {
        let system_messages = request
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(msg) if msg.has_role(aimee_domain::Role::System) => {
                    Some(SystemMessage {
        ...
        let (thinking, output_config) = if reasoning_on && let Some(reasoning) = request.reasoning {
            ...
            let output_config = reasoning.effort.and_then(|effort| {
                let output_effort = match effort {
                    aimee_domain::Effort::None => return None,
                    aimee_domain::Effort::Minimal | aimee_domain::Effort::Low => OutputEffort::Low,
                    aimee_domain::Effort::Medium => OutputEffort::Medium,
                    aimee_domain::Effort::High => OutputEffort::High,
                    aimee_domain::Effort::XHigh => OutputEffort::XHigh,
                    aimee_domain::Effort::Max => OutputEffort::Max,
                };
        ...
        Ok(Self {
            messages: request
                .messages
                .into_iter()
                .filter(|message| !message.has_role(aimee_domain::Role::System))
            ...
            output_format: request.response_format.and_then(|rf| match rf {
                aimee_domain::ResponseFormat::Text => {
                    // Anthropic doesn't have a "text" output format, so we skip it
                    None
                }
                aimee_domain::ResponseFormat::JsonSchema(schema) => {
        ...
        Ok(match value {
            ContextMessage::Text(chat_message) => {
                ...
                match chat_message.role {
                    aimee_domain::Role::User => Message { role: Role::User, content },
                    aimee_domain::Role::Assistant => Message { role: Role::Assistant, content },
                    aimee_domain::Role::System => {
                        ...
                        return Err(
                            aimee_domain::Error::UnsupportedRole("System".to_string()).into()
                        );
                    }
                }
            }
...
impl TryFrom<aimee_domain::ToolCallFull> for Content {
    type Error = anyhow::Error;
    fn try_from(value: aimee_domain::ToolCallFull) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(aimee_domain::Error::ToolCallMissingId)?;
...
impl TryFrom<aimee_domain::ToolResult> for Content {
    type Error = anyhow::Error;
    fn try_from(value: aimee_domain::ToolResult) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(aimee_domain::Error::ToolCallMissingId)?;
...
impl From<aimee_domain::ToolChoice> for ToolChoice {
    fn from(value: aimee_domain::ToolChoice) -> Self {
        match value {
            aimee_domain::ToolChoice::Auto => ToolChoice::Auto { disable_parallel_tool_use: None },
            aimee_domain::ToolChoice::Call(tool_name) => {
                ToolChoice::Tool { name: tool_name.to_string(), disable_parallel_tool_use: None }
            }
            aimee_domain::ToolChoice::Required => {
                ToolChoice::Any { disable_parallel_tool_use: None }
            }
            aimee_domain::ToolChoice::None => ToolChoice::Auto { disable_parallel_tool_use: None },
        }
    }
}
...
impl TryFrom<aimee_domain::ToolDefinition> for ToolDefinition {
    type Error = anyhow::Error;
    fn try_from(value: aimee_domain::ToolDefinition) -> std::result::Result<Self, Self::Error> {
        Ok(ToolDefinition {
            name: value.name.to_string(),
...
mod tests {
    ...
    fn test_reasoning_max_tokens_and_effort_emit_both() {
        ...
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(aimee_domain::Effort::Low),
            enabled: Some(true),
            max_tokens: Some(8000),
            exclude: None,
        });
    ...
    fn test_reasoning_effort_with_exclude_emits_adaptive_omitted() {
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(aimee_domain::Effort::High),
            enabled: Some(true),
            max_tokens: None,
            exclude: Some(true),
        });
    ...
    fn test_reasoning_xhigh_effort_maps_to_xhigh() {
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(aimee_domain::Effort::XHigh),
            enabled: Some(true),
            max_tokens: None,
            exclude: None,
        });
    ...
    fn test_reasoning_effort_none_does_not_emit_anything() {
        ...
        let fixture = Context::default().reasoning(ReasoningConfig {
            enabled: None,
            max_tokens: None,
            effort: Some(aimee_domain::Effort::None),
            exclude: None,
        });
