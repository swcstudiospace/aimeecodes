...
impl From<Model> for aimee_domain::Model {
    fn from(value: Model) -> Self {
        ...
        {
            vec![
                aimee_domain::InputModality::Text,
                aimee_domain::InputModality::Image,
            ]
        } else {
            vec![aimee_domain::InputModality::Text]
        };
...
impl From<Usage> for aimee_domain::Usage {
...
impl From<StopReason> for aimee_domain::FinishReason {
    fn from(value: StopReason) -> Self {
        match value {
            StopReason::EndTurn => aimee_domain::FinishReason::Stop,
            StopReason::MaxTokens => aimee_domain::FinishReason::Length,
            StopReason::StopSequence => aimee_domain::FinishReason::Stop,
            StopReason::ToolUse => aimee_domain::FinishReason::ToolCalls,
            StopReason::Refusal => aimee_domain::FinishReason::ContentFilter,
        }
    }
}
    ...
    fn try_from(value: Event) -> Result<Self, Self::Error> {
        let result = match value {
            ...
            Event::Ping { cost: Some(cost) } => {
                ...
                ChatCompletionMessage::assistant(Content::part(""))
                    .usage(aimee_domain::Usage { cost: Some(cost_value), ..Default::default() })
            }
...
mod tests {
    ...
    fn test_usage_conversion_with_cache_read_tokens() {
        ...
        let actual: aimee_domain::Usage = fixture.into();
    ...
    fn test_vertex_ai_streaming_usage() {
        ...
        let initial_domain: aimee_domain::Usage = initial_usage.into();
        let delta_domain: aimee_domain::Usage = delta_usage.into();
    ...
    fn test_model_conversion_includes_context_length() {
        ...
        let actual: aimee_domain::Model = fixture.into();
    ...
    fn test_ping_event_with_cost_produces_usage() {
        ...
        let expected_usage = aimee_domain::Usage { cost: Some(0.00724710), ..Default::default() };
    ...
    fn test_refusal_maps_to_content_filter() {
        ...
        let actual = aimee_domain::FinishReason::from(fixture);

        let expected = aimee_domain::FinishReason::ContentFilter;
        assert_eq!(actual, expected);
    }
}
