use aimee_domain::{Context, ContextMessage};
...
impl From<Context> for Request {
    fn from(context: Context) -> Self {
        ...
        let system_parts: Vec<Part> = context
            .messages
            .iter()
            .filter(|msg| msg.has_role(aimee_domain::Role::System))
            .filter_map(|msg| {
        ...
        let generation_config = Some(GenerationConfig {
            ...
            response_mime_type: context.response_format.as_ref().and_then(|rf| match rf {
                aimee_domain::ResponseFormat::JsonSchema(_) => Some("application/json".to_string()),
                _ => None,
            }),
...
impl From<aimee_domain::ToolChoice> for ToolConfig {
    fn from(choice: aimee_domain::ToolChoice) -> Self {
        let (mode, allowed_function_names) = match choice {
            aimee_domain::ToolChoice::Auto => (FunctionCallingMode::Auto, None),
            aimee_domain::ToolChoice::None => (FunctionCallingMode::None, None),
            aimee_domain::ToolChoice::Required => (FunctionCallingMode::Any, None),
            aimee_domain::ToolChoice::Call(name) => {
...
impl From<aimee_domain::ToolDefinition> for FunctionDeclaration {
    fn from(tool: aimee_domain::ToolDefinition) -> Self {
        let mut parameters =
...
impl From<aimee_domain::TextMessage> for Content {
    fn from(text_message: aimee_domain::TextMessage) -> Self {
        let role = match text_message.role {
            aimee_domain::Role::User => Some(Role::User),
            aimee_domain::Role::Assistant => Some(Role::Model),
            aimee_domain::Role::System => None, // System messages are handled separately
        };
...
impl From<aimee_domain::ToolResult> for Content {
    fn from(tool_result: aimee_domain::ToolResult) -> Self {
        Content {
...
impl From<aimee_domain::Image> for Content {
    fn from(image: aimee_domain::Image) -> Self {
        Content { role: Some(Role::User), parts: vec![Part::from(image)] }
    }
}

impl From<aimee_domain::ToolCallFull> for Part {
    fn from(tool_call: aimee_domain::ToolCallFull) -> Self {
        Part::FunctionCall {
            function_call: FunctionCallData {
...
impl From<aimee_domain::ToolResult> for Part {
    fn from(tool_result: aimee_domain::ToolResult) -> Self {
...
impl From<aimee_domain::Image> for Part {
    fn from(image: aimee_domain::Image) -> Self {
        Part::Image {
            inline_data: ImageSource {
...
#[cfg(test)]
mod tests {
    use aimee_domain::{ToolCallArguments, ToolCallFull, ToolCallId, ToolName, ToolResult};
    ...
    fn test_consecutive_tool_results_grouped() {
        use aimee_domain::{Context, ContextMessage, ModelId};
    ...
    fn test_text_message_conversion() {
        use aimee_domain::{Role, TextMessage};
    ...
    fn test_response_schema_strips_dollar_schema() {
        use aimee_domain::{Context, ResponseFormat};
        use schemars::schema_for;
    ...
    fn test_tool_result_part_conversion() {
        use aimee_domain::{ToolCallId, ToolName, ToolResult};
