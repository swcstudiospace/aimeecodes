...
fn create_npm_matrix() -> Value {
    serde_json::json!({
        "repository": [
            "swcstudiospace/npm-aimee-codes"
        ]
    })
}
...
mod tests {
    ...
    #[test]
    fn test_npm_release_matrix_uses_aimee_codes_package() {
        let fixture = super::create_npm_matrix();
        let actual = fixture["repository"]
            .as_array()
            .expect("repository matrix should be an array");
        let expected = vec![serde_json::json!("swcstudiospace/npm-aimee-codes")];
