...
This crate provides a unified interface for terminal user interactions across the aimee codebase. It encapsulates all direct dependencies on `dialoguer`, ensuring no other crates need to depend on it directly.
...
```rust
use aimee_select::AimeeSelect;

let options = vec!["Option 1", "Option 2", "Option 3"];
let selected = AimeeSelect::select("Choose an option:", options)
...
let confirmed = AimeeSelect::confirm("Are you sure?")
    .with_default(true)
    .prompt()?;
```
...
let name = AimeeSelect::input("Enter your name:")
    .allow_empty(false)
    .with_default("John")
    .prompt()?;
```
...
let selected = AimeeSelect::multi_select("Choose colors:", options)
    .prompt()?;
```
...
- Create the builder with `AimeeSelect::select()`, `AimeeSelect::confirm()`, etc.
...
- `aimee_main`: For CLI user interactions
- `aimee_infra`: For implementing the `UserInfra` trait
