...
use aimee_domain::{Conversation, ConversationId};
use chrono::Utc;
...
impl Default for PathwayLogOptions {
    fn default() -> Self {
        Self {
            ...
            eternal_label_prefix: "aimee".into(),
            hard_fail: false,
        }
    }
}
    ...
    E: EternalStore,
{
    ...
    pub async fn rollback_to(
        ...
        for checkpoint in all.iter().filter(|c| c.seq <= seq) {
            ...
            if !checkpoint.verify_hash() {
                return Err(AndaError::ContentHashMismatch { seq: checkpoint.seq });
            }
...
mod tests {
    ...
    use aimee_domain::Context;

    fn fixture_service()
    -> SessionPathwayService<MemoryPathwayStore, NoopKipBackend, NoopEternalStore> {
    ...
    fn conversation_with_messages(n: usize) -> Conversation {
        let mut conversation = Conversation::generate().title(Some("rollback demo".into()));
        let mut ctx = Context::default();
        for i in 0..n {
            ctx = ctx.add_message(aimee_domain::ContextMessage::user(format!("msg-{i}"), None));
        }
    ...
    async fn test_log_checkpoint_advances_seq_and_hash_chain() {
        ...
        let c2 = {
            ...
            c.context = Some(
                c.context
                    .unwrap_or_default()
                    .add_message(aimee_domain::ContextMessage::user("msg-1b", None)),
            );
            c
        };
        let b = service
    ...
    async fn test_rollback_restores_snapshot() {
        ...
        c2.context = Some(
            c2.context
                .unwrap_or_default()
                .add_message(aimee_domain::ContextMessage::user("later", None)),
        );
        service
