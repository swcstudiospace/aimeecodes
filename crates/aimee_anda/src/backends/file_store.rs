...
use crate::{AndaError, AndaResult, PathwayCheckpoint, PathwayStore, SessionPathway};
...
impl FilePathwayStore {
    ...
    async fn read_json<T: serde::de::DeserializeOwned>(path: &Path) -> AndaResult<Option<T>> {
        match fs::read(path).await {
            Ok(bytes) => {
                let value =
                    serde_json::from_slice(&bytes).map_err(|e| AndaError::Other(e.into()))?;
                Ok(Some(value))
            }
    ...
    async fn append_checkpoint(&self, checkpoint: PathwayCheckpoint) -> AndaResult<()> {
        ...
        if let Some(prev) = self
            .get_checkpoint(
                ...
                checkpoint.seq.saturating_sub(1),
            )
            .await?
            && checkpoint.parent_hash != prev.content_hash
        {
...
mod tests {
    ...
    use super::*;
    use crate::{CheckpointKind, PathwayCheckpoint};

    #[tokio::test]
    async fn test_file_store_roundtrip() {
        let tmp = TempDir::new().unwrap();
        let store = FilePathwayStore::new(tmp.path());
        let conversation = aimee_domain::Conversation::generate().title(Some("file store".into()));
