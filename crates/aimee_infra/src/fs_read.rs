...
#[derive(Clone)]
pub struct AimeeFileReadService;

impl Default for AimeeFileReadService {
    fn default() -> Self {
        Self
    }
}

impl AimeeFileReadService {
    pub fn new() -> Self {
        Self
    }
}
...
impl FileReaderInfra for AimeeFileReadService {
    async fn read_utf8(&self, path: &Path) -> Result<String> {
        aimee_fs::AimeeFS::read_utf8(path).await
    }
    ...
    async fn read(&self, path: &Path) -> Result<Vec<u8>> {
        aimee_fs::AimeeFS::read(path).await
    }
    ...
    async fn range_read_utf8(
        ...
        start_line: u64,
        end_line: u64,
    ) -> Result<(String, aimee_domain::FileInfo)> {
        aimee_fs::AimeeFS::read_range_utf8(path, start_line, end_line).await
    }
}
...
mod tests {
    ...
    async fn test_read_batch_utf8() {
        let fixture = AimeeFileReadService::new();
