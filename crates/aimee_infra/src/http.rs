...
use aimee_config::{AimeeConfig, TlsBackend, TlsVersion};
use aimee_eventsource::{EventSource, RequestBuilderExt};
use anyhow::Context;
use bytes::Bytes;
...
impl<F: aimee_app::FileWriterInfra + 'static> AimeeHttpInfra<F> {
    /// Creates a new [`AimeeHttpInfra`] from a resolved [`AimeeConfig`].
    pub fn new(config: AimeeConfig, file_writer: Arc<F>) -> Self {
        let http = config.http.unwrap_or(aimee_config::HttpConfig {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        if !headers.contains_key("User-Agent") {
            headers.insert("User-Agent", HeaderValue::from_static("Aimee-Codes"));
        }
        headers.insert("X-Title", HeaderValue::from_static("Aimee Codes"));
        headers.insert(
            "x-app-version",
        ...
        headers.insert(
            "HTTP-Referer",
            HeaderValue::from_static("https://aimeecodes.dev"),
        );
        headers.insert(
...
impl<F: aimee_app::FileWriterInfra + 'static> HttpInfra for AimeeHttpInfra<F> {
...
mod tests {
    ...
    fn create_test_config(debug_requests: Option<PathBuf>) -> AimeeConfig {
        AimeeConfig { debug_requests, ..Default::default() }
    }
    ...
    async fn test_debug_requests_none_does_not_write() {
        ...
        let http = AimeeHttpInfra::new(config, Arc::new(file_writer.clone()));
    ...
    async fn test_debug_requests_with_valid_path() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/aimee-test/debug.json");
    ...
    async fn test_debug_requests_post_writes_body() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/aimee-test/debug-post.json");
        let config = create_test_config(Some(debug_path.clone()));
        let http = AimeeHttpInfra::new(config, Arc::new(file_writer.clone()));
    ...
    fn test_sanitize_headers_redacts_sensitive_values() {
        ...
        headers.insert("x-title", HeaderValue::from_static("aimee"));
        ...
        assert_eq!(
            ...
            Some(&HeaderValue::from_static("aimee"))
        );
        assert_eq!(
