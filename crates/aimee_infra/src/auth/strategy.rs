...
use aimee_app::{AuthStrategy, OAuthHttpProvider, StrategyFactory};
use aimee_domain::{
...
async fn poll_for_tokens(
    device_code: &aimee_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
pub struct AimeeAuthStrategyFactory;

impl Default for AimeeAuthStrategyFactory {
    fn default() -> Self {
        Self
    }
}

impl AimeeAuthStrategyFactory {
    pub fn new(_environment: aimee_domain::Environment) -> Self {
        Self
    }
}

impl StrategyFactory for AimeeAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            aimee_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            aimee_domain::AuthMethod::OAuthCode(config) => {
            ...
            aimee_domain::AuthMethod::OAuthDevice(config) => {
            ...
            aimee_domain::AuthMethod::GoogleAdc => Ok(AnyAuthStrategy::GoogleAdc(
                GoogleAdcStrategy::new(provider_id, required_params),
            )),
            aimee_domain::AuthMethod::AwsProfile => Ok(AnyAuthStrategy::AwsProfile(
                AwsProfileStrategy::new(provider_id, required_params),
            )),
            aimee_domain::AuthMethod::CodexDevice(config) => Ok(AnyAuthStrategy::CodexDevice(
...
mod tests {
    ...
    fn test_create_auth_strategy_api_key() {
        let factory = AimeeAuthStrategyFactory;
    ...
    fn test_create_auth_strategy_codex_device() {
        ...
        let actual = factory.create_auth_strategy(
            ProviderId::CODEX,
            aimee_domain::AuthMethod::CodexDevice(config),
            vec![],
        );
        assert!(actual.is_ok());
    ...
    fn test_enrich_codex_oauth_credential_uses_id_token_claims() {
        ...
        let expected = Some(&aimee_domain::URLParamValue::from(
