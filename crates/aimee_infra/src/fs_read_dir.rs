...
use aimee_app::DirectoryReaderInfra;
...
pub struct AimeeDirectoryReaderService {
    parallel_file_reads: usize,
}

impl AimeeDirectoryReaderService {
    ...
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
        ...
        if !AimeeFS::exists(directory) || AimeeFS::is_file(directory) {
            return Ok(vec![]);
        }
    ...
    async fn read_directory_files(
        ...
        let mut dir = AimeeFS::read_dir(directory).await?;
        ...
        let files = futures::stream::iter(file_paths)
            .map(|path| async move {
                ...
                match AimeeFS::read_to_string(&path).await {
...
impl DirectoryReaderInfra for AimeeDirectoryReaderService {
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
...
mod tests {
    ...
    async fn test_read_directory_files_with_filter() {
        ...
        let actual = AimeeDirectoryReaderService::new(64)
    ...
    async fn test_read_directory_files_without_filter() {
        ...
        let mut actual = AimeeDirectoryReaderService::new(64)
