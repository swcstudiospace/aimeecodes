...
#[async_trait::async_trait]
impl<T: Services + EnvironmentInfra<Config = omega_config::OmegaConfig>> AgentService for T {
    async fn chat_agent(
        &self,
        id: &ModelId,
        context: Context,
...
/// from [`OmegaConfig`] and applied to domain agents at runtime.
pub trait AgentExt {
    ...
    ///
    /// # Arguments
    /// * `config` - The top-level Omega configuration.
    fn apply_config(self, config: &OmegaConfig) -> Agent;
}
...
impl AgentExt for Agent {
    fn apply_config(self, config: &OmegaConfig) -> Agent {
        ...
        if let Some(ref workflow_compact) = config.compact {
            // Convert omega_config::Compact to omega_domain::Compact, then merge.
        ...
        if let Some(ref config_reasoning) = config.reasoning {
            use omega_config::Effort as ConfigEffort;
...
#[cfg(test)]
mod tests {
    use omega_config::{Effort as ConfigEffort, ReasoningConfig as ConfigReasoningConfig};
    use omega_domain::{AgentId, Effort, ModelId, ProviderId, ReasoningConfig};
    ...
    fn test_reasoning_applied_from_config_when_agent_has_none() {
        let config = OmegaConfig::default().reasoning(
    ...
    /// workflow settings. This means user's .omega.toml compact settings
    ...
    fn test_compact_agent_settings_take_priority_over_workflow_config() {
        use omega_config::Percentage;

        // Workflow config with custom compact settings (from .omega.toml)
        let workflow_compact = omega_config::Compact::default()
        ...
        let config = OmegaConfig::default().compact(workflow_compact);
    ...
    fn test_compact_partial_agent_settings_override_workflow_values() {
        use omega_config::Percentage;
        use omega_domain::Compact as DomainCompact;
