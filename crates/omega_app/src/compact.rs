...
impl Compactor {
    ...
    fn compress_single_sequence(
        ...
        let summary = TemplateEngine::default().render(
            "omega-partial-summary-frame.md",
        ...
        if let Some(reasoning) = reasoning_details
            && let Some(ContextMessage::Text(msg)) = context
                .messages
                .iter_mut()
                .find(|msg| msg.has_role(omega_domain::Role::Assistant))
                .map(|msg| &mut **msg)
            && msg
                .reasoning_details
                .as_ref()
...
mod tests {
    ...
    fn render_template(data: &serde_json::Value) -> String {
        TemplateEngine::default()
            .render("omega-partial-summary-frame.md", data)
            .unwrap()
    }

    #[test]
    fn test_template_engine_renders_summary_frame() {
        use omega_domain::{ContextSummary, Role, SummaryBlock, SummaryMessage, SummaryToolCall};
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                Role::Assistant,
                vec![
                    SummaryToolCall::codebase_search(vec![omega_domain::SearchQuery::new(
    ...
    fn test_template_engine_renders_todo_write() {
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                ...
                vec![
                    SummaryToolCall {
                        id: Some(omega_domain::ToolCallId::new("call_1")),
    ...
    async fn test_render_summary_frame_snapshot() {
        ...
        let fixture_json = omega_test_kit::fixture!("/src/fixtures/conversation.json").await;

        let conversation: omega_domain::Conversation =
        ...
        let environment = test_environment().cwd(PathBuf::from(
            "/Users/tushar/Documents/Projects/omega-loops-workspace/omega-loops",
        ));
        let compactor = Compactor::new(Compact::new(), environment);
    ...
    fn test_compaction_preserves_usage_information() {
        use omega_domain::{TokenCount, Usage};
    ...
    fn ctx(pattern: &str) -> Context {
        omega_domain::MessagePattern::new(pattern).build()
    }
    ...
    fn test_safe_threshold_triggers_earlier_than_unsafe_threshold() {
        use omega_domain::{ContextMessage, ToolCallId, ToolName, ToolResult};
    ...
    fn create_large_context(token_count: usize) -> Context {
        use omega_domain::ContextMessage;
