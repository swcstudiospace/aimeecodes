...
impl<S: Services + EnvironmentInfra<Config = omega_config::OmegaConfig>> ToolRegistry<S> {
    ...
    async fn call_inner(
        ...
        // First, try to call a Omega tool
        if ToolCatalog::contains(&input.name) {
            ...
            if let ToolCatalog::Task(task_input) = tool_input {
                ...
                let conversation_id = session_id
                    .map(|id| omega_domain::ConversationId::parse(&id))
                    .transpose()
                    .ok()
                    .flatten();
            ...
            if is_restricted && self.check_tool_permission(&tool_input, context).await? {
                ...
                context
                    .send(omega_domain::TitleFormat::error("Permission Denied"))
                    .await?;
    ...
    pub async fn tools_overview(&self) -> anyhow::Result<ToolsOverview> {
        ...
        // Build TemplateConfig from OmegaConfig for tool description templates
...
#[cfg(test)]
fn create_test_agents() -> Vec<omega_domain::Agent> {
    use omega_domain::{Agent, AgentId, ModelId, ProviderId, ToolName};
...
fn create_test_model(
    id: &str,
    modalities: Vec<omega_domain::InputModality>,
) -> omega_domain::Model {
    use omega_domain::{Model, ModelId};
...
fn test_validate_tool_modality_with_image_file_and_vision_model() {
    use omega_domain::{InputModality, ToolCatalog};

    let vision_model = create_test_model("gpt-4o", vec![InputModality::Text, InputModality::Image]);
    let tool_input = ToolCatalog::Read(omega_domain::FSRead {
...
fn test_validate_tool_modality_with_non_read_tool() {
    ...
    let text_only_model = create_test_model("gpt-3.5-turbo", vec![InputModality::Text]);
    let tool_input = ToolCatalog::Write(omega_domain::FSWrite {
        file_path: "/home/user/test.png".to_string(),
