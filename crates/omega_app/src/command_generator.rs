...
where
    S: EnvironmentInfra<Config = omega_config::OmegaConfig>
        ...
        + AppConfigService,
{
    ...
    ///
    /// Terminal context is read automatically from the `_OMEGA_TERM_COMMANDS`,
    /// `_OMEGA_TERM_EXIT_CODES`, and `_OMEGA_TERM_TIMESTAMPS` environment
    ...
    pub async fn generate(&self, prompt: UserPrompt) -> Result<String> {
        ...
        let rendered_system_prompt = TemplateEngine::default().render(
            "omega-command-generator-prompt.md",
...
mod tests {
    ...
    impl MockServices {
        ...
        fn with_terminal_context(
            ...
            env_vars.insert("_OMEGA_TERM_COMMANDS".to_string(), commands.to_string());
            env_vars.insert("_OMEGA_TERM_EXIT_CODES".to_string(), exit_codes.to_string());
            env_vars.insert("_OMEGA_TERM_TIMESTAMPS".to_string(), timestamps.to_string());
            Arc::new(Self {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<omega_domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<omega_domain::AnyProvider>> {
            Ok(vec![])
        }
    ...
    impl AppConfigService for MockServices {
        async fn get_session_config(&self) -> Option<omega_domain::ModelConfig> {
            Some(omega_domain::ModelConfig::new(
                ProviderId::OPENAI,
                ModelId::new("test-model"),
            ))
        }

        async fn get_commit_config(&self) -> Result<Option<omega_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_suggest_config(&self) -> Result<Option<omega_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_reasoning_effort(&self) -> Result<Option<omega_domain::Effort>> {
            Ok(None)
        }

        async fn update_config(&self, _ops: Vec<omega_domain::ConfigOperation>) -> Result<()> {
            Ok(())
        }
    }
