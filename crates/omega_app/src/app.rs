...
/// Builds a [`TemplateConfig`] from a [`OmegaConfig`].
///
...
pub(crate) fn build_template_config(config: &OmegaConfig) -> omega_domain::TemplateConfig {
    omega_domain::TemplateConfig {
...
/// OmegaApp handles the core chat functionality by orchestrating various
/// services. It encapsulates the complex logic previously contained in the
/// OmegaAPI chat method.
pub struct OmegaApp<S> {
    services: Arc<S>,
    tool_registry: ToolRegistry<S>,
}

impl<S: Services + EnvironmentInfra<Config = omega_config::OmegaConfig>> OmegaApp<S> {
    /// Creates a new OmegaApp instance with the provided services.
    pub fn new(services: Arc<S>) -> Self {
        Self { tool_registry: ToolRegistry::new(services.clone()), services }
    }

    /// Executes a chat request and returns a stream of responses.
    /// This method contains the core chat logic extracted from OmegaAPI.
    pub async fn chat(
        &self,
        agent_id: AgentId,
        chat: ChatRequest,
        ...
        let conversation = services
            ...
            .await?
            .ok_or_else(|| omega_domain::Error::ConversationNotFound(chat.conversation_id))?;

        // Discover files using the discovery service
        let omega_config = self.services.get_config()?;
        ...
        let conversation =
            SystemPrompt::new(self.services.clone(), environment.clone(), agent.clone())
                ...
                .models(models.clone())
                .files(files.clone())
                .max_extensions(omega_config.max_extensions)
                .template_config(build_template_config(&omega_config))
        ...
        // config
        let on_end_hook = if omega_config.verify_todos {
            tracing_handler
                .clone()
    ...
    pub async fn compact_conversation(
        ...
        let mut conversation = self
            ...
            .await?
            .ok_or_else(|| omega_domain::Error::ConversationNotFound(*conversation_id))?;
