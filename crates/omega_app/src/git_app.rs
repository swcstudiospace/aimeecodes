...
impl<S: Services + EnvironmentInfra<Config = omega_config::OmegaConfig>> GitApp<S> {
    ...
    ///
    /// When `use_omega_committer` is true, sets OmegaLoops as the Git committer
    ...
    /// * `use_omega_committer` - Whether to override the Git committer with
    ///   OmegaLoops identity
    ///
    /// # Errors
    ///
    ...
    pub async fn commit(
        ...
        message: String,
        has_staged_files: bool,
        use_omega_committer: bool,
    ) -> Result<CommitResult> {
        let cwd = self.services.get_environment().cwd;
        let flags = if has_staged_files { "" } else { " -a" };
        let commit_command = build_commit_command(&message, flags, use_omega_committer);
    ...
    async fn generate_message_from_diff(&self, ctx: DiffContext) -> Result<CommitMessageDetails> {
        ...
        let rendered_prompt = self
            .services
            .render_template(Template::new("{{> omega-commit-message-prompt.md }}"), &())
            .await?;
        ...
        let context = omega_domain::Context::default()
...
///
/// When `use_omega_committer` is true, prefixes the command with
/// `GIT_COMMITTER_NAME` and `GIT_COMMITTER_EMAIL` environment variables
/// to set OmegaLoops as the committer.
fn build_commit_command(message: &str, flags: &str, use_omega_committer: bool) -> String {
    // Escape single quotes in the message by replacing ' with '\''
    let escaped_message = message.replace('\'', r"'\''");
    if use_omega_committer {
        format!(
            "GIT_COMMITTER_NAME='OmegaLoops' GIT_COMMITTER_EMAIL='noreply@omegaloops.dev' git commit {flags} -m '{escaped_message}'"
        )
    } else {
...
mod tests {
    ...
    #[test]
    fn test_build_commit_command_with_omega_committer_staged() {
        let actual = build_commit_command("feat: add feature", "", true);
        let expected = "GIT_COMMITTER_NAME='OmegaLoops' GIT_COMMITTER_EMAIL='noreply@omegaloops.dev' git commit  -m 'feat: add feature'";
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_build_commit_command_with_omega_committer_unstaged() {
        let actual = build_commit_command("fix: bug", " -a", true);
        let expected = "GIT_COMMITTER_NAME='OmegaLoops' GIT_COMMITTER_EMAIL='noreply@omegaloops.dev' git commit  -a -m 'fix: bug'";
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_build_commit_command_without_omega_committer_staged() {
    ...
    #[test]
    fn test_build_commit_command_without_omega_committer_unstaged() {
    ...
    fn test_build_commit_command_escapes_single_quotes() {
        ...
        let expected = "GIT_COMMITTER_NAME='OmegaLoops' GIT_COMMITTER_EMAIL='noreply@omegaloops.dev' git commit  -m 'feat: it'\\''s done'";
        assert_eq!(actual, expected);
    }
}
