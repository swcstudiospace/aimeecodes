...
pub struct Runner {
    ...
    config: omega_config::OmegaConfig,
    env: Environment,
}
...
impl Runner {
    fn new(setup: &TestContext) -> Self {
        ...
        omega_embed::register_templates(&mut hb, &TEMPLATE_DIR);
...
impl AgentService for Runner {
    async fn chat_agent(
        &self,
        _id: &omega_domain::ModelId,
        context: omega_domain::Context,
        _provider_id: Option<ProviderId>,
    ) -> omega_domain::ResultStream<ChatCompletionMessage, anyhow::Error> {
    ...
    async fn call(
        &self,
        _: &omega_domain::Agent,
        _: &omega_domain::ToolCallContext,
        test_call: omega_domain::ToolCallFull,
    ) -> omega_domain::ToolResult {
...
impl TemplateService for Runner {
    ...
    async fn render_template<V: serde::Serialize + Send + Sync>(
        &self,
        template: omega_domain::Template<V>,
        object: &V,
...
impl AttachmentService for Runner {
    async fn attachments(&self, _url: &str) -> anyhow::Result<Vec<omega_domain::Attachment>> {
...
impl SkillFetchService for Runner {
    async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<omega_domain::Skill> {
