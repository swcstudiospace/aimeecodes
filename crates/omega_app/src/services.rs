...
pub struct TodoWriteOutput {
    ...
    pub todos: Vec<omega_domain::Todo>,
}
...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: omega_domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: omega_domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &omega_domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<omega_domain::Effort>>;
    ...
    /// all configuration changes; use [`omega_domain::ConfigOperation`]
...
pub trait WorkspaceService: Send + Sync {
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> anyhow::Result<omega_stream::MpscStream<anyhow::Result<SyncProgress>>>;
...
pub trait FsPatchService: Send + Sync {
    ...
    async fn multi_patch(
        &self,
        path: String,
        edits: Vec<omega_domain::PatchEdit>,
    ) -> anyhow::Result<PatchOutput>;
}
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<omega_domain::Image>;
}
...
pub trait FsSearchService: Send + Sync {
    ...
    /// * `Err(_)` - Search error
    async fn search(&self, params: omega_domain::FSSearch) -> anyhow::Result<Option<SearchResult>>;
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<omega_domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    /// Load all command definitions from the omega/commands directory
    async fn get_commands(&self) -> anyhow::Result<Vec<omega_domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &omega_domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<omega_domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<omega_domain::Skill>>;
}
