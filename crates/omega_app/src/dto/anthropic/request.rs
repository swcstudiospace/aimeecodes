...
use omega_domain::{ContextMessage, Image};
...
impl TryFrom<omega_domain::Context> for Request {
    type Error = anyhow::Error;
    fn try_from(request: omega_domain::Context) -> std::result::Result<Self, Self::Error> {
        let system_messages = request
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(msg) if msg.has_role(omega_domain::Role::System) => {
                    Some(SystemMessage {
        ...
        let (thinking, output_config) = if reasoning_on && let Some(reasoning) = request.reasoning {
            ...
            let output_config = reasoning.effort.and_then(|effort| {
                let output_effort = match effort {
                    omega_domain::Effort::None => return None,
                    omega_domain::Effort::Minimal | omega_domain::Effort::Low => OutputEffort::Low,
                    omega_domain::Effort::Medium => OutputEffort::Medium,
                    omega_domain::Effort::High => OutputEffort::High,
                    omega_domain::Effort::XHigh => OutputEffort::XHigh,
                    omega_domain::Effort::Max => OutputEffort::Max,
                };
        ...
        Ok(Self {
            messages: request
                .messages
                .into_iter()
                .filter(|message| !message.has_role(omega_domain::Role::System))
            ...
            output_format: request.response_format.and_then(|rf| match rf {
                omega_domain::ResponseFormat::Text => {
                    // Anthropic doesn't have a "text" output format, so we skip it
                    None
                }
                omega_domain::ResponseFormat::JsonSchema(schema) => {
        ...
        Ok(match value {
            ContextMessage::Text(chat_message) => {
                ...
                match chat_message.role {
                    omega_domain::Role::User => Message { role: Role::User, content },
                    omega_domain::Role::Assistant => Message { role: Role::Assistant, content },
                    omega_domain::Role::System => {
                        ...
                        return Err(
                            omega_domain::Error::UnsupportedRole("System".to_string()).into()
                        );
                    }
                }
            }
...
impl TryFrom<omega_domain::ToolCallFull> for Content {
    type Error = anyhow::Error;
    fn try_from(value: omega_domain::ToolCallFull) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(omega_domain::Error::ToolCallMissingId)?;
...
impl TryFrom<omega_domain::ToolResult> for Content {
    type Error = anyhow::Error;
    fn try_from(value: omega_domain::ToolResult) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(omega_domain::Error::ToolCallMissingId)?;
...
impl From<omega_domain::ToolChoice> for ToolChoice {
    fn from(value: omega_domain::ToolChoice) -> Self {
        match value {
            omega_domain::ToolChoice::Auto => ToolChoice::Auto { disable_parallel_tool_use: None },
            omega_domain::ToolChoice::Call(tool_name) => {
                ToolChoice::Tool { name: tool_name.to_string(), disable_parallel_tool_use: None }
            }
            omega_domain::ToolChoice::Required => {
                ToolChoice::Any { disable_parallel_tool_use: None }
            }
            omega_domain::ToolChoice::None => ToolChoice::Auto { disable_parallel_tool_use: None },
        }
    }
}
...
impl TryFrom<omega_domain::ToolDefinition> for ToolDefinition {
    type Error = anyhow::Error;
    fn try_from(value: omega_domain::ToolDefinition) -> std::result::Result<Self, Self::Error> {
        Ok(ToolDefinition {
            name: value.name.to_string(),
...
mod tests {
    ...
    fn test_reasoning_max_tokens_and_effort_emit_both() {
        ...
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(omega_domain::Effort::Low),
            enabled: Some(true),
            max_tokens: Some(8000),
            exclude: None,
        });
    ...
    fn test_reasoning_effort_with_exclude_emits_adaptive_omitted() {
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(omega_domain::Effort::High),
            enabled: Some(true),
            max_tokens: None,
            exclude: Some(true),
        });
    ...
    fn test_reasoning_xhigh_effort_maps_to_xhigh() {
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(omega_domain::Effort::XHigh),
            enabled: Some(true),
            max_tokens: None,
            exclude: None,
        });
    ...
    fn test_reasoning_effort_none_does_not_emit_anything() {
        ...
        let fixture = Context::default().reasoning(ReasoningConfig {
            enabled: None,
            max_tokens: None,
            effort: Some(omega_domain::Effort::None),
            exclude: None,
        });
