...
impl From<Model> for omega_domain::Model {
    fn from(value: Model) -> Self {
        ...
        {
            vec![
                omega_domain::InputModality::Text,
                omega_domain::InputModality::Image,
            ]
        } else {
            vec![omega_domain::InputModality::Text]
        };
...
impl From<Usage> for omega_domain::Usage {
...
impl From<StopReason> for omega_domain::FinishReason {
    fn from(value: StopReason) -> Self {
        match value {
            StopReason::EndTurn => omega_domain::FinishReason::Stop,
            StopReason::MaxTokens => omega_domain::FinishReason::Length,
            StopReason::StopSequence => omega_domain::FinishReason::Stop,
            StopReason::ToolUse => omega_domain::FinishReason::ToolCalls,
            StopReason::Refusal => omega_domain::FinishReason::ContentFilter,
        }
    }
}
    ...
    fn try_from(value: Event) -> Result<Self, Self::Error> {
        let result = match value {
            ...
            Event::Ping { cost: Some(cost) } => {
                ...
                ChatCompletionMessage::assistant(Content::part(""))
                    .usage(omega_domain::Usage { cost: Some(cost_value), ..Default::default() })
            }
...
mod tests {
    ...
    fn test_usage_conversion_with_cache_read_tokens() {
        ...
        let actual: omega_domain::Usage = fixture.into();
    ...
    fn test_vertex_ai_streaming_usage() {
        ...
        let initial_domain: omega_domain::Usage = initial_usage.into();
        let delta_domain: omega_domain::Usage = delta_usage.into();
    ...
    fn test_model_conversion_includes_context_length() {
        ...
        let actual: omega_domain::Model = fixture.into();
    ...
    fn test_ping_event_with_cost_produces_usage() {
        ...
        let expected_usage = omega_domain::Usage { cost: Some(0.00724710), ..Default::default() };
    ...
    fn test_refusal_maps_to_content_filter() {
        ...
        let actual = omega_domain::FinishReason::from(fixture);

        let expected = omega_domain::FinishReason::ContentFilter;
        assert_eq!(actual, expected);
    }
}
