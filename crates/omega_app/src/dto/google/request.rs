...
use omega_domain::{Context, ContextMessage};
use serde::Serialize;
...
impl From<Context> for Request {
    fn from(context: Context) -> Self {
        ...
        let system_parts: Vec<Part> = context
            .messages
            .iter()
            .filter(|msg| msg.has_role(omega_domain::Role::System))
            .filter_map(|msg| {
        ...
        let generation_config = Some(GenerationConfig {
            ...
            response_mime_type: context.response_format.as_ref().and_then(|rf| match rf {
                omega_domain::ResponseFormat::JsonSchema(_) => Some("application/json".to_string()),
                _ => None,
            }),
...
impl From<omega_domain::ToolChoice> for ToolConfig {
    fn from(choice: omega_domain::ToolChoice) -> Self {
        let (mode, allowed_function_names) = match choice {
            omega_domain::ToolChoice::Auto => (FunctionCallingMode::Auto, None),
            omega_domain::ToolChoice::None => (FunctionCallingMode::None, None),
            omega_domain::ToolChoice::Required => (FunctionCallingMode::Any, None),
            omega_domain::ToolChoice::Call(name) => {
...
impl From<omega_domain::ToolDefinition> for FunctionDeclaration {
    fn from(tool: omega_domain::ToolDefinition) -> Self {
        let mut parameters =
...
impl From<omega_domain::TextMessage> for Content {
    fn from(text_message: omega_domain::TextMessage) -> Self {
        let role = match text_message.role {
            omega_domain::Role::User => Some(Role::User),
            omega_domain::Role::Assistant => Some(Role::Model),
            omega_domain::Role::System => None, // System messages are handled separately
        };
...
impl From<omega_domain::ToolResult> for Content {
    fn from(tool_result: omega_domain::ToolResult) -> Self {
        Content {
...
impl From<omega_domain::Image> for Content {
    fn from(image: omega_domain::Image) -> Self {
        Content { role: Some(Role::User), parts: vec![Part::from(image)] }
    }
}

impl From<omega_domain::ToolCallFull> for Part {
    fn from(tool_call: omega_domain::ToolCallFull) -> Self {
        Part::FunctionCall {
            function_call: FunctionCallData {
...
impl From<omega_domain::ToolResult> for Part {
    fn from(tool_result: omega_domain::ToolResult) -> Self {
...
impl From<omega_domain::Image> for Part {
    fn from(image: omega_domain::Image) -> Self {
        Part::Image {
            inline_data: ImageSource {
...
#[cfg(test)]
mod tests {
    use omega_domain::{ToolCallArguments, ToolCallFull, ToolCallId, ToolName, ToolResult};
    ...
    fn test_consecutive_tool_results_grouped() {
        use omega_domain::{Context, ContextMessage, ModelId};
    ...
    fn test_text_message_conversion() {
        use omega_domain::{Role, TextMessage};
    ...
    fn test_response_schema_strips_dollar_schema() {
        use omega_domain::{Context, ResponseFormat};
        use schemars::schema_for;
    ...
    fn test_tool_result_part_conversion() {
        use omega_domain::{ToolCallId, ToolName, ToolResult};
