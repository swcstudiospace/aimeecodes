...
impl From<Model> for omega_domain::Model {
    fn from(value: Model) -> Self {
        ...
        omega_domain::Model {
            id: omega_domain::ModelId::new(id),
    ...
    fn try_from(value: EventData) -> Result<Self, Self::Error> {
        match value {
            ...
            EventData::Ping(ping) => {
                ...
                let usage = omega_domain::Usage { cost: Some(cost), ..Default::default() };
                Ok(ChatCompletionMessage::assistant(omega_domain::Content::part("")).usage(usage))
            }
            EventData::Unknown(_) => {
...
impl From<UsageMetadata> for omega_domain::Usage {
        ...
        match part {
            Part::Text {
                ...
                if is_thought {
                    ...
                    let mut msg = ChatCompletionMessage::assistant(omega_domain::Content::part(""))
                        .reasoning(omega_domain::Content::part(text_content.clone()))
                    ...
                    let mut msg =
                        ChatCompletionMessage::assistant(omega_domain::Content::part(text_content));
                    if let Some(signature) = thought_signature {
                        msg = msg.thought_signature(signature);
                    }
                    Ok(msg)
            ...
            Part::FunctionCall { function_call, thought_signature } => Ok(
                ChatCompletionMessage::assistant(omega_domain::Content::part("")).add_tool_call(
                    ToolCallPart {
...
impl TryFrom<Candidate> for ChatCompletionMessage {
    ...
    fn try_from(candidate: Candidate) -> Result<Self, Self::Error> {
        ...
        let mut tool_calls: Vec<omega_domain::ToolCall> = Vec::new();
        let mut reasoning_details: Option<Vec<omega_domain::Reasoning>> = None;
        ...
        let mut message = ChatCompletionMessage::assistant(omega_domain::Content::part(content));
        ...
        if !reasoning_parts.is_empty() {
            message = message.reasoning(omega_domain::Content::part(reasoning_parts.join("")));
        }
...
impl TryFrom<Response> for ChatCompletionMessage {
    ...
    fn try_from(response: Response) -> Result<Self, Self::Error> {
        ...
        if let Some(candidate) = response.candidates.into_iter().next() {
            ...
            let mut message = ChatCompletionMessage::assistant(omega_domain::Content::part(""));
...
mod tests {
    ...
    fn test_chat_completion_message_from_part_function_call_generates_id() {
        ...
        match tool_call {
            omega_domain::ToolCall::Part(part) => {
                assert!(part.call_id.is_some());
                let call_id = part.call_id.as_ref().unwrap();
                assert!(call_id.as_str().starts_with("omega_call_id_"));
            }
    ...
    fn test_model_conversion() {
        ...
        let domain_model: omega_domain::Model = model.into();
        ...
        let domain_model_v2: omega_domain::Model = model_v2.clone().into();
    ...
    fn test_ping_event_extracts_cost() {
        ...
        let expected = ChatCompletionMessage::assistant(omega_domain::Content::part(""))
            .usage(omega_domain::Usage { cost: Some(0.028894), ..Default::default() });
    ...
    fn test_usage_metadata_conversion() {
        ...
        let domain_usage: omega_domain::Usage = usage.into();
