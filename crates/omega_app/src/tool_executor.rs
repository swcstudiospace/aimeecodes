...
use anyhow::anyhow;
use omega_domain::{CodebaseQueryResult, ToolCallContext, ToolCatalog, ToolOutput};
    ...
    async fn dump_operation(&self, operation: &ToolOperation) -> anyhow::Result<TempContentFiles> {
        match operation {
            ToolOperation::NetFetch { input: _, output } => {
                ...
                if is_truncated {
                    files = files.stdout(
                        self.create_temp_file("omega_fetch_", ".txt", &output.content)
                            .await?,
                    );
                }
            ...
            ToolOperation::Shell { output } => {
                ...
                if stdout_truncated {
                    files = files.stdout(
                        self.create_temp_file("omega_shell_stdout_", ".txt", &output.output.stdout)
                            .await?,
                    );
                }
                if stderr_truncated {
                    files = files.stderr(
                        self.create_temp_file("omega_shell_stderr_", ".txt", &output.output.stderr)
                            .await?,
                    );
                }
        ...
        Ok(match input {
            ...
            ToolCatalog::SemSearch(input) => {
                ...
                let params: Vec<_> = input
                    ...
                    .iter()
                    .map(|search_query| {
                        omega_domain::SearchParams::new(&search_query.query, &search_query.use_case)
                            .limit(limit)
                            .top_k(top_k)
                    })
                    .collect();
                ...
                let output = omega_domain::CodebaseSearchResults { queries: output };
