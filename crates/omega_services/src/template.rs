...
use anyhow::Context;
use omega_app::{EnvironmentInfra, FileReaderInfra, TemplateService};
use omega_domain::Template;
use futures::future;
use handlebars::Handlebars;
use tokio::sync::{OnceCell, RwLock};

#[derive(Clone)]
pub struct OmegaTemplateService<F> {
    hb: Arc<OnceCell<RwLock<Handlebars<'static>>>>,
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + FileReaderInfra> OmegaTemplateService<F> {
    ...
    async fn get_hb(&self) -> &RwLock<Handlebars<'static>> {
        self.hb
            .get_or_init(|| async { RwLock::new(omega_app::TemplateEngine::handlebar_instance()) })
            .await
    }
...
impl<F: EnvironmentInfra + FileReaderInfra> TemplateService for OmegaTemplateService<F> {
...
mod tests {
    ...
    async fn test_render_simple_template() {
        ...
        let service = OmegaTemplateService::new(Arc::new(MockCompositeService::new()));
        let data = json!({
            "name": "Omega",
            "version": "1.0",
        ...
        let expected = "App: Omega v1.0 - Features: templates, rendering, handlebars";
    ...
    async fn test_render_partial_system_info() {
        ...
        let actual = service
            .render_template(Template::new("{{> omega-partial-system-info.md }}"), &data)
            .await
            .unwrap();
