...
use omega_domain::{FileInfo, Image};
...
pub struct OmegaFsRead<F> {
    infra: Arc<F>,
}

impl<F> OmegaFsRead<F> {
...
impl<F: FileInfoInfra + EnvironmentInfra<Config = omega_config::OmegaConfig> + InfraFsReadService>
    FsReadService for OmegaFsRead<F>
{
    async fn read(
        &self,
        path: String,
            ...
            assert_file_size(&*self.infra, path, config.max_image_size_bytes)
                ...
                .with_context(|| {
                    if mime_type == "application/pdf" {
                        "PDF exceeds size limit. Use a smaller PDF or increase OMEGA_MAX_IMAGE_SIZE."
                    } else {
                        "Image exceeds size limit. Compress the image or increase OMEGA_MAX_IMAGE_SIZE."
                    }
                })?;
