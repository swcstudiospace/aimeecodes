...
use omega_app::SkillFetchService;
use omega_domain::Skill;
...
pub struct OmegaSkillFetch<R> {
    repository: Arc<R>,
    cache: OnceCell<Vec<Skill>>,
}

impl<R> OmegaSkillFetch<R> {
...
impl<R: omega_domain::SkillRepository> SkillFetchService for OmegaSkillFetch<R> {
...
impl<R: omega_domain::SkillRepository> OmegaSkillFetch<R> {
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl omega_domain::SkillRepository for MockSkillRepository {
    ...
    async fn test_fetch_skill_found() {
        ...
        let fetch_service = OmegaSkillFetch::new(Arc::new(repo));
