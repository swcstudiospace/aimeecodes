...
use omega_domain::{FSSearch, OutputMode};
...
pub struct OmegaFsSearch<W> {
    infra: Arc<W>,
}

impl<W> OmegaFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> FsSearchService for OmegaFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> OmegaFsSearch<W> {
...
mod test {
    ...
    use omega_app::{WalkedFile, Walker};
    use omega_domain::{FSSearch, OutputMode};
    use tokio::fs;
    ...
    async fn test_basic_content_search() {
        ...
        let actual = OmegaFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await
            .unwrap();
    ...
    async fn test_path_defaults_to_cwd() {
        ...
        let result = OmegaFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await;
