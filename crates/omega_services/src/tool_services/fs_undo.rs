...
use omega_app::{FileInfoInfra, FileReaderInfra, FsUndoOutput, FsUndoService};
use omega_domain::SnapshotRepository;
...
#[derive(Default)]
pub struct OmegaFsUndo<F> {
    infra: Arc<F>,
}

impl<F> OmegaFsUndo<F> {
...
impl<F: FileInfoInfra + FileReaderInfra + SnapshotRepository> FsUndoService for OmegaFsUndo<F> {
