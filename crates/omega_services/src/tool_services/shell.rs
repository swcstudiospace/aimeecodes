...
use anyhow::bail;
use bstr::ByteSlice;
use omega_app::domain::Environment;
use omega_app::{CommandInfra, EnvironmentInfra, ShellOutput, ShellService};
...
/// unrestricted access, advise users to run omega CLI with '-u' flag. Returns
/// complete output including stdout, stderr, and exit code for diagnostic
/// purposes.
pub struct OmegaShell<I> {
    env: Environment,
    infra: Arc<I>,
}

impl<I: EnvironmentInfra> OmegaShell<I> {
...
impl<I: CommandInfra + EnvironmentInfra> ShellService for OmegaShell<I> {
    async fn execute(
        &self,
        command: String,
        cwd: PathBuf,
...
mod tests {
    ...
    use omega_app::domain::{CommandOutput, Environment};
    use omega_app::{CommandInfra, EnvironmentInfra, ShellService};
    use omega_domain::ConfigOperation;
    ...
    async fn test_shell_service_forwards_env_vars() {
        let fixture = OmegaShell::new(Arc::new(MockCommandInfra {
    ...
    async fn test_shell_service_forwards_no_env_vars() {
        let fixture = OmegaShell::new(Arc::new(MockCommandInfra { expected_env_vars: None }));
