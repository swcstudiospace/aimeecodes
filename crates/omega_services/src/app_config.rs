...
use omega_app::{AppConfigService, EnvironmentInfra};
use omega_domain::{ConfigOperation, Effort, ModelConfig, ModelId, ProviderId, ProviderRepository};
use tracing::debug;
...
pub struct OmegaAppConfigService<F> {
    infra: Arc<F>,
}

impl<F> OmegaAppConfigService<F> {
...
impl<F: ProviderRepository + EnvironmentInfra<Config = omega_config::OmegaConfig> + Send + Sync>
    AppConfigService for OmegaAppConfigService<F>
{
    ...
    async fn get_commit_config(&self) -> anyhow::Result<Option<omega_domain::ModelConfig>> {
    ...
    async fn get_suggest_config(&self) -> anyhow::Result<Option<omega_domain::ModelConfig>> {
        let config = self.infra.get_config()?;
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<Effort>> {
        ...
        Ok(config
            ...
            .and_then(|r| r.effort)
            .map(|e| match e {
                omega_config::Effort::None => Effort::None,
                omega_config::Effort::Minimal => Effort::Minimal,
                omega_config::Effort::Low => Effort::Low,
                omega_config::Effort::Medium => Effort::Medium,
                omega_config::Effort::High => Effort::High,
                omega_config::Effort::XHigh => Effort::XHigh,
                omega_config::Effort::Max => Effort::Max,
            }))
    }
...
mod tests {
    ...
    use omega_config::{OmegaConfig, ModelConfig};
    // Alias to avoid collision with omega_config::ModelConfig used in test fixtures
    use omega_domain::ModelConfig as DomainModelConfig;
    use omega_domain::{
    ...
    impl MockInfra {
        fn new() -> Self {
            Self {
                config: Arc::new(Mutex::new(OmegaConfig::default())),
                providers: vec![
                    Provider {
                        ...
                        credential: Some(omega_domain::AuthCredential {
                            id: ProviderId::OPENAI,
                            auth_details: omega_domain::AuthDetails::ApiKey(
                                omega_domain::ApiKey::from("test-key".to_string()),
                            ),
    ...
    impl ChatRepository for MockInfra {
        async fn chat(
            &self,
            _model_id: &omega_app::domain::ModelId,
            _context: omega_app::domain::Context,
            _provider: Provider<Url>,
        ) -> omega_app::domain::ResultStream<omega_app::domain::ChatCompletionMessage, anyhow::Error>
        {
        ...
        async fn models(
            &self,
            _provider: Provider<Url>,
        ) -> anyhow::Result<Vec<omega_app::domain::Model>> {
            Ok(vec![])
        }
    }
        ...
        async fn get_provider(&self, id: ProviderId) -> anyhow::Result<ProviderTemplate> {
            ...
            self.providers
                ...
                .map(|p| Provider {
                    ...
                    url: omega_domain::Template::<omega_domain::URLParameters>::new(p.url.as_str()),
                    models: p.models.as_ref().map(|m| match m {
                        ModelSource::Url(url) => ModelSource::Url(omega_domain::Template::<
                            omega_domain::URLParameters,
                        >::new(
                            url.as_str()
                        )),
    ...
    async fn test_get_session_config_when_none_set() -> anyhow::Result<()> {
        ...
        let service = OmegaAppConfigService::new(Arc::new(fixture));
    ...
    async fn test_get_session_config_when_set() -> anyhow::Result<()> {
        let fixture = MockInfra::new();
        let service = OmegaAppConfigService::new(Arc::new(fixture.clone()));
    ...
    async fn test_set_multiple_default_models() -> anyhow::Result<()> {
        ...
        // OmegaConfig only tracks a single active session, so the last
