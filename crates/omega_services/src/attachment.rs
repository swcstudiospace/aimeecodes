...
use omega_app::utils::format_display_path;
use omega_app::{
...
impl<
    F: FileReaderInfra
        ...
        + FileInfoInfra
        + DirectoryReaderInfra,
> AttachmentService for OmegaChatRequest<F>
{
...
pub mod tests {
    ...
    use base64::Engine;
    use bytes::Bytes;
    use omega_app::domain::{AttachmentContent, Environment};
    use omega_app::utils::compute_hash;
    use omega_app::{
    ...
    use omega_domain::{ConfigOperation, FileInfo};
    use futures::stream;
    ...
    impl EnvironmentInfra for MockEnvironmentInfra {
        ...
        fn get_config(&self) -> anyhow::Result<omega_config::OmegaConfig> {
            Ok(omega_config::OmegaConfig { max_read_lines: 2000, ..Default::default() })
        }
    ...
    async fn test_add_url_with_text_file() {
        ...
        let chat_request = OmegaChatRequest::new(infra.clone());
