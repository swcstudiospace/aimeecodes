...
use omega_app::{FileReaderInfra, SyncProgressCounter, WorkspaceStatus, compute_hash};
use omega_domain::{ApiKey, FileHash, SyncProgress, UserId, WorkspaceId, WorkspaceIndexRepository};
...
/// Extracts [`omega_domain::FileStatus`] entries with
/// [`omega_domain::SyncStatus::Failed`] from a slice of file-read results by
/// downcasting errors to [`FileReadError`].
fn extract_failed_statuses<T>(results: &[Result<T>]) -> Vec<omega_domain::FileStatus> {
    results
        .iter()
        .filter_map(|r| r.as_ref().err())
        .filter_map(|e| e.downcast_ref::<FileReadError>())
        .map(|e| {
            omega_domain::FileStatus::new(
                e.path.to_string_lossy().into_owned(),
                omega_domain::SyncStatus::Failed,
            )
        })
        .collect()
}
    ...
    WorkspaceSyncEngine<F, D>
{
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let added = statuses
            .iter()
            .filter(|s| s.status == omega_domain::SyncStatus::New)
            .count();
        let deleted = statuses
            .iter()
            .filter(|s| s.status == omega_domain::SyncStatus::Deleted)
            .count();
        let modified = statuses
            .iter()
            .filter(|s| s.status == omega_domain::SyncStatus::Modified)
            .count();
        let mut failed_files = statuses
            .iter()
            .filter(|s| s.status == omega_domain::SyncStatus::Failed)
            .count();
        ...
        if failed_files > 0 {
            Err(omega_domain::Error::sync_failed(failed_files).into())
        } else {
            Ok(())
        }
    }
    ...
    pub async fn compute_status(&self) -> Result<Vec<omega_domain::FileStatus>> {
        let results: Vec<Result<FileHash>> = self.read_hashes().collect().await;
    ...
    async fn fetch_remote_hashes(&self) -> anyhow::Result<Vec<FileHash>> {
        ...
        let workspace_files =
            omega_domain::CodeBase::new(self.user_id.clone(), self.workspace_id.clone(), ());
        self.infra
    ...
    async fn delete_files(&self, files_to_delete: Vec<PathBuf>) -> Result<usize> {
        ...
        let deletion =
            omega_domain::CodeBase::new(self.user_id.clone(), self.workspace_id.clone(), paths);
        self.infra
    ...
    /// [`omega_domain::FileUpload`] payload, and uploaded in one request.
            ...
            .then(move |batch| {
                ...
                async move {
                    ...
                    for file_path in &batch {
                        ...
                        files.push(omega_domain::FileRead::new(
                            file_path.to_string_lossy().into_owned(),
                            content,
                        ));
                    }
                    let upload =
                        omega_domain::CodeBase::new(user_id.clone(), workspace_id.clone(), files);
                    infra
...
mod tests {
    ...
    impl WorkspaceIndexRepository for MockInfra {
        ...
        async fn search(&self, _: &omega_domain::CodeSearchQuery<'_>, _: &ApiKey) -> anyhow::Result<Vec<Node>> { unreachable!() }
