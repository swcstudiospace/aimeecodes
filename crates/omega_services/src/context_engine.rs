...
use omega_app::{CommandInfra, EnvironmentInfra, FileReaderInfra, WalkerInfra, WorkspaceService};
use omega_domain::{
...
pub struct OmegaWorkspaceService<F, D> {
    infra: Arc<F>,
    discovery: Arc<D>,
}

impl<F, D> Clone for OmegaWorkspaceService<F, D> {
    fn clone(&self) -> Self {
        Self {
...
impl<F, D> OmegaWorkspaceService<F, D> {
...
> OmegaWorkspaceService<F, D>
{
    ...
    /// Gets the OmegaLoops services credential and extracts workspace auth
    /// components
    ///
    /// # Errors
    /// Returns an error if the credential is not found, if there's a database
    /// error, or if the credential format is invalid
    async fn get_workspace_credentials(&self) -> Result<(omega_domain::ApiKey, UserId)> {
        let credential = self
            .infra
            .get_credential(&ProviderId::OMEGA_SERVICES)
            .await?
        ...
        match &credential.auth_details {
            AuthDetails::ApiKey(token) => {
                ...
                let user_id_str = credential
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!("Missing user_id in OmegaServices credential")
                    })?;
                let user_id = UserId::from_string(user_id_str.as_str())?;

                Ok((token.clone(), user_id))
            }
            _ => anyhow::bail!("OmegaServices credential must be an API key"),
        }
    }
    ...
    async fn find_workspace_by_path(
        ...
        let mut best_match: Option<(&omega_domain::WorkspaceInfo, usize)> = None;
    ...
    async fn get_workspace_by_path(
        ...
        path: PathBuf,
        token: &omega_domain::ApiKey,
    ) -> Result<omega_domain::WorkspaceInfo> {
        self.find_workspace_by_path(path, token)
            .await?
            .context("Workspace not indexed. Please run `omega workspace init` first.")
    }
...
impl<
    F: ProviderRepository
        ...
        + 'static,
    D: FileDiscovery + 'static,
> WorkspaceService for OmegaWorkspaceService<F, D>
{
    ...
    async fn query_workspace(
        ...
        let workspace = self
            ...
            .await?
            .ok_or(omega_domain::Error::WorkspaceNotFound)?;

        let search_query =
            omega_domain::CodeBase::new(user_id, workspace.workspace_id.clone(), params);
    ...
    async fn delete_workspace(&self, workspace_id: &omega_domain::WorkspaceId) -> Result<()> {
        let (token, _) = self.get_workspace_credentials().await?;
    ...
    /// database.
    async fn delete_workspaces(&self, workspace_ids: &[omega_domain::WorkspaceId]) -> Result<()> {
    ...
    async fn init_auth_credentials(&self) -> Result<omega_domain::WorkspaceAuth> {
    ...
    async fn init_workspace(&self, path: PathBuf) -> Result<WorkspaceId> {
        ...
        if is_new {
            ...
            Err(omega_domain::Error::WorkspaceAlreadyInitialized(workspace_id).into())
        }
    }
}
