...
pub struct OmegaProviderService<R> {
    repository: Arc<R>,
}

impl<R> OmegaProviderService<R> {
    /// Creates a new OmegaProviderService instance
    ...
    fn render_url_template(
        &self,
        template: &str,
        params: &HashMap<omega_domain::URLParam, omega_domain::URLParamValue>,
        specs: &[omega_domain::URLParamSpec],
    ) -> Result<Url> {
        ...
        let handlebars = omega_app::TemplateEngine::handlebar_instance();
...
impl<R: ChatRepository + ProviderRepository> ProviderService for OmegaProviderService<R> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
...
#[cfg(test)]
mod tests {
    use omega_app::domain::ProviderId;
    use omega_domain::{
    ...
    fn test_provider() -> Provider<Url> {
        Provider {
            ...
            response: Some(omega_app::domain::ProviderResponse::OpenAI),
            url: Url::parse("https://api.openai.com/v1/chat/completions").unwrap(),
            auth_methods: vec![AuthMethod::ApiKey],
            url_params: vec![],
            credential: Some(AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: AuthDetails::ApiKey(omega_domain::ApiKey::from(
                    "test-key".to_string(),
                )),
    ...
    fn test_template_provider() -> ProviderTemplate {
        Provider {
            ...
            provider_type: ProviderType::Llm,
            response: Some(omega_app::domain::ProviderResponse::OpenAI),
            url: Template::<omega_domain::URLParameters>::new(
            ...
            models: Some(ModelSource::Url(
                Template::<omega_domain::URLParameters>::new("https://api.openai.com/v1/models"),
            )),
            custom_headers: None,
        }
    }
    ...
    async fn test_models_delegates_to_repository() {
        ...
        let service = OmegaProviderService::new(repository);
    ...
    fn test_render_url_template_optional_port_absent() {
        ...
        let service = OmegaProviderService::new(Arc::new(MockProviderRepository::new(vec![])));
        let template = "{{VLLM_SSL_SCHEME}}://{{VLLM_HOST}}{{#if VLLM_PORT}}:{{VLLM_PORT}}{{/if}}/v1/chat/completions";

        let mut params = HashMap::new();
        params.insert(
            omega_domain::URLParam::from("VLLM_SSL_SCHEME".to_string()),
            omega_domain::URLParamValue::from("https".to_string()),
        );
        params.insert(
            omega_domain::URLParam::from("VLLM_HOST".to_string()),
            omega_domain::URLParamValue::from("my.server.url".to_string()),
        );
        // VLLM_PORT intentionally absent — not in the credential map at all.

        let specs = vec![omega_domain::URLParamSpec::optional(
            omega_domain::URLParam::from("VLLM_PORT".to_string()),
        )];
    ...
    fn test_render_url_template_optional_port_with_value() {
        ...
        params.insert(
            omega_domain::URLParam::from("VLLM_PORT".to_string()),
            omega_domain::URLParamValue::from("8000".to_string()),
        );
