...
use anyhow::Result;
use omega_app::domain::File;
use omega_app::{
    DirectoryReaderInfra, EnvironmentInfra, FileDiscoveryService, Walker, WalkerInfra,
};

pub struct OmegaDiscoveryService<F> {
    service: Arc<F>,
}

impl<F> OmegaDiscoveryService<F> {
    pub fn new(service: Arc<F>) -> Self {
        Self { service }
    }
}

impl<F: EnvironmentInfra + WalkerInfra> OmegaDiscoveryService<F> {
...
mod tests {
    ...
    use omega_app::{EnvironmentInfra, WalkedFile};
    ...
    async fn test_list_current_directory_sorts_dirs_first() {
        ...
        let service = OmegaDiscoveryService::new(Arc::new(fixture));
