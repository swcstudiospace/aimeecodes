...
use aimee_app::dto::ToolsOverview;
use aimee_app::{
    AgentProviderResolver, AgentRegistry, AimeeApp, AppConfigService, AuthService, CommandInfra,
    CommandLoaderService, ConversationService, DataGenerationApp, EnvironmentInfra,
    FileDiscoveryService, GitApp, GrpcInfra, McpConfigManager, McpService, ProviderAuthService,
    ProviderService, Services, User, UserUsage, Walker, WorkspaceService,
};
use aimee_config::AimeeConfig;
use aimee_domain::{Agent, ConsoleWriter, *};
use aimee_infra::AimeeInfra;
use aimee_repo::AimeeRepo;
use aimee_services::AimeeServices;
...
pub struct AimeeAPI<S, F> {
    services: Arc<S>,
    infra: Arc<F>,
}

impl<A, F> AimeeAPI<A, F> {
    pub fn new(services: Arc<A>, infra: Arc<F>) -> Self {
        Self { services, infra }
    }

    /// Creates a AimeeApp instance with the current services and latest config.
    fn app(&self) -> AimeeApp<A>
    where
        A: Services + EnvironmentInfra<Config = aimee_config::AimeeConfig>,
        F: EnvironmentInfra<Config = aimee_config::AimeeConfig>,
    {
        AimeeApp::new(self.services.clone())
    }
}

impl AimeeAPI<AimeeServices<AimeeRepo<AimeeInfra>>, AimeeRepo<AimeeInfra>> {
    /// Creates a fully-initialized [`AimeeAPI`] from a pre-read configuration.
    ///
    /// # Arguments
    ...
    pub fn init(cwd: PathBuf, config: AimeeConfig) -> Self {
        let infra = Arc::new(AimeeInfra::new(cwd, config));
        let repo = Arc::new(AimeeRepo::new(infra.clone()));
        let app = Arc::new(AimeeServices::new(repo.clone()));
        AimeeAPI::new(app, repo)
    }

    pub async fn get_skills_internal(&self) -> Result<Vec<Skill>> {
        use aimee_domain::SkillRepository;
...
> API for AimeeAPI<A, F>
{
    ...
    async fn commit(
    ...
    ) -> Result<aimee_app::CommitResult> {
        let use_aimee_committer = self
            .services
            .get_config()
            .context("Failed to read aimee config for commit settings")?
            .use_aimee_committer;
        ...
        if preview {
            ...
            git_app
                .commit(result.message, result.has_staged_files, use_aimee_committer)
                .await
        }
    }
    ...
    async fn update_config(&self, ops: Vec<aimee_domain::ConfigOperation>) -> anyhow::Result<()> {
        ...
        let needs_agent_reload = ops
            .iter()
            .any(|op| matches!(op, aimee_domain::ConfigOperation::SetSessionConfig(_)));
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use aimee_app::CommandGenerator;
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> Result<MpscStream<Result<aimee_domain::SyncProgress>>> {
    ...
    ) -> Result<Vec<aimee_domain::Node>> {
        self.services.query_workspace(path, params).await
    }

    async fn list_workspaces(&self) -> Result<Vec<aimee_domain::WorkspaceInfo>> {
    ...
    async fn get_workspace_info(
        &self,
        path: PathBuf,
    ) -> Result<Option<aimee_domain::WorkspaceInfo>> {
        self.services.get_workspace_info(path).await
    }

    async fn delete_workspaces(&self, workspace_ids: Vec<aimee_domain::WorkspaceId>) -> Result<()> {
        self.services.delete_workspaces(&workspace_ids).await
    }

    async fn get_workspace_status(&self, path: PathBuf) -> Result<Vec<aimee_domain::FileStatus>> {
    ...
    async fn create_auth_credentials(&self) -> Result<aimee_domain::WorkspaceAuth> {
        self.services.init_auth_credentials().await
    }

    async fn init_workspace(&self, path: PathBuf) -> Result<aimee_domain::WorkspaceId> {
        self.services.init_workspace(path).await
    }

    async fn migrate_env_credentials(&self) -> Result<Option<aimee_domain::MigrationResult>> {
    ...
    async fn get_session_config(&self) -> Option<aimee_domain::ModelConfig> {
    ...
    async fn get_default_provider(&self) -> Result<Provider<Url>> {
        let model_config = self
            ...
            .get_session_config()
            .await
            .ok_or_else(|| aimee_domain::Error::NoDefaultSession)?;
    ...
    async fn mcp_auth(&self, server_url: &str) -> Result<()> {
        ...
        aimee_infra::mcp_auth(server_url, &env).await
    }

    async fn mcp_logout(&self, server_url: Option<&str>) -> Result<()> {
        let env = self.services.get_environment().clone();
        match server_url {
            Some(url) => aimee_infra::mcp_logout(url, &env).await,
            None => aimee_infra::mcp_logout_all(&env).await,
        }
    }

    async fn mcp_auth_status(&self, server_url: &str) -> Result<String> {
        let env = self.services.get_environment().clone();
        Ok(aimee_infra::mcp_auth_status(server_url, &env).await)
    }
...
impl<A: Send + Sync, F: ConsoleWriter> ConsoleWriter for AimeeAPI<A, F> {
