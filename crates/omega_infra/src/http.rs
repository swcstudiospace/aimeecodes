...
use omega_config::{OmegaConfig, TlsBackend, TlsVersion};
use omega_eventsource::{EventSource, RequestBuilderExt};
...
impl<F: omega_app::FileWriterInfra + 'static> OmegaHttpInfra<F> {
    /// Creates a new [`OmegaHttpInfra`] from a resolved [`OmegaConfig`].
    pub fn new(config: OmegaConfig, file_writer: Arc<F>) -> Self {
        let http = config.http.unwrap_or(omega_config::HttpConfig {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        if !headers.contains_key("User-Agent") {
            headers.insert("User-Agent", HeaderValue::from_static("Omega-Loops"));
        }
        headers.insert("X-Title", HeaderValue::from_static("Omega Loops"));
        headers.insert(
            "x-app-version",
        ...
        headers.insert(
            "HTTP-Referer",
            HeaderValue::from_static("https://omegaloops.dev"),
        );
        headers.insert(
...
impl<F: omega_app::FileWriterInfra + 'static> HttpInfra for OmegaHttpInfra<F> {
...
mod tests {
    ...
    fn create_test_config(debug_requests: Option<PathBuf>) -> OmegaConfig {
        OmegaConfig { debug_requests, ..Default::default() }
    }
    ...
    async fn test_debug_requests_none_does_not_write() {
        ...
        let http = OmegaHttpInfra::new(config, Arc::new(file_writer.clone()));
    ...
    async fn test_debug_requests_with_valid_path() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/omega-test/debug.json");
    ...
    async fn test_debug_requests_post_writes_body() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/omega-test/debug-post.json");
        let config = create_test_config(Some(debug_path.clone()));
        let http = OmegaHttpInfra::new(config, Arc::new(file_writer.clone()));
    ...
    fn test_sanitize_headers_redacts_sensitive_values() {
        ...
        headers.insert("x-title", HeaderValue::from_static("omega"));
        ...
        assert_eq!(
            ...
            Some(&HeaderValue::from_static("omega"))
        );
        assert_eq!(
