...
#[derive(Clone)]
pub struct OmegaFileReadService;

impl Default for OmegaFileReadService {
    fn default() -> Self {
        Self
    }
}

impl OmegaFileReadService {
    pub fn new() -> Self {
        Self
    }
}
...
impl FileReaderInfra for OmegaFileReadService {
    async fn read_utf8(&self, path: &Path) -> Result<String> {
        omega_fs::OmegaFS::read_utf8(path).await
    }
    ...
    async fn read(&self, path: &Path) -> Result<Vec<u8>> {
        omega_fs::OmegaFS::read(path).await
    }
    ...
    async fn range_read_utf8(
        ...
        start_line: u64,
        end_line: u64,
    ) -> Result<(String, omega_domain::FileInfo)> {
        omega_fs::OmegaFS::read_range_utf8(path, start_line, end_line).await
    }
}
...
mod tests {
    ...
    async fn test_read_batch_utf8() {
        let fixture = OmegaFileReadService::new();
