...
impl OmegaMcpClient {
    ...
    fn client_info(&self) -> ClientInfo {
        ClientInfo::new(Default::default(), Implementation::new("Omega", VERSION))
    }
                    ...
                    match self.create_standard_http_connection(http).await {
                        ...
                        Err(e) => {
                                ...
                                || error_str.contains("oauth")
                            {
                                ...
                                let default_config = omega_domain::McpOAuthConfig::default();
    ...
    async fn create_oauth_connection(
        &self,
        http: &McpHttpServer,
        oauth_config: &omega_domain::McpOAuthConfig,
        ...
        // CSRF state
        oauth_state
            .start_authorization(&scopes, &redirect_uri, Some("Omega"))
            .await
    ...
    async fn run_oauth_callback_server(
        ...
        let response_body = r#"<!doctype html><html><head><title>Omega - Authorization Successful</title><meta charset="utf-8"></head><body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#111827;color:#f9fafb;"><div style="text-align:center;padding:2rem;"><h1 style="margin-bottom:0.75rem;">Authorization Successful</h1><p style="color:#d1d5db;">You can close this window and return to Omega.</p></div><script>setTimeout(()=>window.close(),2000)</script></body></html>"#;
        let response = format!(
...
impl McpClientInfra for OmegaMcpClient {
    async fn list(&self) -> anyhow::Result<Vec<ToolDefinition>> {
...
pub async fn mcp_auth(server_url: &str, env: &Environment) -> anyhow::Result<()> {
    ...
    oauth_state
        .start_authorization(&[], redirect_uri, Some("Omega"))
        .await
        .map_err(|e| anyhow::anyhow!("OAuth authorization flow failed: {}", e))?;
