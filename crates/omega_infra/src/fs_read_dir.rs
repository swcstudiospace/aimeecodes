...
use anyhow::Result;
use omega_app::DirectoryReaderInfra;
...
pub struct OmegaDirectoryReaderService {
    parallel_file_reads: usize,
}

impl OmegaDirectoryReaderService {
    ...
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
        ...
        if !OmegaFS::exists(directory) || OmegaFS::is_file(directory) {
            return Ok(vec![]);
        }
    ...
    async fn read_directory_files(
        ...
        let mut dir = OmegaFS::read_dir(directory).await?;
        ...
        let files = futures::stream::iter(file_paths)
            .map(|path| async move {
                ...
                match OmegaFS::read_to_string(&path).await {
...
impl DirectoryReaderInfra for OmegaDirectoryReaderService {
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
...
mod tests {
    ...
    async fn test_read_directory_files_with_filter() {
        ...
        let actual = OmegaDirectoryReaderService::new(64)
    ...
    async fn test_read_directory_files_without_filter() {
        ...
        let mut actual = OmegaDirectoryReaderService::new(64)
