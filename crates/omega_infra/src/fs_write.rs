...
pub struct OmegaFileWriteService;

impl OmegaFileWriteService {
    pub fn new() -> Self {
        Self
    }
    ...
    async fn create_parent_dirs(&self, path: &Path) -> anyhow::Result<()> {
        if !omega_fs::OmegaFS::exists(path)
            && let Some(parent) = path.parent()
        {
            omega_fs::OmegaFS::create_dir_all(parent).await?;
        }
        Ok(())
    }
}

impl Default for OmegaFileWriteService {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FileWriterInfra for OmegaFileWriteService {
    async fn write(&self, path: &Path, contents: Bytes) -> anyhow::Result<()> {
        self.create_parent_dirs(path).await?;
        Ok(omega_fs::OmegaFS::write(path, contents.to_vec()).await?)
    }

    async fn append(&self, path: &Path, contents: Bytes) -> anyhow::Result<()> {
        self.create_parent_dirs(path).await?;
        Ok(omega_fs::OmegaFS::append(path, contents.to_vec()).await?)
    }
...
mod tests {
    ...
    fn create_test_service() -> OmegaFileWriteService {
        OmegaFileWriteService::new()
    }
