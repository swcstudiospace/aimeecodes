...
use omega_app::{AuthStrategy, OAuthHttpProvider, StrategyFactory};
use omega_domain::{
...
async fn poll_for_tokens(
    device_code: &omega_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
pub struct OmegaAuthStrategyFactory;

impl Default for OmegaAuthStrategyFactory {
    fn default() -> Self {
        Self
    }
}

impl OmegaAuthStrategyFactory {
    pub fn new(_environment: omega_domain::Environment) -> Self {
        Self
    }
}

impl StrategyFactory for OmegaAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            omega_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            omega_domain::AuthMethod::OAuthCode(config) => {
            ...
            omega_domain::AuthMethod::OAuthDevice(config) => {
            ...
            omega_domain::AuthMethod::GoogleAdc => Ok(AnyAuthStrategy::GoogleAdc(
                GoogleAdcStrategy::new(provider_id, required_params),
            )),
            omega_domain::AuthMethod::AwsProfile => Ok(AnyAuthStrategy::AwsProfile(
                AwsProfileStrategy::new(provider_id, required_params),
            )),
            omega_domain::AuthMethod::CodexDevice(config) => Ok(AnyAuthStrategy::CodexDevice(
...
mod tests {
    ...
    fn test_create_auth_strategy_api_key() {
        let factory = OmegaAuthStrategyFactory;
    ...
    fn test_create_auth_strategy_codex_device() {
        ...
        let actual = factory.create_auth_strategy(
            ProviderId::CODEX,
            omega_domain::AuthMethod::CodexDevice(config),
            vec![],
        );
        assert!(actual.is_ok());
    ...
    fn test_enrich_codex_oauth_credential_uses_id_token_claims() {
        ...
        let expected = Some(&omega_domain::URLParamValue::from(
