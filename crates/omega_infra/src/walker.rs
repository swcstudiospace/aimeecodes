...
pub struct OmegaWalkerService;

impl OmegaWalkerService {
    pub fn new() -> Self {
        Self
    }

    pub async fn walk(&self, config: Walker) -> Result<Vec<WalkedFile>> {
        // Convert domain config to omega_walker config
            ...
            && config.max_total_size.is_none()
        {
            ...
            omega_walker::Walker::max_all().hidden(true)
        } else {
            omega_walker::Walker::min_all()
        };
...
mod tests {
    ...
    async fn test_walker_service_basic_functionality() {
        ...
        let service = OmegaWalkerService::new();
