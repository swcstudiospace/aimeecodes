...
use omega_api::API;
use omega_config::{Update, UpdateFrequency};
use omega_select::OmegaWidget;
use omega_tracker::VERSION;
use update_informer::{Check, Version, registry};

/// Runs the official installation script to update Omega, failing silently.
...
async fn execute_update_command(api: Arc<impl API>, auto_update: bool) {
    ...
    let output = api
        .execute_shell_command_raw("curl -fsSL https://omegaloops.dev/cli | sh")
        .await;
        ...
        Ok(output) => {
            if output.success() {
                let should_exit = if auto_update {
                    ...
                    let answer = omega_select::OmegaWidget::confirm(
                        "You need to close omega to complete update. Do you want to close it now?",
                    )
                    .with_default(true)
                    .prompt();
...
async fn confirm_update(version: Version) -> bool {
    let answer = OmegaWidget::confirm(format!(
...
pub async fn on_update(api: Arc<impl API>, update: Option<&Update>) {
    ...
    let informer = update_informer::new(registry::GitHub, "omegaloops/omegaloops", VERSION)
