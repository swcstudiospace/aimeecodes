...
pub fn generate_zsh_plugin() -> Result<String> {
    ...
    for file in omega_embed::files(&ZSH_PLUGIN_LIB) {
    ...
    generate(Zsh, &mut cmd, "omega", &mut completions);
    ...
    output.push_str("\n_OMEGA_PLUGIN_LOADED=$(date +%s)\n");

    Ok(output)
}

/// Generates the ZSH theme for Omega
pub fn generate_zsh_theme() -> Result<String> {
    let mut content =
        super::normalize_script(include_str!("../../../../shell-plugin/omega.theme.zsh"));

    // Set environment variable to indicate theme is loaded (with timestamp)
    content.push_str("\n_OMEGA_THEME_LOADED=$(date +%s)\n");
...
fn create_temp_zsh_script(script_content: &str) -> Result<(tempfile::TempDir, PathBuf)> {
    ...
    let script_path = temp_dir.path().join("omega_script.zsh");
...
/// * `omega_editor` - If Some(editor), adds OMEGA_EDITOR export to .zshrc
///
/// # Errors
///
/// Returns error if:
...
/// - Invalid omega markers are found (incomplete or incorrectly ordered)
/// - A backup of the existing .zshrc cannot be created
pub fn setup_zsh_integration(
    disable_nerd_font: bool,
    omega_editor: Option<&str>,
) -> Result<ZshSetupResult> {
    const START_MARKER: &str = "# >>> omega initialize >>>";
    const END_MARKER: &str = "# <<< omega initialize <<<";
    const OMEGA_INIT_CONFIG_RAW: &str = include_str!("../../../../shell-plugin/omega.setup.zsh");
    let omega_init_config = super::normalize_script(OMEGA_INIT_CONFIG_RAW);
    ...
    // Build the omega config block with markers
    let mut omega_config: Vec<String> = vec![START_MARKER.to_string()];
    omega_config.extend(omega_init_config.lines().map(String::from));

    // Add nerd font configuration if requested
    if disable_nerd_font {
        omega_config.push(String::new()); // Add blank line before comment
        omega_config.push(
            "# Disable Nerd Fonts (set during setup - icons not displaying correctly)".to_string(),
        );
        omega_config.push("# To re-enable: remove this line and install a Nerd Font from https://www.nerdfonts.com/".to_string());
        omega_config.push("export NERD_FONT=0".to_string());
    }

    // Add editor configuration if requested
    if let Some(editor) = omega_editor {
        omega_config.push(String::new()); // Add blank line before comment
        omega_config.push("# Editor for editing prompts (set during setup)".to_string());
        omega_config.push("# To change: update OMEGA_EDITOR or remove to use $EDITOR".to_string());
        omega_config.push(format!("export OMEGA_EDITOR=\"{}\"", editor));
    }

    omega_config.push(END_MARKER.to_string());

    // Add or update omega configuration block based on marker state
    let (new_content, config_action) = match marker_state {
        MarkerState::Valid { start, end } => {
            // Markers exist - replace content between them
            lines.splice(start..=end, omega_config.iter().cloned());
            (lines.join("\n") + "\n", "updated")
        }
        MarkerState::Invalid { start, end } => {
            ...
            let mut error =
                anyhow::anyhow!("Invalid omega markers found in {}", zshrc_path.display());
        ...
        MarkerState::NotFound => {
            ...
            lines.extend(omega_config.iter().cloned());
            (lines.join("\n") + "\n", "added")
        }
    };
    ...
    Ok(ZshSetupResult {
        message: format!("omega plugins {}", config_action),
        backup_path,
    })
}
...
mod tests {
    ...
    fn test_run_zsh_doctor_streaming() {
        ...
        unsafe {
            std::env::set_var("OMEGA_SKIP_INTERACTIVE", "1");
        }
        ...
        unsafe {
            std::env::remove_var("OMEGA_SKIP_INTERACTIVE");
        }
    ...
    fn test_generated_plugin_wraps_zle_commands_with_osc133_markers() {
        ...
        let actual = fixture.contains("    _omega_osc133_emit \"B\"\n    _omega_osc133_emit \"C\"")
            ...
            // _omega_reset pads with newlines before clearing to prevent ZLE from clearing
            ...
            // D and A markers follow the action, with _omega_reset at the end
            && fixture.contains(
    ...
    /// Regression: omega keybindings must survive zsh-vi-mode's `zvm_init`
    ...
    fn test_generated_plugin_registers_zvm_after_init_hook() {
        ...
        let fixture = generate_zsh_plugin().unwrap();
        let actual = fixture.contains("function _omega_apply_keybindings()")
            && fixture.contains("typeset -ga zvm_after_init_commands")
            && fixture.contains("zvm_after_init_commands+=('_omega_apply_keybindings')");
        let expected = true;
        assert_eq!(actual, expected);
    }
    ...
    fn test_setup_zsh_integration_without_nerd_font_config() {
        ...
        assert!(content.contains("# >>> omega initialize >>>"));
        assert!(content.contains("# <<< omega initialize <<<"));
    }
    ...
    fn test_setup_zsh_integration_with_editor() {
        ...
        // Should contain OMEGA_EDITOR with explanatory comments
        assert!(
            content.contains("export OMEGA_EDITOR=\"code --wait\""),
            "Content should contain OMEGA_EDITOR:\n{}",
            content
        );
        assert!(
        ...
        assert!(
            content.contains("# To change: update OMEGA_EDITOR or remove to use $EDITOR"),
    ...
    fn test_setup_zsh_integration_with_both_configs() {
        ...
        assert!(
            content.contains("export OMEGA_EDITOR=\"vim\""),
            "Content should contain OMEGA_EDITOR:\n{}",
            content
        );
    ...
    fn test_setup_zsh_integration_updates_existing_markers() {
        ...
        assert!(
            !content.contains("export OMEGA_EDITOR"),
            "Should not contain OMEGA_EDITOR after first setup"
        );
        ...
        assert!(
            content.contains("export OMEGA_EDITOR=\"nvim\""),
            "Should contain OMEGA_EDITOR after update:\n{}",
            content
        );
        ...
        assert_eq!(
            content.matches("# >>> omega initialize >>>").count(),
            1,
