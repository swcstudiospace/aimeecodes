...
use clap::Parser;
use omega_api::OmegaAPI;
use omega_config::OmegaConfig;
use omega_domain::TitleFormat;
use omega_main::{Cli, Sandbox, TitleDisplayExt, TopLevelCommand, UI, tracker};
...
///
/// Without VT processing on stdout, ANSI escape codes from omega's markdown
...
async fn run() -> Result<()> {
    ...
    // Check if there's piped input, but skip for `omega select` since that
    ...
    let config =
        OmegaConfig::read().context("Failed to read Omega configuration from .omega.toml")?;
    ...
    let mut ui = UI::init(cli, config, move |config| {
        OmegaAPI::init(cwd.clone(), config)
    })?;
    ui.run().await;
...
#[cfg(test)]
mod tests {
    use omega_main::TopLevelCommand;
    ...
    fn test_stdin_detection_logic() {
        ...
        let cli_with_prompt = Cli::parse_from(["omega", "--prompt", "existing prompt"]);
        ...
        let cli_no_prompt = Cli::parse_from(["omega"]);
    ...
    fn test_cli_parsing_with_short_flag() {
        ...
        let cli_with_short_prompt = Cli::parse_from(["omega", "-p", "short flag prompt"]);
        assert_eq!(
    ...
    fn test_cli_parsing_other_flags_work_with_piping() {
        ...
        let cli_with_flags = Cli::parse_from(["omega", "--verbose"]);
    ...
    fn test_commit_command_diff_field_initially_none() {
        ...
        let cli = Cli::parse_from(["omega", "commit", "--preview"]);
