...
//! For example: `omega provider login` instead of `omega providers login`.
//!
//! NOTE: With every change to this CLI structure, verify that the ZSH plugin
//! remains compatible. The plugin at `shell-plugin/omega.plugin.zsh` implements
//! shell completion and command shortcuts that depend on the CLI structure.

use std::path::PathBuf;

use clap::{Parser, Subcommand, ValueEnum};
use omega_domain::{AgentId, ConversationId, Effort, ModelId, ProviderId};

#[derive(Parser)]
#[command(
    name = "omega",
    version = env!("CARGO_PKG_VERSION"),
    about = "Omega Loops — WEB3-native AI coding agent"
)]
pub struct Cli {
    ...
    ///
    /// This field is automatically populated when content is piped to omega
    ...
    ///
    /// When provided, changes to this directory before starting omega.
    #[arg(long, short = 'C')]
...
pub enum TopLevelCommand {
    ...
    /// Update omega to the latest version.
    Update(UpdateArgs),
    ...
    /// Stream omega log output (defaults to the most recent log file).
    Logs(LogsArgs),
...
/// Command group for the `omega select` interactive picker.
///
...
///
/// Each variant fetches data internally through the Omega API, presents an
...
impl From<Scope> for omega_domain::Scope {
    fn from(value: Scope) -> Self {
        match value {
            Scope::Local => omega_domain::Scope::Local,
            Scope::User => omega_domain::Scope::User,
        }
    }
}
...
pub enum ConfigCommand {
    ...
    /// Migrate the legacy ~/omega directory to ~/.omega.
    Migrate,
}

/// Arguments for `omega config set`.
#[derive(Parser, Debug, Clone)]
...
/// Arguments for `omega config get`.
#[derive(Parser, Debug, Clone)]
...
/// Type-safe subcommands for `omega config set`.
#[derive(Subcommand, Debug, Clone)]
...
/// Type-safe subcommands for `omega config get`.
#[derive(Subcommand, Debug, Clone)]
...
pub struct CommitCommandGroup {
    ...
    /// diff content: `git diff | omega commit --preview`
    #[arg(skip)]
    ...
    /// quotes: `omega commit fix typo in readme`
    pub text: Vec<String>,
}
...
impl From<DataCommandGroup> for omega_domain::DataGenerationParameters {
...
/// Arguments for the `omega logs` command.
#[derive(Parser, Debug, Clone)]
pub struct LogsArgs {
    ...
    /// file in the omega logs directory.
    #[arg(long, short = 'f')]
...
mod tests {
    ...
    fn test_data_command_group_conversion() {
        ...
        let actual: omega_domain::DataGenerationParameters = fixture.into();
        let expected = omega_domain::DataGenerationParameters {
    ...
    fn test_commit_default_max_diff_size() {
        let fixture = Cli::parse_from(["omega", "commit", "--preview"]);
    ...
    fn test_commit_custom_max_diff_size() {
        let fixture = Cli::parse_from(["omega", "commit", "--preview", "--max-diff", "50000"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Commit(commit)) => commit.max_diff_size,
            _ => panic!("Expected Commit command"),
        };
    ...
    #[test]
    fn test_config_list() {
        let fixture = Cli::parse_from(["omega", "config", "list"]);
        let actual = match fixture.subcommands {
    ...
    fn test_config_get_specific_field() {
        let fixture = Cli::parse_from(["omega", "config", "get", "model"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Config(config)) => match config.command {
    ...
    fn test_conversation_list() {
        let fixture = Cli::parse_from(["omega", "conversation", "list"]);
    ...
    fn test_session_alias_list() {
        let fixture = Cli::parse_from(["omega", "session", "list"]);
        let is_list = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => {
                matches!(conversation.command, ConversationCommand::List { .. })
            }
    ...
    fn test_list_tools_command_with_agent() {
        let fixture = Cli::parse_from(["omega", "list", "tool", "sage"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_conversation_command() {
        let fixture = Cli::parse_from(["omega", "list", "conversation"]);
    ...
    fn test_list_session_alias_command() {
        let fixture = Cli::parse_from(["omega", "list", "session"]);
        let is_conversation_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => {
                matches!(list.command, ListCommand::Conversation { .. })
            }
    ...
    fn test_list_command_without_custom_flag() {
        let fixture = Cli::parse_from(["omega", "list", "command"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_command_with_custom_flag() {
        let fixture = Cli::parse_from(["omega", "list", "command", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Command { custom } => custom,
                _ => false,
    ...
    fn test_cmd_list_with_custom_flag() {
        let fixture = Cli::parse_from(["omega", "cmd", "list", "--custom"]);
        let actual = match fixture.subcommands {
    ...
    fn test_command_list_with_custom_flag() {
        let fixture = Cli::parse_from(["omega", "command", "list", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Cmd(cmd_group)) => match cmd_group.command {
                CmdCommand::List { custom } => custom,
                _ => false,
    ...
    fn test_info_command_without_porcelain() {
        let fixture = Cli::parse_from(["omega", "info"]);
        let actual = match fixture.subcommands {
    ...
    fn test_info_command_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "info", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Info { porcelain, .. }) => porcelain,
            _ => false,
        };
    ...
    fn test_list_agents_without_porcelain() {
        let fixture = Cli::parse_from(["omega", "list", "agents"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_agents_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "list", "agents", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_mcp_list_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "mcp", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_conversation_list_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "conversation", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
    ...
    fn test_list_models_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "list", "models", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_config_list_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "config", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_prompt_command() {
        let fixture = Cli::parse_from(["omega", "zsh", "rprompt"]);
    ...
    fn test_cmd_command_with_args() {
        let fixture =
            Cli::parse_from(["omega", "cmd", "execute", "custom-command", "arg1", "arg2"]);
        let actual = match fixture.subcommands {
    ...
    fn test_commit_with_custom_text() {
        let fixture = Cli::parse_from(["omega", "commit", "fix", "typo", "in", "readme"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_skill_command() {
        let fixture = Cli::parse_from(["omega", "list", "skill"]);
    ...
    fn test_list_skills_alias_command() {
        let fixture = Cli::parse_from(["omega", "list", "skills"]);
        let is_skill_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Skill { .. }),
            _ => false,
        };
    ...
    fn test_list_skill_with_porcelain() {
        let fixture = Cli::parse_from(["omega", "list", "skill", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_suggest_with_double_dash_prompt() {
        let fixture = Cli::parse_from(["omega", "suggest", "--date tomorrow"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Suggest { prompt }) => prompt,
            _ => panic!("Expected suggest subcommand"),
        };
    ...
    fn test_suggest_with_single_dash_prompt() {
        let fixture = Cli::parse_from(["omega", "suggest", "-v file.txt"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Suggest { prompt }) => prompt,
            _ => panic!("Expected suggest subcommand"),
        };
    ...
    fn test_terminal_theme_zsh() {
        let fixture = Cli::parse_from(["omega", "zsh", "theme"]);
        let actual = match fixture.subcommands {
    ...
    fn test_terminal_plugin_zsh() {
        let fixture = Cli::parse_from(["omega", "zsh", "plugin"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_doctor() {
        let fixture = Cli::parse_from(["omega", "zsh", "doctor"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_setup() {
        let fixture = Cli::parse_from(["omega", "zsh", "setup"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    fn test_zsh_keyboard() {
        let fixture = Cli::parse_from(["omega", "zsh", "keyboard"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_format() {
        let fixture = Cli::parse_from(["omega", "zsh", "format", "--buffer", "hello world"]);
        let actual = match fixture.subcommands {
    ...
    #[test]
    fn test_setup_alias() {
        let fixture = Cli::parse_from(["omega", "setup"]);
    ...
    fn test_doctor_alias() {
        let fixture = Cli::parse_from(["omega", "doctor"]);
    ...
    fn test_install_vscode_extension() {
        let fixture = Cli::parse_from(["omega", "vscode", "install-extension"]);
        let actual = matches!(
            fixture.subcommands,
    ...
    fn test_list_agent_with_custom_flag() {
        let fixture = Cli::parse_from(["omega", "list", "agent", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_agent_without_custom_flag() {
        let fixture = Cli::parse_from(["omega", "list", "agent"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Agent { custom } => custom,
                _ => true,
    ...
    fn test_list_skill_with_custom_flag() {
        let fixture = Cli::parse_from(["omega", "list", "skill", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_update_with_no_confirm() {
        let fixture = Cli::parse_from(["omega", "update", "--no-confirm"]);
        let actual = match fixture.subcommands {
    ...
    fn test_update_without_no_confirm() {
        let fixture = Cli::parse_from(["omega", "update"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Update(args)) => args.no_confirm,
            _ => panic!("Expected Update command"),
        };
