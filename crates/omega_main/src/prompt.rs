...
use omega_api::{AgentId, Effort, ModelId, Usage};
...
pub struct OmegaPrompt {
    ...
    /// suppressed (see [`OmegaPrompt::render_prompt_right`]).
    pub reasoning_effort: Option<Effort>,
    pub git_branch: Option<String>,
}

impl OmegaPrompt {
    /// Creates a new `OmegaPrompt`, resolving the git branch once at
    /// construction time.
    ...
    pub fn render_prompt_right(&self) -> Cow<'_, str> {
            ...
            && active
        {
            let prefix = match tokens {
                omega_api::TokenCount::Actual(_) => "",
                omega_api::TokenCount::Approx(_) => "~",
            };
...
mod tests {
    ...
    fn test_render_prompt_left() {
        let prompt = OmegaPrompt::default();
    ...
    fn test_render_prompt_left_with_branch() {
        let prompt = OmegaPrompt { git_branch: Some("main".to_string()), ..Default::default() };
        let actual = prompt.render_prompt_left();
    ...
    fn test_render_prompt_right_active_with_tokens() {
        ...
        let usage = Usage {
            prompt_tokens: omega_api::TokenCount::Actual(10),
            completion_tokens: omega_api::TokenCount::Actual(20),
            total_tokens: omega_api::TokenCount::Approx(30),
            ..Default::default()
        };
        let mut prompt = OmegaPrompt::default();
    ...
    fn test_render_prompt_right_strips_provider_prefix() {
        ...
        let usage = Usage {
            prompt_tokens: omega_api::TokenCount::Actual(10),
            completion_tokens: omega_api::TokenCount::Actual(20),
            total_tokens: omega_api::TokenCount::Actual(30),
            ..Default::default()
        };
        let mut prompt = OmegaPrompt::default();
        let _ = prompt.usage(usage);
    ...
    fn test_render_prompt_right_with_cost() {
        ...
        let usage = Usage {
            total_tokens: omega_api::TokenCount::Actual(1500),
            cost: Some(0.01),
            ..Default::default()
        };
        let mut prompt = OmegaPrompt::default();
