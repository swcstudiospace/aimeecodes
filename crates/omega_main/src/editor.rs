...
use crate::highlighter::OmegaHighlighter;
use crate::model::OmegaCommandManager;
use crate::prompt::OmegaPrompt;

const HISTORY_CAPACITY: usize = 1024 * 1024;

/// Interactive terminal editor used by the Omega prompt.
pub struct OmegaEditor {
    editor: Editor<OmegaHelper, DefaultHistory>,
...
impl OmegaEditor {
    ...
    pub fn new(
        ...
        manager: Arc<OmegaCommandManager>,
    ) -> Self {
        let history_file = env.history_path(custom_history_path.as_ref());
        let helper = OmegaHelper::new(env.cwd, manager);
        let config = Config::builder()
            ...
            .enable_signals(true)
            .build();
        let mut editor = Editor::<OmegaHelper, DefaultHistory>::with_config(config)
    ...
    pub fn prompt(&mut self, prompt: &mut OmegaPrompt) -> anyhow::Result<ReadResult> {
...
fn render_prompt(prompt: &OmegaPrompt) -> ResponsivePrompt {
...
struct OmegaHelper {
    ...
    highlighter: OmegaHighlighter,
    hinter: HistoryHinter,
}

impl OmegaHelper {
    fn new(cwd: PathBuf, command_manager: Arc<OmegaCommandManager>) -> Self {
        Self {
...
impl Helper for OmegaHelper {}

impl Completer for OmegaHelper {
    type Candidate = Pair;
...
impl Highlighter for OmegaHelper {
...
impl Validator for OmegaHelper {}
...
mod tests {
    ...
    fn test_render_prompt_raw_has_no_ansi_escapes() {
        ...
        let mut prompt = OmegaPrompt::new(PathBuf::from("project"), AgentId::default());
