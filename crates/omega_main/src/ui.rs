...
use omega_app::utils::{format_display_path, truncate_key};
use omega_app::{CommitResult, ToolResolver};
use omega_config::OmegaConfig;
use omega_display::MarkdownFormat;
use omega_domain::{
    AuthMethod, ChatResponseContent, ConsoleWriter, ContextMessage, Role, TitleFormat, UserCommand,
};
use omega_fs::OmegaFS;
use omega_select::{OmegaWidget, SelectRow};
use omega_spinner::SpinnerManager;
use omega_tracker::ToolCallPayload;
use omega_walker::Walker;
use futures::future;
...
fn format_mcp_server(server: &omega_domain::McpServerConfig) -> String {
    match server {
        omega_domain::McpServerConfig::Stdio(stdio) => {
        ...
        omega_domain::McpServerConfig::Http(http) => http.url.clone(),
    }
}

/// Formats HTTP headers for display, redacting values.
/// Returns None if there are no headers.
fn format_mcp_headers(server: &omega_domain::McpServerConfig) -> Option<String> {
    match server {
        omega_domain::McpServerConfig::Stdio(_) => None,
        omega_domain::McpServerConfig::Http(http) => {
...
pub struct UI<A: ConsoleWriter, F: Fn(OmegaConfig) -> A> {
    ...
    _guard: omega_tracker::Guard,
}

impl<A: API + ConsoleWriter + 'static, F: Fn(OmegaConfig) -> A + Send + Sync> UI<A, F> {
    ...
    fn select_raw_row(
        ...
        OmegaWidget::select_rows(prompt, rows)
            .query(query)
    ...
    async fn on_new(&mut self) -> Result<()> {
        let config = omega_config::OmegaConfig::read().unwrap_or_default();
    ...
    ///   command; receives the latest [`OmegaConfig`] so that config changes
    ///   from `omega config set` are reflected in new conversations
    pub fn init(cli: Cli, config: OmegaConfig, f: F) -> Result<Self> {
        // Parse CLI arguments first to get flags
        let api = Arc::new(f(config.clone()));
        let env = api.environment();
        let command = Arc::new(OmegaCommandManager::default());
        ...
        Ok(Self {
            ...
            config,
            _guard: omega_tracker::init_tracing(env.log_path(), TRACKER.clone())?,
        })
    }
    ...
    async fn prompt(&self) -> Result<AppCommand> {
        ...
        let mut omega_prompt = OmegaPrompt::new(self.state.cwd.clone(), agent_id);
        if let Some(u) = usage {
            omega_prompt.usage(u);
        }
        if let Some(m) = model {
            omega_prompt.model(m);
        }
        if let Some(e) = reasoning_effort {
            omega_prompt.reasoning_effort(e);
        }
        self.console.prompt(&mut omega_prompt).await
    }
    ...
    async fn handle_generate_conversation_id(&mut self) -> Result<()> {
        let conversation_id = omega_domain::ConversationId::generate();
        ...
        match subcommand {
            ...
            TopLevelCommand::Mcp(mcp_command) => match mcp_command.command {
                McpCommand::Import(import_args) => {
                    let scope: omega_domain::Scope = import_args.scope.into();

                    // Parse the incoming MCP configuration
                    let incoming_config: omega_domain::McpConfig = serde_json::from_str(&import_args.json)
                ...
                McpCommand::Remove(rm) => {
                    let name = omega_api::ServerName::from(rm.name);
                    let scope: omega_domain::Scope = rm.scope.into();
                ...
                McpCommand::Show(val) => {
                    let name = omega_api::ServerName::from(val.name);
            ...
            TopLevelCommand::Workspace(index_group) => {
                match index_group.command {
                    ...
                    crate::cli::WorkspaceCommand::Query {
                        ...
                        let mut params =
                            omega_domain::SearchParams::new(&query, &use_case).limit(limit);
                        if let Some(k) = top_k {
            ...
            TopLevelCommand::Update(args) => {
                let update = omega_config::Update::default().auto_update(args.no_confirm);
    ...
    async fn handle_mcp_login(&mut self, name: &str) -> anyhow::Result<()> {
        let server_name = omega_api::ServerName::from(name.to_string());
        let config = self.api.read_mcp_config(None).await?;
        let server = config.mcp_servers.get(&server_name);

        match server {
            Some(omega_domain::McpServerConfig::Http(http)) => {
    ...
    async fn on_show_providers(
        &mut self,
        porcelain: bool,
        types: Vec<omega_domain::ProviderType>,
    ) -> anyhow::Result<()> {
        ...
        for pm in &all_provider_models {
            ...
            for model in &pm.models {
                ...
                let supports_image = model
                    .input_modalities
                    .contains(&omega_domain::InputModality::Image);
                info = info.add_key_value(
                    "Image",
                    if supports_image {
                        status::YES
    ...
    async fn on_show_commands(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        if porcelain {
            ...
            let command_manager = OmegaCommandManager::default();
    ...
    /// Lists only custom commands (used by `omega run`)
    ...
    async fn on_show_config(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        if porcelain {
            ...
            self.writeln("\nCONFIGURATION\n".bold().dimmed())?;
            let highlighted =
                omega_display::SyntaxHighlighter::default().highlight(&config_toml, "toml");
    ...
    async fn on_show_mcp_servers(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        for (name, server) in mcp_servers.mcp_servers {
            let label = match server {
                omega_domain::McpServerConfig::Stdio(_) => "Command",
                omega_domain::McpServerConfig::Http(_) => "URL",
            };
    ...
    /// Install the Omega VS Code extension
    async fn on_vscode_extension_install(&mut self) -> anyhow::Result<()> {
        self.spinner
            .start(Some("Installing Omega VS Code extension"))?;

        match crate::vscode::install_extension() {
            Ok(true) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::info(
                    "Omega VS Code extension installed successfully",
                ))?;
            }
            Ok(false) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::error(
                    "Failed to install Omega VS Code extension.",
                ))?;
            }
            Err(e) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::error(format!(
                    "Failed to install Omega VS Code extension: {e}"
                )))?;
            }
        }
    ...
    async fn on_zsh_setup(&mut self) -> anyhow::Result<()> {
        ...
        let can_see_nerd_fonts =
            OmegaWidget::confirm("Can you see all the icons clearly without any overlap?")
                .with_default(true)
                .prompt()?;
        ...
        let selected_editor = OmegaWidget::select(
            "Which editor would you like to use for editing prompts?",
            editor_options,
        )
        .prompt()?;

        let omega_editor = match selected_editor {
        ...
        let result = crate::zsh::setup_zsh_integration(disable_nerd_font, omega_editor)?;
        self.spinner.stop(None)?;
        ...
        self.writeln_title(TitleFormat::debug("running omega zsh doctor"))?;
        println!();
    ...
    async fn on_command(&mut self, command: AppCommand) -> anyhow::Result<bool> {
        match command {
            ...
            AppCommand::Omega => {
                self.on_agent_change(AgentId::OMEGA).await?;
            }
            AppCommand::Muse => {
            ...
            AppCommand::SuperGrok => {
                self.handle_provider_login(Some(&ProviderId::XAI_OAUTH)).await?;
            }
            AppCommand::Logout => {
    ...
    async fn on_reasoning_effort_selection(&mut self, global: bool) -> anyhow::Result<()> {
        ...
        if let Some(effort_str) = selected {
            let effort = omega_domain::Effort::from_str(&effort_str)
    ...
    async fn on_config_commit_model(&mut self) -> anyhow::Result<()> {
        ...
        if let Some((model, provider_id)) = selection {
            let commit_config = omega_domain::ModelConfig::new(provider_id.clone(), model.clone());
            self.api
    ...
    async fn on_config_suggest_model(&mut self) -> anyhow::Result<()> {
        let selection = self.select_model(None, None).await?;
        if let Some((model, provider_id)) = selection {
            let suggest_config = omega_domain::ModelConfig::new(provider_id.clone(), model.clone());
            self.api
    ...
    async fn on_config_edit(&mut self) -> anyhow::Result<()> {
        let config_path = omega_config::ConfigReader::config_path();
        ...
        let editor = std::env::var("OMEGA_EDITOR")
        ...
        let status = std::process::Command::new(&editor_binary)
            ...
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to open editor '{}': {}. Set OMEGA_EDITOR or EDITOR.",
                    editor_binary,
                    e
                )
            })?;
    ...
    async fn on_edit_buffer(&mut self, initial: Option<String>) -> anyhow::Result<()> {
        ...
        // Create .omega directory for the temp file
        let omega_dir = self.state.cwd.join(".omega");
        std::fs::create_dir_all(&omega_dir)?;
        let temp_file = omega_dir.join("OMEGA_EDITMSG.md");
    ...
    async fn on_suggest(&mut self, description: Option<String>) -> anyhow::Result<()> {
        let description = match description {
            ...
            _ => {
                let input = OmegaWidget::input("Describe the command you want")
                    .allow_empty(false)
                    .prompt()?;
                match input {
                    Some(d) if !d.is_empty() => d,
    ...
    async fn on_slash_conversation_rename(&mut self, args: Option<String>) -> anyhow::Result<()> {
        if let Some(args) = args {
            ...
            if let Some(conv) = selected {
                let name_result = OmegaWidget::input("New name").allow_empty(false).prompt()?;
    ...
    async fn on_copy(&mut self) -> anyhow::Result<()> {
        ...
        let content = context.messages.iter().rev().find_map(|msg| match &**msg {
            omega_domain::ContextMessage::Text(omega_api::TextMessage {
                content,
                role: omega_domain::Role::Assistant,
                ..
            }) => Some(content.clone()),
            _ => None,
        });
    ...
    async fn select_model(
        ...
        // `omega list models --porcelain`), then optionally filter by provider.
        self.spinner.start(Some("Loading"))?;
    ...
    async fn handle_api_key_input(
            ...
            .map(|param| {
                let param_value = if let Some(options) = &param.options {
                    ...
                    let starting = existing_url_params
                        ...
                        .unwrap_or(0);
                    OmegaWidget::select(format!("Select {}", param.name), options.clone())
                    ...
                    let mut input = OmegaWidget::input(label);
        ...
        let api_key_str = if let Some(default_key) = &request.api_key {
            ...
            if key_str == "google_adc_marker" || key_str == "aws_profile_marker" {
                ...
                let input = OmegaWidget::input(format!(
                ...
                let input = OmegaWidget::input(format!("Enter your {provider_id} API key"))
    ...
    async fn handle_code_flow(
        ...
        let code = if let Some(server) = callback_server {
            ...
            let code =
                OmegaWidget::input(format!("Paste the authorization code for {provider_id}"))
                    .prompt()?
    ...
    async fn select_auth_method(
        ...
        match OmegaWidget::select("Select authentication method:", method_names.clone())
    ...
    /// Creates OmegaLoops Services credentials if not already authenticated and
    /// displays the credentials file location to the user.
    async fn init_omega_services(&mut self) -> Result<()> {
        self.api.create_auth_credentials().await?;
        let env = self.api.environment();
        let credentials_path = crate::info::format_path_for_display(&env, &env.credentials_path());
        self.writeln_title(
            TitleFormat::info("OmegaLoops Services enabled").sub_title(&credentials_path),
        )?;
        Ok(())
    }
    ...
    async fn configure_provider(
        ...
        if provider_id == ProviderId::OMEGA_SERVICES {
            self.init_omega_services().await?;
            return Ok(None);
        }
        ...
        if provider_id == ProviderId::CLAUDE_CODE {
            ...
            let confirmed = OmegaWidget::confirm("Do you want to continue with this provider?")
                .with_default(false)
                .prompt()?;
    ...
    /// selected provider.
    ///
    /// The display matches the shell plugin's `_omega_select_provider`:
    ...
    async fn select_provider(
        ...
        let mut providers: Vec<AnyProvider> = self
            ...
            .into_iter()
            .filter(|p| {
                let filter = omega_domain::ProviderType::Llm;
                match &p {
    ...
    async fn on_model_selection(
        ...
        self.api
            .update_config(vec![ConfigOperation::SetSessionConfig(
                omega_domain::ModelConfig::new(provider_id, model.clone()),
            )])
            .await?;
    ...
    async fn finalize_provider_activation(
        ...
        if let Some(model) = model {
            ...
            self.api
                .update_config(vec![ConfigOperation::SetSessionConfig(
                    omega_domain::ModelConfig::new(provider.id.clone(), model_id.clone()),
                )])
                .await?;
            self.writeln_title(
        ...
        if needs_model_selection {
            ...
            self.api
                .update_config(vec![ConfigOperation::SetSessionConfig(
                    omega_domain::ModelConfig::new(provider.id.clone(), model),
                )])
                .await?;
    ...
    async fn init_conversation(&mut self) -> Result<ConversationId> {
        ...
        let id = if let Some(id) = self.state.conversation_id {
            ...
            let content = OmegaFS::read_utf8(path).await?;
    ...
    async fn on_message(&mut self, content: Option<String>) -> Result<()> {
        ...
        // input are provided. This handles the case: `echo "context" | omega -p
        ...
        match message {
            ...
            ChatResponse::TaskComplete => {
                ...
                if let Some(format) = self.config.auto_dump.clone() {
                    let html = matches!(format, omega_config::AutoDumpFormat::Html);
                    self.on_dump(html).await?;
                }
            }
        }
    ...
    async fn should_continue(&mut self) -> anyhow::Result<bool> {
        let should_continue = OmegaWidget::confirm("Do you want to continue anyway?")
            .with_default(true)
            .prompt()?;
    ...
    async fn handle_config_command(
        ...
        match command {
            ...
            crate::cli::ConfigCommand::Path => {
                let path = omega_config::ConfigReader::config_path();
    ...
    fn handle_config_migrate(&mut self) -> Result<()> {
        ...
        let legacy = home.join("omega");
        let new = home.join(".omega");
    ...
    async fn handle_config_set(&mut self, args: crate::cli::ConfigSetArgs) -> Result<()> {
        ...
        match args.field {
            ...
            ConfigSetField::Commit { provider, model } => {
                ...
                let commit_config =
                    omega_domain::ModelConfig::new(provider.clone(), validated_model.clone());
                self.api
                    .update_config(vec![ConfigOperation::SetCommitConfig(Some(commit_config))])
                    .await?;
                self.writeln_title(
    ...
    async fn handle_zsh_rprompt_command(&mut self) -> Option<String> {
        let cid = std::env::var("_OMEGA_CONVERSATION_ID")
            .ok()
            .filter(|text| !text.trim().is_empty())
            .and_then(|str| ConversationId::from_str(str.as_str()).ok());

        let agent_id = std::env::var("_OMEGA_ACTIVE_AGENT")
            .ok()
            .filter(|text| !text.trim().is_empty())
            .map(AgentId::new);
    ...
    async fn validate_model(
        &self,
        model_str: &str,
        provider: Option<&omega_domain::ProviderId>,
    ) -> Result<ModelId> {
    ...
    async fn on_index(&mut self, path: std::path::PathBuf, init: bool) -> anyhow::Result<()> {
        use omega_domain::SyncProgress;
        use omega_spinner::ProgressBarManager;
    ...
    async fn on_query(
        &mut self,
        path: PathBuf,
        params: omega_domain::SearchParams<'_>,
    ) -> anyhow::Result<()> {
        ...
        for result in results.iter() {
            match &result.node {
                omega_domain::NodeData::FileChunk(chunk) => {
                    info = info.add_key_value(
                        "File",
                        format!(
                            "{}:{}-{}",
    ...
    fn format_workspace_info(workspace: &omega_domain::WorkspaceInfo, is_active: bool) -> Info {
    ...
    async fn on_delete_workspaces(&mut self, workspace_ids: Vec<String>) -> anyhow::Result<()> {
        ...
        let parsed_ids: Vec<omega_domain::WorkspaceId> = workspace_ids
            .iter()
            .map(|id| {
                omega_domain::WorkspaceId::from_string(id)
    ...
    async fn on_workspace_init(
        ...
        // with the OmegaLoops Service.
        let display_path = path.display().to_string();

        let confirmed = if yes {
            Some(true)
        } else {
            OmegaWidget::confirm(format!(
                "This will sync and share the contents of '{}' with OmegaLoops Services. Do you wish to continue?",
                display_path
            ))
            .with_default(true)
            .prompt()?
    ...
    async fn handle_migrate_credentials(&mut self) -> Result<()> {
        ...
        if let Some(result) = result {
            self.writeln_title(
                TitleFormat::warning("Omega no longer reads API keys from environment variables.")
                    .sub_title("Learn more: https://omegaloops.dev/docs/custom-providers/"),
            )?;
...
mod tests {
    ...
    // OmegaSelect::confirm is not easily mockable in the current
