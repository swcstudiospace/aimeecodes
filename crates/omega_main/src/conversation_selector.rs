use anyhow::Result;
use chrono::Utc;
use omega_api::Conversation;
use omega_domain::ConversationId;
use omega_select::{OmegaWidget, PreviewLayout, PreviewPlacement, SelectRow};
...
impl ConversationSelector {
    ...
    ///
    /// The preview command uses `omega conversation info` and
    /// `omega conversation show` to display the selected conversation's
    ...
    pub async fn select_conversation(
        ...
        let preview_command =
            "CLICOLOR_FORCE=1 omega conversation info {1}; echo; CLICOLOR_FORCE=1 omega conversation show {1}"
                .to_string();

        let selected_uuid = tokio::task::spawn_blocking(move || -> Result<Option<String>> {
            Ok(OmegaWidget::select_rows("Conversation", rows)
                .query(query)
...
mod tests {
    use chrono::Utc;
    use omega_api::Conversation;
    use omega_domain::{ConversationId, MetaData, Metrics};
