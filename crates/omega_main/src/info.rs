...
use omega_api::{Conversation, Environment, OmegaConfig, Metrics, Role, Usage, UserUsage};
use omega_tracker::VERSION;
use num_format::{Locale, ToFormattedString};

use crate::display_constants::markers;
use crate::model::OmegaCommandManager;
...
impl From<&OmegaConfig> for Info {
    fn from(config: &OmegaConfig) -> Self {
        let mut info = Info::new();
        ...
        info = info
            ...
            .add_key_value("OmegaLoops Service URL", config.services_url.to_string())
...
/// Create an info instance for available commands from a OmegaCommandManager
impl From<&OmegaCommandManager> for Info {
    fn from(command_manager: &OmegaCommandManager) -> Self {
        ...
        info = info
            ...
            .add_key_value("<CTRL+D>", "Quit Omega interactive shell")
    ...
    fn from(user_usage: &UserUsage) -> Self {
        ...
        if plan.is_upgradeable() {
            info = info.add_key_value(
                "Subscription",
                format!(
                    "{} [Upgrade https://app.omegaloops.dev/app/billing]",
...
fn format_user_message(msg: &omega_api::ContextMessage) -> Option<String> {
    let content = msg
        .as_value()
...
mod tests {
    ...
    use omega_api::{Environment, EventValue};
    ...
    fn test_metrics_info_display() {
        use omega_api::Metrics;
        use omega_domain::{FileOperation, ToolKind};
    ...
    fn test_conversation_info_display() {
        ...
        let fixture = Conversation {
            ...
            context: None,
            metrics,
            metadata: omega_domain::MetaData::new(Utc::now()),
        };
    ...
    fn test_conversation_info_display_with_task() {
        use chrono::Utc;
        use omega_api::{Context, ContextMessage, ConversationId, Role};
        ...
        let context = Context::default()
            ...
            .add_message(ContextMessage::Text(
                omega_domain::TextMessage::new(Role::User, "First user message")
            ...
            .add_message(ContextMessage::Text(
                omega_domain::TextMessage::new(Role::User, "Create a new feature")
    ...
    fn test_info_from_command_manager() {
        let command_manager = super::OmegaCommandManager::default();
