...
use omega_anda::{AndaError, AndaResult, EternalMode, EternalReceipt, EternalStore, PathwayCheckpoint};
use serde::Serialize;
use sha2::{Digest, Sha256};
use tokio::fs;

use crate::{IcpError, IcpResult};
...
impl PathwayCapsule {
    const SCHEMA: &'static str = "omega.anda.pathway_capsule.v1";
...
impl LocalReceiptEternalStore {
    ...
    fn paths(&self, checkpoint: &PathwayCheckpoint) -> (PathBuf, PathBuf) {
        ...
        let prefix = format!(
            "{:020}-{}",
            checkpoint.seq,
            checkpoint.content_hash.get(..12).unwrap_or(&checkpoint.content_hash)
        );
        (
...
impl EternalStore for LocalReceiptEternalStore {
    async fn export_checkpoint(
        ...
        let receipt = EternalReceipt::ok(
            EternalMode::Local,
            label,
            checkpoint.content_hash.clone(),
            capsule_path.display().to_string(),
        )
        .detail(format!("capsule_digest={digest}; receipt={}", receipt_path.display()));
...
mod tests {
    ...
    use super::*;
    use omega_anda::{CheckpointKind, PathwayCheckpoint, PathwayId};
    use omega_domain::Conversation;
    ...
    async fn test_local_receipt_writes_capsule_and_receipt() {
        ...
        let actual = store
            .export_checkpoint(&checkpoint, "omega-test-1")
            .await
            .unwrap();
