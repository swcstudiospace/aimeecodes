...
//! For example: `aimee provider login` instead of `aimee providers login`.
//!
//! NOTE: With every change to this CLI structure, verify that the ZSH plugin
//! remains compatible. The plugin at `shell-plugin/aimee.plugin.zsh` implements
//! shell completion and command shortcuts that depend on the CLI structure.

use std::path::PathBuf;

use aimee_domain::{AgentId, ConversationId, Effort, ModelId, ProviderId};
...
#[command(
    ...
    about = "🍑 Aimee Codes 🍑 — WEB3-native AI coding agent"
)]
pub struct Cli {
    ...
    ///
    /// This field is automatically populated when content is piped to aimee
    ...
    ///
    /// When provided, changes to this directory before starting aimee.
    #[arg(long, short = 'C')]
...
pub enum TopLevelCommand {
    ...
    /// Update aimee to the latest version.
    Update(UpdateArgs),
    ...
    /// Stream aimee log output (defaults to the most recent log file).
    Logs(LogsArgs),

    /// Isolated DevPod workspace for untrusted agent PRs.
    #[command(alias = "codespace", alias = "devpod")]
    Pod(PodCommandGroup),
...
/// Command group for `aimee pod` (rebranded DevPod).
///
/// Trailing args after each verb are forwarded to the DevPod CLI. `ui` is
/// Aimee-native and does not spawn DevPod.
#[derive(Parser, Debug, Clone)]
pub struct PodCommandGroup {
    #[command(subcommand)]
    pub command: PodCommand,
}

/// Rebranded DevPod verbs plus Aimee `ui`.
#[derive(Subcommand, Debug, Clone)]
pub enum PodCommand {
    /// Start or create an isolated workspace (`devpod up`).
    Up {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// List workspaces (`devpod list`). `--porcelain` emits JSON.
    List {
        #[arg(long)]
        porcelain: bool,
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
    ...
    /// Stop a workspace (`devpod stop`).
    Stop {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Delete a workspace (`devpod delete`).
    Delete {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// SSH into a workspace (`devpod ssh`). Forwards `-L` / `-R`.
    Ssh {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Run a command inside a workspace (`devpod ssh --command`).
    Exec {
        /// Workspace id (`aimee pod up --id`).
        workspace: String,
        /// Shell command to run in the pod.
        #[arg(trailing_var_arg = true, allow_hyphen_values = true, required = true)]
        command: Vec<String>,
    },
    /// Workspace status (`devpod status`).
    Status {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Workspace logs (`devpod logs`).
    Logs {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Build a workspace image (`devpod build`).
    Build {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Provider add/list/use (`devpod provider …`).
    Provider {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// IDE list/use (`devpod ide …`).
    Ide {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Context commands (`devpod context …`).
    Context {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Machine commands (`devpod machine …`).
    Machine {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// DevPod Pro (`devpod pro …`).
    Pro {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Select default resources (`devpod use …`).
    Use {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Upgrade the DevPod CLI (`devpod upgrade`).
    Upgrade {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Print the DevPod version (`devpod version`).
    Version {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// How to watch pods from a Mac Mini (no DevPod spawn).
    Ui,
    /// Check DevPod, Docker, gh, and SSH reachability for the Mac Mini.
    Doctor,
    /// Any other DevPod verb, forwarded unchanged.
    #[command(external_subcommand)]
    External(Vec<String>),
}

/// Command group for the `aimee select` interactive picker.
///
...
///
/// Each variant fetches data internally through the Aimee API, presents an
...
impl From<Scope> for aimee_domain::Scope {
    fn from(value: Scope) -> Self {
        match value {
            Scope::Local => aimee_domain::Scope::Local,
            Scope::User => aimee_domain::Scope::User,
        }
    }
}
...
pub enum ConfigCommand {
    ...
    /// Migrate a legacy config directory (`~/aimee`, `~/.omega`, `~/omega`) to `~/.aimee`.
    Migrate,
}

/// Arguments for `aimee config set`.
#[derive(Parser, Debug, Clone)]
...
/// Arguments for `aimee config get`.
#[derive(Parser, Debug, Clone)]
...
/// Type-safe subcommands for `aimee config set`.
#[derive(Subcommand, Debug, Clone)]
...
/// Type-safe subcommands for `aimee config get`.
#[derive(Subcommand, Debug, Clone)]
...
pub enum ConversationCommand {
    ...
    /// Inspect or roll back the Anda session pathway (chat-only).
    Pathway {
        /// Conversation ID whose pathway to inspect.
        id: ConversationId,
...
/// Session pathway operations for eternal chat rollback.
#[derive(Subcommand, Debug, Clone)]
pub enum PathwayCommand {
    /// List hash-chained checkpoints for a conversation.
    List {
        /// Output in machine-readable format.
        #[arg(long)]
        porcelain: bool,
    ...
    /// Show a single pathway checkpoint.
    Show {
        /// Checkpoint sequence number (1-based).
        seq: u64,
    },

    /// Restore conversation chat state to a checkpoint sequence.
    ///
    /// Workspace files are not reverted; only conversation context is restored.
    Rollback {
        /// Checkpoint sequence number to restore.
        seq: u64,
    },
}
...
pub struct CommitCommandGroup {
    ...
    /// diff content: `git diff | aimee commit --preview`
    #[arg(skip)]
    ...
    /// quotes: `aimee commit fix typo in readme`
    pub text: Vec<String>,
}
...
impl From<DataCommandGroup> for aimee_domain::DataGenerationParameters {
...
/// Arguments for the `aimee logs` command.
#[derive(Parser, Debug, Clone)]
pub struct LogsArgs {
    ...
    /// file in the aimee logs directory.
    #[arg(long, short = 'f')]
...
mod tests {
    ...
    fn test_data_command_group_conversion() {
        ...
        let actual: aimee_domain::DataGenerationParameters = fixture.into();
        let expected = aimee_domain::DataGenerationParameters {
    ...
    fn test_commit_default_max_diff_size() {
        let fixture = Cli::parse_from(["aimee", "commit", "--preview"]);
    ...
    fn test_commit_custom_max_diff_size() {
        let fixture = Cli::parse_from(["aimee", "commit", "--preview", "--max-diff", "50000"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Commit(commit)) => commit.max_diff_size,
            _ => panic!("Expected Commit command"),
        };
    ...
    #[test]
    fn test_config_list() {
        let fixture = Cli::parse_from(["aimee", "config", "list"]);
        let actual = match fixture.subcommands {
    ...
    fn test_config_get_specific_field() {
        let fixture = Cli::parse_from(["aimee", "config", "get", "model"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Config(config)) => match config.command {
    ...
    fn test_conversation_list() {
        let fixture = Cli::parse_from(["aimee", "conversation", "list"]);
    ...
    fn test_session_alias_list() {
        let fixture = Cli::parse_from(["aimee", "session", "list"]);
        let is_list = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => {
                matches!(conversation.command, ConversationCommand::List { .. })
            }
    ...
    fn test_list_tools_command_with_agent() {
        let fixture = Cli::parse_from(["aimee", "list", "tool", "sage"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_conversation_command() {
        let fixture = Cli::parse_from(["aimee", "list", "conversation"]);
    ...
    fn test_list_session_alias_command() {
        let fixture = Cli::parse_from(["aimee", "list", "session"]);
        let is_conversation_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => {
                matches!(list.command, ListCommand::Conversation { .. })
            }
    ...
    fn test_list_command_without_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "list", "command"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_command_with_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "list", "command", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Command { custom } => custom,
                _ => false,
    ...
    fn test_cmd_list_with_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "cmd", "list", "--custom"]);
        let actual = match fixture.subcommands {
    ...
    fn test_command_list_with_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "command", "list", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Cmd(cmd_group)) => match cmd_group.command {
                CmdCommand::List { custom } => custom,
                _ => false,
    ...
    fn test_info_command_without_porcelain() {
        let fixture = Cli::parse_from(["aimee", "info"]);
        let actual = match fixture.subcommands {
    ...
    fn test_info_command_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "info", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Info { porcelain, .. }) => porcelain,
            _ => false,
        };
    ...
    fn test_list_agents_without_porcelain() {
        let fixture = Cli::parse_from(["aimee", "list", "agents"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_agents_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "list", "agents", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_mcp_list_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "mcp", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_conversation_list_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "conversation", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
    ...
    fn test_list_models_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "list", "models", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_config_list_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "config", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_prompt_command() {
        let fixture = Cli::parse_from(["aimee", "zsh", "rprompt"]);
    ...
    fn test_cmd_command_with_args() {
        let fixture =
            Cli::parse_from(["aimee", "cmd", "execute", "custom-command", "arg1", "arg2"]);
        let actual = match fixture.subcommands {
    ...
    fn test_commit_with_custom_text() {
        let fixture = Cli::parse_from(["aimee", "commit", "fix", "typo", "in", "readme"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_skill_command() {
        let fixture = Cli::parse_from(["aimee", "list", "skill"]);
    ...
    fn test_list_skills_alias_command() {
        let fixture = Cli::parse_from(["aimee", "list", "skills"]);
        let is_skill_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Skill { .. }),
            _ => false,
        };
    ...
    fn test_list_skill_with_porcelain() {
        let fixture = Cli::parse_from(["aimee", "list", "skill", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_suggest_with_double_dash_prompt() {
        let fixture = Cli::parse_from(["aimee", "suggest", "--date tomorrow"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Suggest { prompt }) => prompt,
            _ => panic!("Expected suggest subcommand"),
        };
    ...
    fn test_suggest_with_single_dash_prompt() {
        let fixture = Cli::parse_from(["aimee", "suggest", "-v file.txt"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Suggest { prompt }) => prompt,
            _ => panic!("Expected suggest subcommand"),
        };
    ...
    fn test_terminal_theme_zsh() {
        let fixture = Cli::parse_from(["aimee", "zsh", "theme"]);
        let actual = match fixture.subcommands {
    ...
    fn test_terminal_plugin_zsh() {
        let fixture = Cli::parse_from(["aimee", "zsh", "plugin"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_doctor() {
        let fixture = Cli::parse_from(["aimee", "zsh", "doctor"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_setup() {
        let fixture = Cli::parse_from(["aimee", "zsh", "setup"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    fn test_zsh_keyboard() {
        let fixture = Cli::parse_from(["aimee", "zsh", "keyboard"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_format() {
        let fixture = Cli::parse_from(["aimee", "zsh", "format", "--buffer", "hello world"]);
        let actual = match fixture.subcommands {
    ...
    #[test]
    fn test_setup_alias() {
        let fixture = Cli::parse_from(["aimee", "setup"]);
    ...
    fn test_doctor_alias() {
        let fixture = Cli::parse_from(["aimee", "doctor"]);
    ...
    fn test_install_vscode_extension() {
        let fixture = Cli::parse_from(["aimee", "vscode", "install-extension"]);
        let actual = matches!(
            fixture.subcommands,
    ...
    fn test_list_agent_with_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "list", "agent", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_agent_without_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "list", "agent"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Agent { custom } => custom,
                _ => true,
    ...
    fn test_list_skill_with_custom_flag() {
        let fixture = Cli::parse_from(["aimee", "list", "skill", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_update_with_no_confirm() {
        let fixture = Cli::parse_from(["aimee", "update", "--no-confirm"]);
        let actual = match fixture.subcommands {
    ...
    fn test_update_without_no_confirm() {
        let fixture = Cli::parse_from(["aimee", "update"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Update(args)) => args.no_confirm,
            _ => panic!("Expected Update command"),
        };
    ...
    #[test]
    fn test_conversation_pathway_list() {
        let fixture = Cli::parse_from([
            "aimee",
            "conversation",
            "pathway",
        ...
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
                ConversationCommand::Pathway {
                    command: PathwayCommand::List { porcelain },
                    ..
                } => !porcelain,
                _ => false,
            },
    ...
    #[test]
    fn test_conversation_pathway_rollback() {
        let fixture = Cli::parse_from([
            "aimee",
            "conversation",
            "pathway",
        ...
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
                ConversationCommand::Pathway {
                    command: PathwayCommand::Rollback { seq }, ..
                } => seq,
                _ => 0,
            },
            _ => 0,
    ...
    #[test]
    fn test_pod_up_trailing_args() {
        let fixture = Cli::parse_from(["aimee", "pod", "up", ".", "--id", "agent-pr"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Pod(group)) => match group.command {
                PodCommand::Up { args } => args,
                _ => vec!["wrong".into()],
            },
            _ => vec!["missing".into()],
        };
        let expected = vec![".".to_string(), "--id".into(), "agent-pr".into()];
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_pod_alias_codespace() {
        let fixture = Cli::parse_from(["aimee", "codespace", "ui"]);
        let actual = matches!(
            fixture.subcommands,
            Some(TopLevelCommand::Pod(PodCommandGroup {
                command: PodCommand::Ui
            }))
        );
        let expected = true;
        assert_eq!(actual, expected);
    ...
    #[test]
    fn test_cli_pod_flag_is_workspace_id() {
        let fixture = Cli::parse_from(["aimee", "--pod", "omega-ship", "-p", "hi"]);
        let actual = fixture.pod;
        let expected = Some("omega-ship".to_string());
        assert_eq!(actual, expected);
    }
}
