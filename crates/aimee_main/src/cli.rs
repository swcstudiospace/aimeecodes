...
pub struct Cli {
    ...
    /// Provision an isolated container workspace (docker/ssh/cloud) before the
    /// session. Unlike `--sandbox`, this starts a container, not a git worktree.
    #[arg(long)]
...
pub enum TopLevelCommand {
    ...
    /// Isolated container workspace for untrusted agent PRs.
...
/// Command group for `aimee pod`.
///
/// Trailing args after each verb are forwarded to the pod runtime. `ui` is
/// Aimee-native and does not spawn the runtime.
...
/// Isolated workspace verbs plus Aimee `ui`.
#[derive(Subcommand, Debug, Clone)]
pub enum PodCommand {
    /// Start or create an isolated workspace.
    Up {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// List workspaces. `--porcelain` emits JSON.
    List {
        #[arg(long)]
        porcelain: bool,
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
    ...
    /// SSH into a workspace. Forwards `-L` / `-R`.
    Ssh {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    ...
    /// Upgrade the pod runtime.
    Upgrade {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// Print the pod runtime version.
    Version {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// How to watch pods from a Mac Mini (no runtime spawn).
    Ui,
    /// Check pod runtime, Docker, gh, and SSH reachability for the Mac Mini.
    Doctor,
    /// Any other pod verb, forwarded unchanged.
...
mod tests {
    ...
    #[test]
    fn test_pod_help_is_aimee_branded() {
        let root = Cli::command();
        let mut help = Vec::new();
        root.clone().write_long_help(&mut help).unwrap();
        root.find_subcommand("pod")
            .expect("pod")
            .clone()
            .write_long_help(&mut help)
            .unwrap();
        let fixture = String::from_utf8(help).unwrap();
        let actual = fixture.to_ascii_lowercase().contains("devpod");
        let expected = false;
        assert_eq!(actual, expected, "help still mentions DevPod:\n{fixture}");
    }
}
