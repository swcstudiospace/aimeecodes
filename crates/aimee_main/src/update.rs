...
use aimee_api::API;
use aimee_config::{Update, UpdateFrequency};
use aimee_select::AimeeWidget;
use aimee_tracker::VERSION;
use colored::Colorize;
use update_informer::{Check, Version, registry};

/// Runs the official installation script to update Aimee, failing silently.
...
async fn execute_update_command(api: Arc<impl API>, auto_update: bool) {
    ...
    let output = api
        .execute_shell_command_raw("curl -fsSL https://aimeecodes.dev/cli | sh")
        .await;
        ...
        Ok(output) => {
            if output.success() {
                let should_exit = if auto_update {
                    ...
                    let answer = aimee_select::AimeeWidget::confirm(
                        "You need to close aimee to complete update. Do you want to close it now?",
                    )
                    .with_default(true)
                    .prompt();
...
async fn confirm_update(version: Version) -> bool {
    let answer = AimeeWidget::confirm(format!(
...
pub async fn on_update(api: Arc<impl API>, update: Option<&Update>) {
    ...
    let informer = update_informer::new(registry::GitHub, "aimeecodes/aimeecodes", VERSION)
