...
use aimee_app::utils::{format_display_path, truncate_key};
use aimee_app::{CommitResult, ToolResolver};
use aimee_config::AimeeConfig;
use aimee_display::MarkdownFormat;
use aimee_domain::{
    AgentWorkflow, AuthMethod, ChannelEndpoint, ChannelKind, ChannelStore, ChatResponseContent,
    ConsoleWriter, ContextMessage, GitHubPrPlan, GoalProbeSet, LinearEnsureRequest,
    LinearGraphqlIssue, PromptUpgrade, Role, SkillDraft, SoulDocument, TelegramGetUpdatesRequest,
    TelegramSendRequest, ThoughtGraphRequest, TitleFormat, UserCommand, WorkflowRun,
};
use aimee_fs::AimeeFS;
use aimee_select::{AimeeWidget, SelectRow};
use aimee_spinner::SpinnerManager;
use aimee_tracker::ToolCallPayload;
...
use crate::cli::{
    Cli, CommitCommandGroup, ConversationCommand, ListCommand, McpCommand, PathwayCommand,
...
fn format_mcp_server(server: &aimee_domain::McpServerConfig) -> String {
    match server {
        aimee_domain::McpServerConfig::Stdio(stdio) => {
        ...
        aimee_domain::McpServerConfig::Http(http) => http.url.clone(),
    }
}

/// Formats HTTP headers for display, redacting values.
/// Returns None if there are no headers.
fn format_mcp_headers(server: &aimee_domain::McpServerConfig) -> Option<String> {
    match server {
        aimee_domain::McpServerConfig::Stdio(_) => None,
        aimee_domain::McpServerConfig::Http(http) => {
...
pub struct UI<A: ConsoleWriter, F: Fn(AimeeConfig) -> A> {
    ...
    _guard: aimee_tracker::Guard,
}

impl<A: API + ConsoleWriter + 'static, F: Fn(AimeeConfig) -> A + Send + Sync> UI<A, F> {
    ...
    fn select_raw_row(
        ...
        AimeeWidget::select_rows(prompt, rows)
            .query(query)
    ...
    async fn on_new(&mut self) -> Result<()> {
        let config = aimee_config::AimeeConfig::read().unwrap_or_default();
    ...
    async fn on_agent_change(&mut self, agent_id: AgentId) -> Result<()> {
        ...
        let title = agent.title.as_deref().unwrap_or(MISSING_AGENT_TITLE);
        ...
        self.writeln_title(TitleFormat::agent(name).sub_title(format!("{title} · {model_info}")))?;
    ...
    ///   command; receives the latest [`AimeeConfig`] so that config changes
    ///   from `aimee config set` are reflected in new conversations
    pub fn init(cli: Cli, config: AimeeConfig, f: F) -> Result<Self> {
        // Parse CLI arguments first to get flags
        let api = Arc::new(f(config.clone()));
        let env = api.environment();
        let command = Arc::new(AimeeCommandManager::default());
        ...
        Ok(Self {
            ...
            config,
            _guard: aimee_tracker::init_tracing(env.log_path(), TRACKER.clone())?,
        })
    }
    ...
    async fn prompt(&self) -> Result<AppCommand> {
        ...
        self.console.prompt(&mut aimee_prompt).await
    }
        ...
        match subcommand {
            ...
            TopLevelCommand::Mcp(mcp_command) => match mcp_command.command {
                McpCommand::Import(import_args) => {
                    let scope: aimee_domain::Scope = import_args.scope.into();

                    // Parse the incoming MCP configuration
                    let incoming_config: aimee_domain::McpConfig = serde_json::from_str(&import_args.json)
                ...
                McpCommand::Remove(rm) => {
                    let name = aimee_api::ServerName::from(rm.name);
                    let scope: aimee_domain::Scope = rm.scope.into();
                ...
                McpCommand::Show(val) => {
                    let name = aimee_api::ServerName::from(val.name);
            ...
            TopLevelCommand::Update(args) => {
                let update = aimee_config::Update::default().auto_update(args.no_confirm);
    ...
    async fn handle_conversation_command(
        ...
        match conversation_group.command {
            ...
            ConversationCommand::Pathway { id, command } => {
                self.handle_pathway_command(id, command).await?;
            }
        }
        Ok(())
    }

    async fn handle_pathway_command(
        &mut self,
        id: ConversationId,
        ...
        self.validate_conversation_exists(&id).await?;
        let config = aimee_config::AimeeConfig::read().unwrap_or_default();
        let home = aimee_config::ConfigReader::base_path();
        let anda = config.anda.as_ref();

        match command {
            PathwayCommand::List { porcelain } => {
                let checkpoints = aimee_app::list_session_pathway(anda, home, &id).await?;
                if porcelain {
                    ...
                    self.writeln_title(TitleFormat::info(format!(
                        "No session pathway checkpoints for {id}"
                    )))?;
                } else {
                    self.writeln_title(TitleFormat::info(format!(
                        "Session pathway for {id} ({} checkpoints)",
                        checkpoints.len()
                    )))?;
                    for checkpoint in checkpoints {
                        let hash = checkpoint
            ...
            PathwayCommand::Show { seq } => {
                let checkpoint = aimee_app::show_session_pathway(anda, home, &id, seq)
                    .await?
                    .ok_or_else(|| {
                        anyhow::anyhow!("No pathway checkpoint seq {seq} for conversation {id}")
                    })?;
                self.writeln_title(TitleFormat::info(format!(
                    "Pathway checkpoint #{seq} for {id}"
                )))?;
                self.writeln(format!("kind:           {}", checkpoint.kind))?;
                self.writeln(format!("messages:       {}", checkpoint.message_count))?;
                self.writeln(format!("content_hash:   {}", checkpoint.content_hash))?;
                self.writeln(format!("parent_hash:    {}", checkpoint.parent_hash))?;
                self.writeln(format!(
                    "created_at:     {}",
                    checkpoint.created_at.to_rfc3339()
                ))?;
                if let Some(agent_id) = checkpoint.agent_id {
                    self.writeln(format!("agent:          {agent_id}"))?;
                }
                if let Some(ok) = checkpoint.kip_ok {
                    self.writeln(format!("kip_receipt:    {ok}"))?;
                }
                if let Some(ok) = checkpoint.eternal_ok {
                    self.writeln(format!("eternal_receipt:{ok}"))?;
                }
            }
            PathwayCommand::Rollback { seq } => {
                let restored = aimee_app::rollback_session_pathway(anda, home, id, seq).await?;
                self.api.upsert_conversation(restored.clone()).await?;
                self.writeln_title(TitleFormat::info(format!(
                    "Rolled conversation {id} back to pathway seq {seq} ({} messages)",
                    restored.len()
                )))?;
            }
        }
    ...
    async fn handle_mcp_login(&mut self, name: &str) -> anyhow::Result<()> {
        let server_name = aimee_api::ServerName::from(name.to_string());
        let config = self.api.read_mcp_config(None).await?;
        let server = config.mcp_servers.get(&server_name);

        match server {
            Some(aimee_domain::McpServerConfig::Http(http)) => {
    ...
    async fn on_show_providers(
        &mut self,
        porcelain: bool,
        types: Vec<aimee_domain::ProviderType>,
    ) -> anyhow::Result<()> {
        ...
        for pm in &all_provider_models {
            ...
            for model in &pm.models {
                ...
                let supports_image = model
                    .input_modalities
                    .contains(&aimee_domain::InputModality::Image);
                info = info.add_key_value(
                    "Image",
                    if supports_image {
                        status::YES
    ...
    async fn on_show_commands(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        if porcelain {
            ...
            let command_manager = AimeeCommandManager::default();
    ...
    /// Lists only custom commands (used by `aimee run`)
    ...
    async fn on_show_config(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        if porcelain {
            ...
            self.writeln("\nCONFIGURATION\n".bold().dimmed())?;
            let highlighted =
                aimee_display::SyntaxHighlighter::default().highlight(&config_toml, "toml");
    ...
    async fn on_show_mcp_servers(&mut self, porcelain: bool) -> anyhow::Result<()> {
        ...
        for (name, server) in mcp_servers.mcp_servers {
            let label = match server {
                aimee_domain::McpServerConfig::Stdio(_) => "Command",
                aimee_domain::McpServerConfig::Http(_) => "URL",
            };
    ...
    /// Install the Aimee VS Code extension
    async fn on_vscode_extension_install(&mut self) -> anyhow::Result<()> {
        self.spinner
            .start(Some("Installing Aimee VS Code extension"))?;

        match crate::vscode::install_extension() {
            Ok(true) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::info(
                    "Aimee VS Code extension installed successfully",
                ))?;
            }
            Ok(false) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::error(
                    "Failed to install Aimee VS Code extension.",
                ))?;
            }
            Err(e) => {
                self.spinner.stop(None)?;
                self.writeln_title(TitleFormat::error(format!(
                    "Failed to install Aimee VS Code extension: {e}"
                )))?;
            }
        }
    ...
    async fn on_zsh_setup(&mut self) -> anyhow::Result<()> {
        ...
        let can_see_nerd_fonts =
            AimeeWidget::confirm("Can you see all the icons clearly without any overlap?")
                .with_default(true)
                .prompt()?;
        ...
        let selected_editor = AimeeWidget::select(
            "Which editor would you like to use for editing prompts?",
            editor_options,
        )
        .prompt()?;

        let aimee_editor = match selected_editor {
        ...
        let result = crate::zsh::setup_zsh_integration(disable_nerd_font, aimee_editor)?;
        self.spinner.stop(None)?;
        ...
        self.writeln_title(TitleFormat::debug("running aimee zsh doctor"))?;
        println!();
    ...
    async fn on_command(&mut self, command: AppCommand) -> anyhow::Result<bool> {
        match command {
            ...
            AppCommand::Message(ref content) => {
                self.spinner.start(None)?;
                let content = match self.state.goal.continuation_prompt() {
                    Some(preamble) if !aimee_domain::GoalState::is_continuation(content) => {
                        format!("{preamble}\n\n{content}")
                    }
                    _ => content.clone(),
                };
                let upgraded = PromptUpgrade::wrap(&content, self.state.goal.current()).xml;
                Box::pin(self.log_prompt_telemetry(&upgraded)).await?;
                self.on_message(Some(upgraded)).await?;
            }
            AppCommand::Aimee => {
                self.on_agent_change(AgentId::AIMEE).await?;
            }
            AppCommand::Muse => {
            ...
            AppCommand::Custom(event) => {
                if event.name == "swarm" && !event.parameters.is_empty() {
                    self.on_goal(event.parameters.clone()).await?;
                }
                self.spinner.start(None)?;
            ...
            AppCommand::SuperGrok => {
                self.handle_provider_login(Some(&ProviderId::XAI_OAUTH))
                    .await?;
            }
            AppCommand::Logout => {
            ...
            AppCommand::Subgoal { args } => {
                self.on_subgoal(args).await?;
            }
            AppCommand::Soul => {
                self.on_soul().await?;
            }
            AppCommand::Team { args } => {
                self.on_team(args).await?;
            }
            AppCommand::Learn { args } => {
                self.on_learn(args).await?;
            }
            AppCommand::Channel { args } => {
                self.on_channel(args).await?;
            }
        }
    ...
    async fn on_goal(&mut self, args: Vec<String>) -> anyhow::Result<()> {
        ...
        let token = joined.trim();
        let head = args.first().map(|s| s.as_str()).unwrap_or("");
        match head {
            "" | "status" => {
                let snapshot = self.state.goal.current().cloned();
                match snapshot {
                    Some(goal) => {
                        self.writeln_title(TitleFormat::info(format!(
                            "/goal [{}] {}",
                        ...
                        if let Some(pod_id) = goal.pod_id.as_deref() {
                            self.writeln(format!("  pod: {pod_id}"))?;
                        }
                        if let Some(pr_url) = goal.pr_url.as_deref() {
                            self.writeln(format!("  pr: {pr_url}"))?;
                        }
                    }
                    None => {
                        self.writeln_title(TitleFormat::info(
                            "No standing /goal. Set one with /goal <text>.",
                        ))?;
                    }
                }
            }
            "pause" => {
                let label = self.state.goal.pause()?.goal.clone();
                self.writeln_title(TitleFormat::info(format!("Paused /goal: {label}")))?;
            }
            "resume" => {
                let label = self.state.goal.resume()?.goal.clone();
                self.writeln_title(TitleFormat::info(format!("Resumed /goal: {label}")))?;
            }
            "clear" | "done" => {
                self.state.goal.clear()?;
                self.writeln_title(TitleFormat::info("Cleared standing /goal"))?;
            }
            "exec" => {
                let command: Vec<String> = args.iter().skip(1).cloned().collect();
                if command.is_empty() {
                    anyhow::bail!("Usage: /goal exec <command>  (runs inside the goal sandbox)");
                }
                let id = self
                    .state
                    .goal
                    .current()
                    .and_then(|g| g.pod_id.clone())
                    .ok_or_else(|| {
                        anyhow::anyhow!("No sandbox id. Set /goal <text> then /goal pod.")
                    })?;
                self.writeln_title(TitleFormat::info(format!(
                    "Exec in sandbox {id}: {}",
                    command.join(" ")
                )))?;
                crate::pod::exec_in_workspace(&id, &command)?;
            }
            "pod" => {
                let cwd = self.state.cwd.clone();
                let source = cwd.to_string_lossy().into_owned();
                let id = match self.state.goal.current().and_then(|g| g.pod_id.clone()) {
                    Some(id) => id,
                    None => {
                        let goal = self
                            .state
                            ...
                            .map(|g| g.goal.clone())
                            .ok_or_else(|| {
                                anyhow::anyhow!("No active goal. Set one with /goal <text>.")
                            })?;
                        let id = crate::pod::workspace_id_for_goal(&goal);
                        self.state.goal.attach_pod(id.clone())?;
                        id
                    }
                };
                self.writeln_title(TitleFormat::info(format!(
                    "Provisioning sandbox pod {id} (headless, no IDE)"
                )))?;
                crate::pod::provision_for_goal(&id, &source)?;
            }
            "pr" => {
                if self.state.goal.current().is_none() {
                    anyhow::bail!("No active goal. Set one with /goal <text>.");
                }
                let url = crate::pod::open_pull_request()?;
                self.state.goal.attach_pr(url.clone())?;
                self.writeln_title(TitleFormat::info(format!("Opened pull request {url}")))?;
            }
            _ => {
                let (headline, parsed) = GoalProbeSet::parse_from_text(token);
                let probes = match parsed {
                    Ok(set) => set,
                    Err(_) => self.collect_goal_probes()?,
                };
                ...
                let label = self.state.goal.set_loop(&goal_text, probes)?.goal.clone();
                let pod_id = crate::pod::workspace_id_for_goal(&label);
                self.state.goal.attach_pod(pod_id.clone())?;
                self.writeln_title(TitleFormat::info(format!(
                    "Standing /goal set (loop on, 5 probes). {label}"
                )))?;
                self.writeln(format!(
                    "Sandbox id {pod_id}. Start it with /goal pod. Open a PR with /goal pr. CoT×GoT: {}",
                    ThoughtGraphRequest::drop_mcp_url()
                ))?;
                let xml = PromptUpgrade::wrap(&label, self.state.goal.current()).xml;
                Box::pin(self.log_prompt_telemetry(&xml)).await?;
            }
        }
        Ok(())
    }

    fn collect_goal_probes(&mut self) -> anyhow::Result<GoalProbeSet> {
        self.writeln_title(TitleFormat::info(
            "Five probing questions are mandatory before this /goal can loop.",
        ))?;
        let mut answers = Vec::new();
        for (i, question) in aimee_domain::canonical_probe_questions().iter().enumerate() {
            let label = format!("Probe {}/5 — {question}", i + 1);
            let answer = AimeeWidget::input(label)
                .prompt()?
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "All 5 probing questions are required. Re-run /goal with five `probe: …` lines if stdin is not a TTY."
                    )
                })?;
            answers.push(answer);
        }
        GoalProbeSet::try_from_answers(answers).map_err(|e| anyhow::anyhow!("{e}"))
    }

    async fn log_prompt_telemetry(&mut self, prompt: &str) -> anyhow::Result<()> {
        let plan = GitHubPrPlan::from_prompt(prompt);
        let body = LinearEnsureRequest::from_prompt(
            format!("aimee:{}", plan.branch),
            plan.title.clone(),
            prompt,
        );
        ...
        let mut filed = false;
        match client
            .post(LinearEnsureRequest::connector_url())
            .json(&body)
            .send()
            .await
        {
            Ok(response) if response.status().is_success() => {
                ...
                self.writeln_title(TitleFormat::info(format!(
                    "Linear logged {ident} via connector :8792"
                )))?;
                filed = true;
            }
            Ok(response) => {
                self.writeln_title(TitleFormat::info(format!(
                    "Linear connector {} — falling back to GraphQL API",
                    response.status()
                )))?;
            }
            Err(_) => {
                self.writeln_title(TitleFormat::info(
                    "Linear connector unreachable — falling back to GraphQL API",
                ))?;
            }
        }
        if !filed {
            self.file_linear_graphql(&plan.title, prompt).await?;
        }
        let cwd = self.state.cwd.clone();
        if cwd.join(".git").exists() {
            let commit = tokio::process::Command::new("git")
                .args(["commit", "--allow-empty", "-m", &plan.commit_message])
                .current_dir(&cwd)
                .output()
                .await;
            match commit {
                Ok(out) if out.status.success() => {
                    ...
                    match crate::pod::open_pull_request() {
                        Ok(url) => {
                            let _ = self.state.goal.attach_pr(url.clone());
                            self.writeln_title(TitleFormat::info(format!(
                                "Opened pull request {url}"
                            )))?;
                        }
                        Err(err) => {
                            self.writeln_title(TitleFormat::info(format!("PR not opened: {err}")))?;
                        }
                    }
                }
                _ => {
                    self.writeln_title(TitleFormat::info(
                        "Git commit skipped (nothing to commit or git failed)",
                    ))?;
                }
            }
        }
    ...
    async fn file_linear_graphql(&mut self, title: &str, prompt: &str) -> anyhow::Result<()> {
        let Some(token) = std::env::var("LINEAR_API_KEY")
            .ok()
            .filter(|t| !t.trim().is_empty())
        else {
            self.writeln_title(TitleFormat::info(
                "LINEAR_API_KEY unset — Linear GraphQL fallback skipped",
            ))?;
            return Ok(());
        };
        let team = std::env::var("LINEAR_TEAM_ID")
            .ok()
            .filter(|t| !t.trim().is_empty())
            .unwrap_or_else(|| LinearGraphqlIssue::default_team_id().to_string());
        let body = LinearGraphqlIssue::issue_create(&team, title, prompt);
        let response = reqwest::Client::new()
            .post(LinearGraphqlIssue::url())
            .bearer_auth(token)
            .json(&body)
            .send()
            .await;
        match response {
            Ok(resp) if resp.status().is_success() => {
                let ident = resp
                    ...
                    .unwrap_or_else(|| "ok".into());
                self.writeln_title(TitleFormat::info(format!(
                    "Linear logged {ident} via GraphQL API"
                )))?;
            }
            Ok(resp) => {
                self.writeln_title(TitleFormat::info(format!(
                    "Linear GraphQL {} — issue not filed",
                    resp.status()
                )))?;
            }
            Err(_) => {
                self.writeln_title(TitleFormat::info("Linear GraphQL unreachable"))?;
            }
        }
        Ok(())
    }

    async fn on_subgoal(&mut self, args: Vec<String>) -> anyhow::Result<()> {
        let text = args.join(" ");
        if text.trim().is_empty() {
            anyhow::bail!("Usage: /subgoal <criterion>");
        }
        let count = self.state.goal.add_subgoal(text.trim())?.subgoals.len();
        self.writeln_title(TitleFormat::info(format!("Added /subgoal ({count} total)")))?;
        Ok(())
    }

    async fn on_soul(&mut self) -> anyhow::Result<()> {
        let env = self.api.environment();
        let docs = SoulDocument::discover_all(&env.cwd, &env.base_path);
        if docs.is_empty() {
            self.writeln_title(TitleFormat::info(
                "No SOUL.md found. Create cwd/SOUL.md or ~/.aimee/SOUL.md.",
            ))?;
            return Ok(());
        }
        for doc in docs {
            self.writeln_title(TitleFormat::info(format!(
                "SOUL {} — {}",
                match doc.kind {
                    aimee_domain::SoulKind::Identity => "identity",
                    aimee_domain::SoulKind::Project => "project",
                },
                doc.path.display()
            )))?;
            let preview: String = doc.body.lines().take(8).collect::<Vec<_>>().join("\n");
            self.writeln(preview)?;
        }
        Ok(())
    }

    async fn on_team(&mut self, args: Vec<String>) -> anyhow::Result<()> {
        if args.first().map(String::as_str) == Some("run") {
            return self
                .on_team_run_named(args.get(1).map(String::as_str))
                .await;
        }
        if args.is_empty() || args[0] == "list" {
            for fixture in aimee_domain::Team::engineering_org() {
                self.writeln_title(TitleFormat::info(format!(
                    "Team {} ({} members, lead={})",
                    fixture.name,
                    fixture.members.len(),
            ...
            self.writeln(AgentWorkflow::engineering_ship().render())?;
            self.writeln("Start with /team run  (or /team run frontend|backend|platform)")?;
            return Ok(());
        }
        self.writeln_title(TitleFormat::info(format!(
            "Team spec recorded: {}",
    ...
    async fn on_team_run_named(&mut self, name: Option<&str>) -> anyhow::Result<()> {
        let workflow = match name {
            None | Some("engineering") | Some("ship") => AgentWorkflow::engineering_ship(),
            Some("frontend") => AgentWorkflow::frontend_ship(),
            Some("backend") => AgentWorkflow::backend_ship(),
            Some("platform") => AgentWorkflow::platform_ship(),
            Some(other) => anyhow::bail!(
                "Unknown team '{other}'. Use engineering, frontend, backend, or platform."
            ),
        };
        let Some(run) = WorkflowRun::start(workflow) else {
            anyhow::bail!("Workflow has no steps");
        };
        self.state.workflow = Some(run);
        self.dispatch_workflow_step().await
    }

    async fn dispatch_workflow_step(&mut self) -> anyhow::Result<()> {
        let (agent, prompt, label) = {
            let run = self
                .state
                .workflow
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No workflow is running"))?;
            let step = run
                .current()
                .ok_or_else(|| anyhow::anyhow!("Workflow has no current step"))?;
            (
                step.agent_id.clone(),
                run.step_prompt().expect("current step"),
                run.progress_label(),
            )
        };
        self.writeln_title(TitleFormat::info(format!("Workflow step {label}")))?;
        self.on_agent_change(agent).await?;
        self.spinner.start(None)?;
        Box::pin(self.on_message(Some(prompt))).await?;
        Ok(())
    }

    async fn on_learn(&mut self, args: Vec<String>) -> anyhow::Result<()> {
        let raw = args.join(" ");
        if raw.trim().is_empty() {
            anyhow::bail!("Usage: /learn <name> | <description>");
        }
        let (name, rest) = raw
            .split_once('|')
            .map(|(n, d)| (n.trim(), d.trim()))
            .unwrap_or((raw.trim(), "Autonomously captured Aimee Codes skill."));
        ...
        let path = draft.write_to(&self.api.environment().local_skills_path())?;
        self.writeln_title(TitleFormat::info(format!(
            "Wrote skill {} → {}",
            draft.name,
            path.display()
    ...
    async fn on_channel(&mut self, args: Vec<String>) -> anyhow::Result<()> {
        let path = self.api.environment().base_path.join("channels.json");
        let mut store = ChannelStore::load(path);
        if args.is_empty() || args[0] == "list" {
            if store.items.is_empty() {
                self.writeln_title(TitleFormat::info(
                    "No channels yet. Bind with /channel telegram <chat-id>, send with /channel send <text>.",
                ))?;
            } else {
                self.writeln_title(TitleFormat::info(format!(
                    "{} channel(s)",
        ...
        if args[0] == "send" {
            ...
            return self.on_channel_send(&store, text).await;
        }
        if args[0] == "poll" {
            return self.on_channel_poll(&store).await;
        }
        let kind = ChannelKind::parse(&args[0]).ok_or_else(|| {
            anyhow::anyhow!(
                "Unknown channel '{}'. Try /channel telegram <chat-id> or /channel send <text>.",
                args[0]
            )
        })?;
        let address = args.get(1).cloned().unwrap_or_else(|| "(unbound)".into());
        let endpoint = ChannelEndpoint::new(kind, address).enabled(true);
        store.upsert(endpoint.clone())?;
        self.writeln_title(TitleFormat::info(format!(
            "Channel {} (token stays in env, never this file)",
            endpoint.render()
        )))?;
        Ok(())
    }

    async fn on_channel_send(&mut self, store: &ChannelStore, text: String) -> anyhow::Result<()> {
        let endpoint = store
            .first_enabled(ChannelKind::Telegram)
            .ok_or_else(|| anyhow::anyhow!("Bind a chat first: /channel telegram <chat-id>"))?;
        let token = std::env::var("TELEGRAM_BOT_TOKEN").map_err(|_| {
            anyhow::anyhow!("TELEGRAM_BOT_TOKEN is not set (never store it in channels.json)")
        })?;
        let request = TelegramSendRequest::new(&token, &endpoint.address, text)?;
        let response = reqwest::Client::new()
        ...
        if !status.is_success() {
            anyhow::bail!(
                "Telegram send failed ({status}) via {}",
                TelegramSendRequest::redacted_url()
            );
        }
    ...
    async fn on_channel_poll(&mut self, store: &ChannelStore) -> anyhow::Result<()> {
        let token = std::env::var("TELEGRAM_BOT_TOKEN").map_err(|_| {
            anyhow::anyhow!("TELEGRAM_BOT_TOKEN is not set (never store it in channels.json)")
        })?;
        ...
        if !status.is_success() {
            anyhow::bail!(
                "Telegram poll failed ({status}) via {}",
                TelegramGetUpdatesRequest::redacted_url()
            );
        }
        let (updates, next) = aimee_domain::parse_get_updates(&body)?;
        if let Some(next) = next {
            if let Some(parent) = offset_path.parent() {
                std::fs::create_dir_all(parent)?;
            }
            std::fs::write(&offset_path, serde_json::to_string(&next)?)?;
        }
        let bound = store
            .first_enabled(ChannelKind::Telegram)
            .map(|e| e.address.as_str());
        let matched: Vec<_> = updates
            .into_iter()
            .filter(|item| bound.is_none_or(|chat| chat == item.chat_id))
            .collect();
        if matched.is_empty() {
            self.writeln_title(TitleFormat::info(format!(
                "No new Telegram messages via {}",
                TelegramGetUpdatesRequest::redacted_url()
            )))?;
            return Ok(());
        }
        self.writeln_title(TitleFormat::info(format!(
            "{} inbound telegram message(s)",
            matched.len()
        )))?;
        for item in &matched {
            self.writeln(item.as_user_prompt())?;
        }
        if let Some(latest) = matched.last() {
            self.spinner.start(None)?;
            Box::pin(self.on_message(Some(latest.as_user_prompt()))).await?;
        }
        Ok(())
    }
    ...
    async fn on_reasoning_effort_selection(&mut self, global: bool) -> anyhow::Result<()> {
        ...
        if let Some(effort_str) = selected {
            let effort = aimee_domain::Effort::from_str(&effort_str)
    ...
    async fn on_config_commit_model(&mut self) -> anyhow::Result<()> {
        ...
        if let Some((model, provider_id)) = selection {
            let commit_config = aimee_domain::ModelConfig::new(provider_id.clone(), model.clone());
            self.api
    ...
    async fn on_config_suggest_model(&mut self) -> anyhow::Result<()> {
        let selection = self.select_model(None, None).await?;
        if let Some((model, provider_id)) = selection {
            let suggest_config = aimee_domain::ModelConfig::new(provider_id.clone(), model.clone());
            self.api
    ...
    async fn on_config_edit(&mut self) -> anyhow::Result<()> {
        let config_path = aimee_config::ConfigReader::config_path();
        ...
        let editor = std::env::var("AIMEE_EDITOR")
        ...
        let status = std::process::Command::new(&editor_binary)
            ...
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to open editor '{}': {}. Set AIMEE_EDITOR or EDITOR.",
                    editor_binary,
                    e
                )
            })?;
    ...
    async fn on_edit_buffer(&mut self, initial: Option<String>) -> anyhow::Result<()> {
        ...
        // Create .aimee directory for the temp file
        let aimee_dir = self.state.cwd.join(".aimee");
        std::fs::create_dir_all(&aimee_dir)?;
        let temp_file = aimee_dir.join("AIMEE_EDITMSG.md");
    ...
    async fn on_suggest(&mut self, description: Option<String>) -> anyhow::Result<()> {
        let description = match description {
            ...
            _ => {
                let input = AimeeWidget::input("Describe the command you want")
                    .allow_empty(false)
                    .prompt()?;
                match input {
                    Some(d) if !d.is_empty() => d,
    ...
    async fn on_slash_conversation_rename(&mut self, args: Option<String>) -> anyhow::Result<()> {
        if let Some(args) = args {
            ...
            if let Some(conv) = selected {
                let name_result = AimeeWidget::input("New name").allow_empty(false).prompt()?;
    ...
    async fn on_copy(&mut self) -> anyhow::Result<()> {
        ...
        let content = context.messages.iter().rev().find_map(|msg| match &**msg {
            aimee_domain::ContextMessage::Text(aimee_api::TextMessage {
                content,
                role: aimee_domain::Role::Assistant,
                ..
            }) => Some(content.clone()),
            _ => None,
        });
    ...
    async fn select_model(
        ...
        // `aimee list models --porcelain`), then optionally filter by provider.
        self.spinner.start(Some("Loading"))?;
    ...
    async fn handle_api_key_input(
            ...
            .map(|param| {
                let param_value = if let Some(options) = &param.options {
                    ...
                    let starting = existing_url_params
                        ...
                        .unwrap_or(0);
                    AimeeWidget::select(format!("Select {}", param.name), options.clone())
                    ...
                    let mut input = AimeeWidget::input(label);
        ...
        let api_key_str = if let Some(default_key) = &request.api_key {
            ...
            if key_str == "google_adc_marker" || key_str == "aws_profile_marker" {
                ...
                let input = AimeeWidget::input(format!(
                ...
                let input = AimeeWidget::input(format!("Enter your {provider_id} API key"))
    ...
    fn display_oauth_device_info_new(
        ...
        let is_supergrok = display_uri.contains("x.ai") || verification_uri.contains("x.ai");

        self.writeln("")?;
        if is_supergrok {
            self.writeln(format!(
                "{}",
                "SuperGrok / SuperGrok Heavy login (xAI OAuth device code)".bold()
            ))?;
            self.writeln(format!(
                "{}",
                "Approve in the browser with your SuperGrok or eligible X Premium+ account. No API key required."
                    .dimmed()
            ))?;
        }
        self.writeln(format!(
    ...
    async fn handle_code_flow(
        ...
        let code = if let Some(server) = callback_server {
            ...
            let code =
                AimeeWidget::input(format!("Paste the authorization code for {provider_id}"))
                    .prompt()?
    ...
    async fn select_auth_method(
        ...
        let method_names: Vec<String> = auth_methods
            ...
            .map(|method| match method {
                ...
                AuthMethod::OAuthDevice(config)
                    if config.auth_url.host_str() == Some("auth.x.ai")
                        || provider_id == ProviderId::XAI_OAUTH =>
                {
                    "SuperGrok / SuperGrok Heavy (device login)".to_string()
                }
        ...
        match AimeeWidget::select("Select authentication method:", method_names.clone())
    ...
    async fn init_aimee_services(&mut self) -> Result<()> {
        ...
        self.writeln_title(
            TitleFormat::info("AimeeCodes Services enabled").sub_title(&credentials_path),
        )?;
        Ok(())
    }
    ...
    async fn configure_provider(
        ...
        if provider_id == ProviderId::AIMEE_SERVICES {
        ...
        if provider_id == ProviderId::CLAUDE_CODE {
            ...
            let confirmed = AimeeWidget::confirm("Do you want to continue with this provider?")
                .with_default(false)
                .prompt()?;
    ...
    /// selected provider.
    ///
    /// The display matches the shell plugin's `_aimee_select_provider`:
    ...
    async fn select_provider(
        ...
        let mut providers: Vec<AnyProvider> = self
            ...
            .into_iter()
            .filter(|p| {
                let filter = aimee_domain::ProviderType::Llm;
                match &p {
    ...
    async fn on_model_selection(
        ...
        self.api
            .update_config(vec![ConfigOperation::SetSessionConfig(
                aimee_domain::ModelConfig::new(provider_id, model.clone()),
            )])
            .await?;
    ...
    async fn finalize_provider_activation(
        ...
        if let Some(model) = model {
            ...
            self.api
                .update_config(vec![ConfigOperation::SetSessionConfig(
                    aimee_domain::ModelConfig::new(provider.id.clone(), model_id.clone()),
                )])
                .await?;
            self.writeln_title(
        ...
        if needs_model_selection {
            ...
            self.api
                .update_config(vec![ConfigOperation::SetSessionConfig(
                    aimee_domain::ModelConfig::new(provider.id.clone(), model),
                )])
                .await?;
    ...
    async fn init_conversation(&mut self) -> Result<ConversationId> {
        ...
        let id = if let Some(id) = self.state.conversation_id {
            ...
            let content = AimeeFS::read_utf8(path).await?;
    ...
    async fn on_message(&mut self, content: Option<String>) -> Result<()> {
        ...
        // input are provided. This handles the case: `echo "context" | aimee -p
        ...
        match message {
            ...
            ChatResponse::TaskComplete => {
                ...
                if let Some(format) = self.config.auto_dump.clone() {
                    let html = matches!(format, aimee_config::AutoDumpFormat::Html);
                    self.on_dump(html).await?;
                }
                Box::pin(self.after_turn_loops()).await?;
            }
        }
        Ok(())
    }
    ...
    async fn should_continue(&mut self) -> anyhow::Result<bool> {
        if aimee_domain::ApprovalMode::current().skips_prompts() {
            self.spinner.start(None)?;
            Box::pin(self.on_message(None)).await?;
            return Ok(true);
        }
        let should_continue = AimeeWidget::confirm("Do you want to continue anyway?")
            .with_default(true)
            .prompt()?;
    ...
    /// After a completed turn, advance a workflow or keep `/goal` looping.
    async fn after_turn_loops(&mut self) -> anyhow::Result<()> {
        if Box::pin(self.maybe_advance_workflow()).await? {
            return Ok(());
        }
        Box::pin(self.maybe_continue_goal()).await
    }

    async fn maybe_advance_workflow(&mut self) -> anyhow::Result<bool> {
        let Some(run) = self.state.workflow.as_mut() else {
            return Ok(false);
        };
        if run.advance() {
            Box::pin(self.dispatch_workflow_step()).await?;
            Ok(true)
        } else {
            self.state.workflow = None;
            self.writeln_title(TitleFormat::info("Workflow complete"))?;
            Ok(false)
        }
    }
    ...
    async fn last_assistant_text(&self) -> Option<String> {
        let id = self.state.conversation_id.as_ref()?;
        let conversation = self.api.conversation(id).await.ok().flatten()?;
        let context = conversation.context.as_ref()?;
        context.messages.iter().rev().find_map(|msg| match &**msg {
            ContextMessage::Text(aimee_domain::TextMessage {
                content,
                role: Role::Assistant,
                ..
            }) => Some(content.clone()),
    ...
    async fn handle_config_command(
        ...
        match command {
            ...
            crate::cli::ConfigCommand::Path => {
                let path = aimee_config::ConfigReader::config_path();
    ...
    /// Rename a legacy config directory (`~/aimee`, `~/.omega`, or `~/omega`)
    /// to `~/.aimee`.
    ///
    /// Errors if no legacy directory exists, if the new directory already
    /// exists, or if the rename fails.
    fn handle_config_migrate(&mut self) -> Result<()> {
        let home = dirs::home_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not determine home directory"))?;
        let new = home.join(".aimee");
        let legacy = ["aimee", ".omega", "omega"]
            .into_iter()
            .map(|name| home.join(name))
            .find(|path| path.exists() && path != &new)
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "No legacy config directory found (~/aimee, ~/.omega, or ~/omega) — nothing to migrate"
                )
            })?;
    ...
    async fn handle_config_set(&mut self, args: crate::cli::ConfigSetArgs) -> Result<()> {
        ...
        match args.field {
            ...
            ConfigSetField::Commit { provider, model } => {
                ...
                let commit_config =
                    aimee_domain::ModelConfig::new(provider.clone(), validated_model.clone());
                self.api
                    .update_config(vec![ConfigOperation::SetCommitConfig(Some(commit_config))])
                    .await?;
                self.writeln_title(
    ...
    async fn handle_zsh_rprompt_command(&mut self) -> Option<String> {
        let cid = std::env::var("_AIMEE_CONVERSATION_ID")
            .ok()
            .filter(|text| !text.trim().is_empty())
            .and_then(|str| ConversationId::from_str(str.as_str()).ok());

        let agent_id = std::env::var("_AIMEE_ACTIVE_AGENT")
            .ok()
            .filter(|text| !text.trim().is_empty())
            .map(AgentId::new);
    ...
    async fn validate_model(
        &self,
        model_str: &str,
        provider: Option<&aimee_domain::ProviderId>,
    ) -> Result<ModelId> {
    ...
    async fn on_query(
        &mut self,
        path: PathBuf,
        params: aimee_domain::SearchParams<'_>,
    ) -> anyhow::Result<()> {
        ...
        for result in results.iter() {
            match &result.node {
                aimee_domain::NodeData::FileChunk(chunk) => {
                    info = info.add_key_value(
                        "File",
                        format!(
                            "{}:{}-{}",
    ...
    fn format_workspace_info(workspace: &aimee_domain::WorkspaceInfo, is_active: bool) -> Info {
    ...
    async fn on_delete_workspaces(&mut self, workspace_ids: Vec<String>) -> anyhow::Result<()> {
        ...
        let parsed_ids: Vec<aimee_domain::WorkspaceId> = workspace_ids
            .iter()
            .map(|id| {
                aimee_domain::WorkspaceId::from_string(id)
    ...
    async fn on_workspace_init(
        ...
        let confirmed = if yes {
            ...
            AimeeWidget::confirm(format!(
                "This will sync and share the contents of '{}' with AimeeCodes Services. Do you wish to continue?",
                display_path
            ))
            .with_default(true)
            .prompt()?
    ...
    async fn handle_migrate_credentials(&mut self) -> Result<()> {
        ...
        if let Some(result) = result {
            self.writeln_title(
                TitleFormat::warning("Aimee no longer reads API keys from environment variables.")
                    .sub_title("Learn more: https://aimeecodes.dev/docs/custom-providers/"),
            )?;
