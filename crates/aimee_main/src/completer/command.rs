...
use aimee_select::AimeeWidget;

use crate::completer::input_completer::InputSuggestion;
use crate::completer::search_term::Span;
use crate::model::{AimeeCommand, AimeeCommandManager};

/// A display wrapper for `AimeeCommand` that renders the name and description
/// side-by-side for the interactive picker.
#[derive(Clone)]
struct CommandRow(AimeeCommand);
...
#[derive(Clone)]
pub struct CommandCompleter(Arc<AimeeCommandManager>);

impl CommandCompleter {
    pub fn new(command_manager: Arc<AimeeCommandManager>) -> Self {
    ...
    pub fn complete(&mut self, line: &str, _: usize) -> Vec<InputSuggestion> {
        ...
        let commands: Vec<CommandRow> = self
            ...
            .filter_map(|cmd| {
                ...
                // Bare `/` or `:` or empty → full menu; otherwise prefix filter.
                let matches =
                    line.is_empty() || line == "/" || line == ":" || display_name.starts_with(line);
                if matches {
                    Some(CommandRow(AimeeCommand {
                        name: display_name,
        ...
        let mut builder = AimeeWidget::select("Command", commands);
