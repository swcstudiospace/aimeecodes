...
use aimee_api::{AimeeConfig, Conversation, Environment, Metrics, Role, Usage, UserUsage};
...
impl From<&AimeeConfig> for Info {
    fn from(config: &AimeeConfig) -> Self {
        let mut info = Info::new();
        ...
        info = info
            ...
            .add_key_value("AimeeCodes Service URL", config.services_url.to_string())
...
/// Create an info instance for available commands from a AimeeCommandManager
impl From<&AimeeCommandManager> for Info {
    fn from(command_manager: &AimeeCommandManager) -> Self {
        ...
        info = info
            ...
            .add_key_value("<CTRL+D>", "Quit Aimee interactive shell")
    ...
    fn from(user_usage: &UserUsage) -> Self {
        ...
        if plan.is_upgradeable() {
            info = info.add_key_value(
                "Subscription",
                format!(
                    "{} [Upgrade https://app.aimeecodes.dev/app/billing]",
...
fn format_user_message(msg: &aimee_api::ContextMessage) -> Option<String> {
    let content = msg
        .as_value()
...
mod tests {
    ...
    use aimee_api::{Environment, EventValue};
    ...
    fn test_metrics_info_display() {
        use aimee_api::Metrics;
        use aimee_domain::{FileOperation, ToolKind};
    ...
    fn test_conversation_info_display() {
        ...
        let fixture = Conversation {
            ...
            context: None,
            metrics,
            metadata: aimee_domain::MetaData::new(Utc::now()),
        };
    ...
    fn test_conversation_info_display_with_task() {
        use aimee_api::{Context, ContextMessage, ConversationId, Role};
        use chrono::Utc;
        ...
        let context = Context::default()
            ...
            .add_message(ContextMessage::Text(
                aimee_domain::TextMessage::new(Role::User, "First user message")
            ...
            .add_message(ContextMessage::Text(
                aimee_domain::TextMessage::new(Role::User, "Create a new feature")
    ...
    fn test_info_from_command_manager() {
        let command_manager = super::AimeeCommandManager::default();
    ...
    #[test]
    fn test_upgradeable_plan_points_at_aimee_codes_billing() {
        use aimee_api::{Plan, UsageInfo, UserUsage};

        let fixture = UserUsage {
            plan: Plan { r#type: "free".to_string() },
            usage: UsageInfo { current: 1, limit: 10, remaining: 9, reset_in: None },
        };

        let actual = super::Info::from(&fixture).to_string();
        let expected = "https://app.aimeecodes.dev/app/billing";
        assert!(
            actual.contains(expected),
            "upgradeable plan should advertise Aimee Codes billing, got:\n{actual}"
        );
        assert!(
            !actual.contains("omegaloops"),
            "upgradeable plan must not advertise Omega Loops billing, got:\n{actual}"
        );
    }
}
