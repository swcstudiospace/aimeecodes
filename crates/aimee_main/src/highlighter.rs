...
/// Syntax highlighter for the aimee readline prompt.
///
...
pub struct AimeeHighlighter;

impl AimeeHighlighter {
    pub(crate) fn highlight(&self, line: &str, _cursor: usize) -> StyledText {
        ...
        if line.starts_with('!') {
            styled.push((theme::shell_style(), line.to_string()));
            return styled;
        }
            ...
            Some(start) => {
                ...
                match after_open.get(2..).and_then(|s| s.find(']')) {
                    ...
                    Some(rel_close) => {
                        ...
                        styled.push((theme::mention_style(), mention.to_string()));
...
mod tests {
    ...
    fn test_slash_command_highlighted() {
        let fixture = AimeeHighlighter;
        let actual = fixture.highlight("/compact", 0);
        assert_eq!(render(&actual), "/compact");
        assert_eq!(styles(&actual)[0], theme::command_style());
    }
    ...
    fn test_colon_command_with_args() {
        ...
        let parts: Vec<_> = actual
            ...
            .map(|(s, t)| (*s, t.as_str()))
            .collect();
        assert_eq!(parts[0], (theme::command_style(), ":commit"));
    ...
    fn test_shell_command_highlighted() {
        ...
        assert_eq!(styles(&actual)[0], theme::shell_style());
    }
    ...
    fn test_file_mention_highlighted() {
        ...
        assert_eq!(parts[1], (theme::mention_style(), "@[src/main.rs]"));
    ...
    fn test_multiple_file_mentions() {
        ...
        assert_eq!(actual.buffer[0].0, theme::mention_style());
        assert_eq!(actual.buffer[2].0, theme::mention_style());
    }
    ...
    fn test_file_mention_with_line_range() {
        ...
        assert_eq!(styles(&actual)[0], theme::mention_style());
    }
    ...
    fn test_file_mention_with_symbol() {
        ...
        let parts: Vec<_> = actual
            ...
            .map(|(s, t)| (*s, t.as_str()))
            .collect();
        assert_eq!(parts[1], (theme::mention_style(), "@[src/lib.rs#my_fn]"));
    }
    ...
    fn test_unterminated_mention_not_highlighted() {
        ...
        // No cyan segment
        assert!(styles(&actual).iter().all(|s| *s != theme::mention_style()));
    }
    ...
    fn test_slash_command_with_mention_in_args() {
        ...
        let parts: Vec<_> = actual
            ...
            .map(|(s, t)| (*s, t.as_str()))
            .collect();
        assert_eq!(parts[0], (theme::command_style(), "/review"));
        assert_eq!(parts[1], (Style::new(), " "));
        assert_eq!(parts[2], (theme::mention_style(), "@[src/main.rs]"));
    ...
    fn test_colon_command_with_mention_in_args() {
        ...
        assert_eq!(parts[0], (theme::command_style(), ":review"));
        assert_eq!(parts[1], (Style::new(), " "));
        assert_eq!(parts[2], (theme::mention_style(), "@[src/lib.rs]"));
    }
