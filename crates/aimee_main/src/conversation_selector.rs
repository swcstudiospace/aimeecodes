...
impl ConversationSelector {
    ...
    ///
    /// The preview command uses `aimee conversation info` and
    /// `aimee conversation show` to display the selected conversation's
    ...
    pub async fn select_conversation(
        ...
        let preview_command =
            "CLICOLOR_FORCE=1 aimee conversation info {1}; echo; CLICOLOR_FORCE=1 aimee conversation show {1}"
                .to_string();

        let selected_uuid = tokio::task::spawn_blocking(move || -> Result<Option<String>> {
            Ok(AimeeWidget::select_rows("Conversation", rows)
                .query(query)
...
mod tests {
    use aimee_api::Conversation;
    use aimee_domain::{ConversationId, MetaData, Metrics};
    use chrono::Utc;
