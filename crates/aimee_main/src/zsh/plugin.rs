...
pub fn generate_zsh_plugin() -> Result<String> {
    ...
    for file in aimee_embed::files(&ZSH_PLUGIN_LIB) {
    ...
    generate(Zsh, &mut cmd, "aimee", &mut completions);
    ...
    output.push_str("\n_AIMEE_PLUGIN_LOADED=$(date +%s)\n");

    Ok(output)
}

/// Generates the ZSH theme for Aimee
pub fn generate_zsh_theme() -> Result<String> {
    let mut content =
        super::normalize_script(include_str!("../../../../shell-plugin/aimee.theme.zsh"));

    // Set environment variable to indicate theme is loaded (with timestamp)
    content.push_str("\n_AIMEE_THEME_LOADED=$(date +%s)\n");
...
fn create_temp_zsh_script(script_content: &str) -> Result<(tempfile::TempDir, PathBuf)> {
    ...
    let script_path = temp_dir.path().join("aimee_script.zsh");
...
/// * `aimee_editor` - If Some(editor), adds AIMEE_EDITOR export to .zshrc
///
/// # Errors
///
/// Returns error if:
...
/// - Invalid aimee markers are found (incomplete or incorrectly ordered)
/// - A backup of the existing .zshrc cannot be created
pub fn setup_zsh_integration(
    disable_nerd_font: bool,
    aimee_editor: Option<&str>,
) -> Result<ZshSetupResult> {
    const START_MARKER: &str = "# >>> aimee initialize >>>";
    const END_MARKER: &str = "# <<< aimee initialize <<<";
    const AIMEE_INIT_CONFIG_RAW: &str = include_str!("../../../../shell-plugin/aimee.setup.zsh");
    let aimee_init_config = super::normalize_script(AIMEE_INIT_CONFIG_RAW);
    ...
    // Build the aimee config block with markers
    let mut aimee_config: Vec<String> = vec![START_MARKER.to_string()];
    aimee_config.extend(aimee_init_config.lines().map(String::from));

    // Add nerd font configuration if requested
    if disable_nerd_font {
        aimee_config.push(String::new()); // Add blank line before comment
        aimee_config.push(
            "# Disable Nerd Fonts (set during setup - icons not displaying correctly)".to_string(),
        );
        aimee_config.push("# To re-enable: remove this line and install a Nerd Font from https://www.nerdfonts.com/".to_string());
        aimee_config.push("export NERD_FONT=0".to_string());
    }

    // Add editor configuration if requested
    if let Some(editor) = aimee_editor {
        aimee_config.push(String::new()); // Add blank line before comment
        aimee_config.push("# Editor for editing prompts (set during setup)".to_string());
        aimee_config.push("# To change: update AIMEE_EDITOR or remove to use $EDITOR".to_string());
        aimee_config.push(format!("export AIMEE_EDITOR=\"{}\"", editor));
    }

    aimee_config.push(END_MARKER.to_string());

    // Add or update aimee configuration block based on marker state
    let (new_content, config_action) = match marker_state {
        MarkerState::Valid { start, end } => {
            // Markers exist - replace content between them
            lines.splice(start..=end, aimee_config.iter().cloned());
            (lines.join("\n") + "\n", "updated")
        }
        MarkerState::Invalid { start, end } => {
            ...
            let mut error =
                anyhow::anyhow!("Invalid aimee markers found in {}", zshrc_path.display());
        ...
        MarkerState::NotFound => {
            ...
            lines.extend(aimee_config.iter().cloned());
            (lines.join("\n") + "\n", "added")
        }
    };
    ...
    Ok(ZshSetupResult {
        message: format!("aimee plugins {}", config_action),
        backup_path,
    })
}
...
mod tests {
    ...
    fn test_run_zsh_doctor_streaming() {
        ...
        unsafe {
            std::env::set_var("AIMEE_SKIP_INTERACTIVE", "1");
        }
        ...
        unsafe {
            std::env::remove_var("AIMEE_SKIP_INTERACTIVE");
        }
    ...
    fn test_generated_plugin_wraps_zle_commands_with_osc133_markers() {
        ...
        let actual = fixture.contains("    _aimee_osc133_emit \"B\"\n    _aimee_osc133_emit \"C\"")
            ...
            // _aimee_reset pads with newlines before clearing to prevent ZLE from clearing
            ...
            // D and A markers follow the action, with _aimee_reset at the end
            && fixture.contains(
    ...
    /// Regression: aimee keybindings must survive zsh-vi-mode's `zvm_init`
    ...
    fn test_generated_plugin_registers_zvm_after_init_hook() {
        ...
        let fixture = generate_zsh_plugin().unwrap();
        let actual = fixture.contains("function _aimee_apply_keybindings()")
            && fixture.contains("typeset -ga zvm_after_init_commands")
            && fixture.contains("zvm_after_init_commands+=('_aimee_apply_keybindings')");
        let expected = true;
        assert_eq!(actual, expected);
    }
    ...
    fn test_setup_zsh_integration_without_nerd_font_config() {
        ...
        assert!(content.contains("# >>> aimee initialize >>>"));
        assert!(content.contains("# <<< aimee initialize <<<"));
    }
    ...
    fn test_setup_zsh_integration_with_editor() {
        ...
        // Should contain AIMEE_EDITOR with explanatory comments
        assert!(
            content.contains("export AIMEE_EDITOR=\"code --wait\""),
            "Content should contain AIMEE_EDITOR:\n{}",
            content
        );
        assert!(
        ...
        assert!(
            content.contains("# To change: update AIMEE_EDITOR or remove to use $EDITOR"),
    ...
    fn test_setup_zsh_integration_with_both_configs() {
        ...
        assert!(
            content.contains("export AIMEE_EDITOR=\"vim\""),
            "Content should contain AIMEE_EDITOR:\n{}",
            content
        );
    ...
    fn test_setup_zsh_integration_updates_existing_markers() {
        ...
        assert!(
            !content.contains("export AIMEE_EDITOR"),
            "Should not contain AIMEE_EDITOR after first setup"
        );
        ...
        assert!(
            content.contains("export AIMEE_EDITOR=\"nvim\""),
            "Should contain AIMEE_EDITOR after update:\n{}",
            content
        );
        ...
        assert_eq!(
            content.matches("# >>> aimee initialize >>>").count(),
            1,
