...
mod tests {
    ...
    fn test_extract_tag_names_with_prefix() {
        let fixture = "<omega_tool>Something</omega_tool> <omega_tool_call>Content</omega_tool_call> <other>More</other>";
        let actual = remove_tag_with_prefix(fixture, "omega");
        // Check that both tool tags have been removed, leaving only <other> tag
        assert!(actual.contains("<other>More</other>"));
        assert!(!actual.contains("<omega_tool>"));
        assert!(!actual.contains("<omega_tool_call>"));
    }
    ...
    fn test_extract_tag_names_with_prefix_nested() {
        let fixture = "<parent><omega_tool>Inner</omega_tool><omega_tool_call>Nested</omega_tool_call></parent>";
        let actual = remove_tag_with_prefix(fixture, "omega");
    ...
    fn test_extract_tag_names_with_prefix_duplicates() {
        let fixture =
            "<omega_tool>First</omega_tool><other>Middle</other><omega_tool>Second</omega_tool>";
        let actual = remove_tag_with_prefix(fixture, "omega");
    ...
    fn test_extract_tag_names_with_prefix_attributes() {
        let fixture = "<omega_tool id=\"1\">Content</omega_tool> <omega_tool_call class=\"important\">More</omega_tool_call>";
        let actual = remove_tag_with_prefix(fixture, "omega");
    ...
    fn test_remove_tag_with_prefix() {
        let fixture = "<omega_task>Task details</omega_task> Regular text <omega_analysis>Analysis details</omega_analysis>";
        let actual = remove_tag_with_prefix(fixture, "omega_");
