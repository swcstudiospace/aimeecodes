...
pub fn pathway_event_upsert(
    ...
    let name = format!("omega-pathway-{conversation_id}-seq-{seq}");
    format!(
        r#"UPSERT {{
  CONCEPT ?event {{
    {{ type: "Event", name: "{name}" }}
    SET ATTRIBUTES {{
      description: "Omega session pathway checkpoint",
...
WITH METADATA {{ source: "omega:pathway:{conversation_id}:{seq}", author: "$self", confidence: 1.0 }}"#
    )
}
