...
use crate::{OmegaConfig, ModelConfig};

/// Intermediate representation of the legacy `~/omega/.config.json` format.
///
...
impl LegacyConfig {
    /// Reads the legacy `~/omega/.config.json` file at `path`, parses it, and
    /// returns the equivalent TOML representation as a [`String`].
    ///
    /// Because every field in [`OmegaConfig`] is `Option`, fields not covered
    ...
    pub(crate) fn read(path: &PathBuf) -> crate::Result<String> {
        ...
        let omega_config = config.into_omega_config();
        let content = toml_edit::ser::to_string_pretty(&omega_config)?;
        Ok(content)
    }

    /// Converts a [`LegacyConfig`] into the fields of [`OmegaConfig`] that it
    /// covers, leaving all other fields at their defaults (`None`).
    fn into_omega_config(self) -> OmegaConfig {
        ...
        OmegaConfig { session, commit, suggest, ..Default::default() }
    }
}
