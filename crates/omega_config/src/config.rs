...
pub enum ModelListConfig {
    ...
    Hardcoded(Vec<omega_domain::Model>),
}

/// A single provider entry defined inline in `omega.toml`.
///
...
/// Top-level Omega configuration merged from all sources (defaults, file,
/// environment).
...
pub struct OmegaConfig {
    ...
    /// Base URL of the Omega services API used for semantic search and
    /// indexing.
    #[serde(default)]
    #[dummy(expr = "\"https://api.omegaloops.dev/api\".to_string()")]
    ...
    /// Whether `omega commit` should override `GIT_COMMITTER_NAME` and
    /// `GIT_COMMITTER_EMAIL` with the Omega identity. Defaults to `true` via
    /// the embedded `.omega.toml` defaults.
    #[serde(default)]
    pub use_omega_committer: bool,
    ...
    // --- Workflow fields ---
    /// Configuration for automatic Omega updates.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// behavior unless explicitly enabled in `omega.toml`.
    #[serde(default)]
    ...
    /// Enables subagent support via the task tool; when true the omega agent
    ...
    /// Enables automatic VS Code extension installation when Omega runs inside
...
impl OmegaConfig {
    ...
    pub fn read() -> crate::Result<OmegaConfig> {
...
mod tests {
    ...
    fn test_f32_temperature_round_trip() {
        let fixture = OmegaConfig { temperature: Some(Decimal(0.1)), ..Default::default() };
    ...
    fn test_f32_top_p_round_trip() {
        let fixture = OmegaConfig { top_p: Some(Decimal(0.9)), ..Default::default() };
...
"#;
        ...
        let expected = vec![ProviderEntry {
            ...
            models: Some(ModelListConfig::Hardcoded(vec![
                omega_domain::Model::new("Qwen3.6-35B-A3b-q3-mlx")
                ...
                omega_domain::Model::new("llama3.2-3b")
