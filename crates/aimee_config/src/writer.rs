...
/// Writes a [`AimeeConfig`] to the user configuration file on disk.
...
impl ConfigWriter {
    ...
    pub fn new(config: AimeeConfig) -> Self {
        Self { config }
    }
    ...
    ///
    /// The output includes a leading `$schema` key pointing to the Aimee
    ...
    pub fn write(&self, path: &Path) -> crate::Result<()> {
        ...
        let contents =
            format!("\"$schema\" = \"https://aimeecodes.dev/schema.json\"\n\n{config_toml}");
