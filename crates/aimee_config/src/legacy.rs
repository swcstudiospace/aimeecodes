...
use crate::{AimeeConfig, ModelConfig};

/// Intermediate representation of the legacy `~/aimee/.config.json` format.
///
...
impl LegacyConfig {
    /// Reads the legacy `~/aimee/.config.json` file at `path`, parses it, and
    /// returns the equivalent TOML representation as a [`String`].
    ///
    /// Because every field in [`AimeeConfig`] is `Option`, fields not covered
    ...
    pub(crate) fn read(path: &PathBuf) -> crate::Result<String> {
        ...
        let aimee_config = config.into_aimee_config();
        let content = toml_edit::ser::to_string_pretty(&aimee_config)?;
        Ok(content)
    }

    /// Converts a [`LegacyConfig`] into the fields of [`AimeeConfig`] that it
    /// covers, leaving all other fields at their defaults (`None`).
    fn into_aimee_config(self) -> AimeeConfig {
        ...
        AimeeConfig { session, commit, suggest, ..Default::default() }
    }
}
