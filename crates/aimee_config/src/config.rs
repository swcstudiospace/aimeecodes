...
use crate::{
    AndaConfig, AutoDumpFormat, Compact, Decimal, HttpConfig, ModelConfig, ReasoningConfig,
    RetryConfig, Update,
};
...
pub enum ModelListConfig {
    ...
    Hardcoded(Vec<aimee_domain::Model>),
}

/// A single provider entry defined inline in `aimee.toml`.
///
...
/// Top-level Aimee configuration merged from all sources (defaults, file,
/// environment).
#[derive(Default, Debug, Setters, Clone, PartialEq, Serialize, Deserialize, JsonSchema, Dummy)]
#[serde(rename_all = "snake_case")]
#[setters(strip_option)]
pub struct AimeeConfig {
    ...
    /// Base URL of the Aimee services API used for semantic search and
    /// indexing.
    #[serde(default)]
    #[dummy(expr = "\"https://api.aimeecodes.dev/api\".to_string()")]
    ...
    /// Whether `aimee commit` should override `GIT_COMMITTER_NAME` and
    /// `GIT_COMMITTER_EMAIL` with the Aimee identity. Defaults to `true` via
    /// the embedded `.aimee.toml` defaults.
    #[serde(default)]
    pub use_aimee_committer: bool,
    ...
    // --- Workflow fields ---
    /// Configuration for automatic Aimee updates.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// behavior unless explicitly enabled in `aimee.toml`.
    #[serde(default)]
    ...
    /// Enables subagent support via the task tool; when true the aimee agent
    ...
    /// Enables automatic VS Code extension installation when Aimee runs inside
    ...
    /// Anda / KIP eternal session pathway settings.
    ///
    /// When enabled, agent outputs append hash-chained conversation checkpoints
    /// for chat-only rollbacks and optional ICP durability.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub anda: Option<AndaConfig>,
}
...
impl AimeeConfig {
    ...
    pub fn read() -> crate::Result<AimeeConfig> {
...
mod tests {
    ...
    fn test_f32_temperature_round_trip() {
        let fixture = AimeeConfig { temperature: Some(Decimal(0.1)), ..Default::default() };
    ...
    fn test_f32_top_p_round_trip() {
        let fixture = AimeeConfig { top_p: Some(Decimal(0.9)), ..Default::default() };
...
"#;
        ...
        let expected = vec![ProviderEntry {
            ...
            models: Some(ModelListConfig::Hardcoded(vec![
                aimee_domain::Model::new("Qwen3.6-35B-A3b-q3-mlx")
                ...
                aimee_domain::Model::new("llama3.2-3b")
...
"#;
    ...
    #[test]
    fn test_default_services_url_uses_aimee_codes_domain() {
        let fixture = ConfigReader::default().read_defaults().build().unwrap();
        let actual = fixture.services_url;
        let expected = "https://api.aimeecodes.dev/".to_string();
