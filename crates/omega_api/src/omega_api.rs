...
use omega_app::dto::ToolsOverview;
use omega_app::{
    AgentProviderResolver, AgentRegistry, AppConfigService, AuthService, CommandInfra,
    CommandLoaderService, ConversationService, DataGenerationApp, EnvironmentInfra,
    FileDiscoveryService, OmegaApp, GitApp, GrpcInfra, McpConfigManager, McpService,
    ProviderAuthService, ProviderService, Services, User, UserUsage, Walker, WorkspaceService,
};
use omega_config::OmegaConfig;
use omega_domain::{Agent, ConsoleWriter, *};
use omega_infra::OmegaInfra;
use omega_repo::OmegaRepo;
use omega_services::OmegaServices;
use omega_stream::MpscStream;
use futures::stream::BoxStream;
use url::Url;

use crate::API;

pub struct OmegaAPI<S, F> {
    services: Arc<S>,
    infra: Arc<F>,
}

impl<A, F> OmegaAPI<A, F> {
    pub fn new(services: Arc<A>, infra: Arc<F>) -> Self {
        Self { services, infra }
    }

    /// Creates a OmegaApp instance with the current services and latest config.
    fn app(&self) -> OmegaApp<A>
    where
        A: Services + EnvironmentInfra<Config = omega_config::OmegaConfig>,
        F: EnvironmentInfra<Config = omega_config::OmegaConfig>,
    {
        OmegaApp::new(self.services.clone())
    }
}

impl OmegaAPI<OmegaServices<OmegaRepo<OmegaInfra>>, OmegaRepo<OmegaInfra>> {
    /// Creates a fully-initialized [`OmegaAPI`] from a pre-read configuration.
    ///
    /// # Arguments
    ...
    pub fn init(cwd: PathBuf, config: OmegaConfig) -> Self {
        let infra = Arc::new(OmegaInfra::new(cwd, config));
        let repo = Arc::new(OmegaRepo::new(infra.clone()));
        let app = Arc::new(OmegaServices::new(repo.clone()));
        OmegaAPI::new(app, repo)
    }

    pub async fn get_skills_internal(&self) -> Result<Vec<Skill>> {
        use omega_domain::SkillRepository;
...
> API for OmegaAPI<A, F>
{
    ...
    async fn commit(
    ...
    ) -> Result<omega_app::CommitResult> {
        let use_omega_committer = self
            .services
            .get_config()
            .context("Failed to read omega config for commit settings")?
            .use_omega_committer;
        ...
        if preview {
            ...
            git_app
                .commit(result.message, result.has_staged_files, use_omega_committer)
                .await
        }
    }
    ...
    async fn update_config(&self, ops: Vec<omega_domain::ConfigOperation>) -> anyhow::Result<()> {
        ...
        let needs_agent_reload = ops
            .iter()
            .any(|op| matches!(op, omega_domain::ConfigOperation::SetSessionConfig(_)));
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use omega_app::CommandGenerator;
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> Result<MpscStream<Result<omega_domain::SyncProgress>>> {
    ...
    ) -> Result<Vec<omega_domain::Node>> {
        self.services.query_workspace(path, params).await
    }

    async fn list_workspaces(&self) -> Result<Vec<omega_domain::WorkspaceInfo>> {
    ...
    async fn get_workspace_info(
        &self,
        path: PathBuf,
    ) -> Result<Option<omega_domain::WorkspaceInfo>> {
        self.services.get_workspace_info(path).await
    }

    async fn delete_workspaces(&self, workspace_ids: Vec<omega_domain::WorkspaceId>) -> Result<()> {
        self.services.delete_workspaces(&workspace_ids).await
    }

    async fn get_workspace_status(&self, path: PathBuf) -> Result<Vec<omega_domain::FileStatus>> {
    ...
    async fn create_auth_credentials(&self) -> Result<omega_domain::WorkspaceAuth> {
        self.services.init_auth_credentials().await
    }

    async fn init_workspace(&self, path: PathBuf) -> Result<omega_domain::WorkspaceId> {
        self.services.init_workspace(path).await
    }

    async fn migrate_env_credentials(&self) -> Result<Option<omega_domain::MigrationResult>> {
    ...
    async fn get_session_config(&self) -> Option<omega_domain::ModelConfig> {
    ...
    async fn get_default_provider(&self) -> Result<Provider<Url>> {
        let model_config = self
            ...
            .get_session_config()
            .await
            .ok_or_else(|| omega_domain::Error::NoDefaultSession)?;
    ...
    async fn mcp_auth(&self, server_url: &str) -> Result<()> {
        ...
        omega_infra::mcp_auth(server_url, &env).await
    }

    async fn mcp_logout(&self, server_url: Option<&str>) -> Result<()> {
        let env = self.services.get_environment().clone();
        match server_url {
            Some(url) => omega_infra::mcp_logout(url, &env).await,
            None => omega_infra::mcp_logout_all(&env).await,
        }
    }

    async fn mcp_auth_status(&self, server_url: &str) -> Result<String> {
        let env = self.services.get_environment().clone();
        Ok(omega_infra::mcp_auth_status(server_url, &env).await)
    }
...
impl<A: Send + Sync, F: ConsoleWriter> ConsoleWriter for OmegaAPI<A, F> {
