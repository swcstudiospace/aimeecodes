...
use omega_eventsource::EventSource;
// Re-export CacacheStorage from omega_infra
pub use omega_infra::CacacheStorage;
use reqwest::Response;
use reqwest::header::HeaderMap;
use url::Url;

use crate::agent::OmegaAgentRepository;
use crate::context_engine::OmegaContextEngineRepository;
use crate::conversation::ConversationRepositoryImpl;
use crate::database::{DatabasePool, PoolConfig};
use crate::fs_snap::OmegaFileSnapshotService;
use crate::fuzzy_search::OmegaFuzzySearchRepository;
use crate::provider::{OmegaChatRepository, OmegaProviderRepository};
use crate::skill::OmegaSkillRepository;
use crate::validation::OmegaValidationRepository;
...
#[derive(Clone)]
pub struct OmegaRepo<F> {
    infra: Arc<F>,
    file_snapshot_service: Arc<OmegaFileSnapshotService>,
    conversation_repository: Arc<ConversationRepositoryImpl>,
    mcp_cache_repository: Arc<CacacheStorage>,
    provider_repository: Arc<OmegaProviderRepository<F>>,
    chat_repository: Arc<OmegaChatRepository<F>>,
    codebase_repo: Arc<OmegaContextEngineRepository<F>>,
    agent_repository: Arc<OmegaAgentRepository<F>>,
    skill_repository: Arc<OmegaSkillRepository<F>>,
    validation_repository: Arc<OmegaValidationRepository<F>>,
    fuzzy_search_repository: Arc<OmegaFuzzySearchRepository<F>>,
}
...
> OmegaRepo<F>
{
    pub fn new(infra: Arc<F>) -> Self {
        ...
        let file_snapshot_service = Arc::new(OmegaFileSnapshotService::new(env.clone()));
        let db_pool =
        ...
        let provider_repository = Arc::new(OmegaProviderRepository::new(infra.clone()));
        let chat_repository = Arc::new(OmegaChatRepository::new(infra.clone()));

        let codebase_repo = Arc::new(OmegaContextEngineRepository::new(infra.clone()));
        let agent_repository = Arc::new(OmegaAgentRepository::new(infra.clone()));
        let skill_repository = Arc::new(OmegaSkillRepository::new(infra.clone()));
        let validation_repository = Arc::new(OmegaValidationRepository::new(infra.clone()));
        let fuzzy_search_repository = Arc::new(OmegaFuzzySearchRepository::new(infra.clone()));
        Self {
            infra,
...
impl<F: Send + Sync> SnapshotRepository for OmegaRepo<F> {
...
#[async_trait::async_trait]
impl<F: Send + Sync> ConversationRepository for OmegaRepo<F> {
...
impl<
    F: EnvironmentInfra<Config = omega_config::OmegaConfig>
        ...
        + Send
        + Sync,
> ChatRepository for OmegaRepo<F>
{
    async fn chat(
        &self,
        model_id: &ModelId,
...
impl<
    F: EnvironmentInfra<Config = omega_config::OmegaConfig>
        ...
        + Send
        + Sync,
> ProviderRepository for OmegaRepo<F>
{
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra<Config = omega_config::OmegaConfig> + Send + Sync> EnvironmentInfra
    for OmegaRepo<F>
{
...
#[async_trait::async_trait]
impl<F: Send + Sync> KVStore for OmegaRepo<F> {
...
#[async_trait::async_trait]
impl<F: HttpInfra> HttpInfra for OmegaRepo<F> {
...
#[async_trait::async_trait]
impl<F> FileReaderInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> WalkerInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileWriterInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileInfoInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileDirectoryInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileRemoverInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> DirectoryReaderInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> UserInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> McpServerInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F> CommandInfra for OmegaRepo<F>
where
...
#[async_trait::async_trait]
impl<F: FileInfoInfra + EnvironmentInfra<Config = OmegaConfig> + DirectoryReaderInfra + Send + Sync>
    AgentRepository for OmegaRepo<F>
{
    async fn get_agents(&self) -> anyhow::Result<Vec<omega_domain::Agent>> {
        self.agent_repository.get_agents().await
    }

    async fn get_agent_infos(&self) -> anyhow::Result<Vec<omega_domain::AgentInfo>> {
...
impl<F: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra + Send + Sync>
    SkillRepository for OmegaRepo<F>
{
...
impl<F: StrategyFactory> StrategyFactory for OmegaRepo<F> {
    type Strategy = F::Strategy;

    fn create_auth_strategy(
        &self,
        provider_id: ProviderId,
        auth_method: omega_domain::AuthMethod,
        required_params: Vec<omega_domain::URLParamSpec>,
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> omega_domain::WorkspaceIndexRepository for OmegaRepo<F> {
    async fn authenticate(&self) -> anyhow::Result<omega_domain::WorkspaceAuth> {
    ...
    async fn create_workspace(
        ...
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<omega_domain::WorkspaceId> {
        self.codebase_repo
    ...
    async fn upload_files(
        &self,
        upload: &omega_domain::FileUpload,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<omega_domain::FileUploadInfo> {
        self.codebase_repo.upload_files(upload, auth_token).await
    }

    async fn search(
        &self,
        query: &omega_domain::CodeSearchQuery<'_>,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<Vec<omega_domain::Node>> {
    ...
    async fn list_workspaces(
        &self,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<Vec<omega_domain::WorkspaceInfo>> {
        self.codebase_repo.list_workspaces(auth_token).await
    }

    async fn get_workspace(
        &self,
        workspace_id: &omega_domain::WorkspaceId,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<Option<omega_domain::WorkspaceInfo>> {
        self.codebase_repo
    ...
    async fn list_workspace_files(
        &self,
        workspace: &omega_domain::WorkspaceFiles,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<Vec<omega_domain::FileHash>> {
        self.codebase_repo
    ...
    async fn delete_files(
        &self,
        deletion: &omega_domain::FileDeletion,
        auth_token: &omega_domain::ApiKey,
    ) -> anyhow::Result<()> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> omega_domain::ValidationRepository for OmegaRepo<F> {
    async fn validate_file(
        &self,
        path: impl AsRef<std::path::Path> + Send,
        content: &str,
    ) -> anyhow::Result<Vec<omega_domain::SyntaxError>> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> FuzzySearchRepository for OmegaRepo<F> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> TextPatchRepository for OmegaRepo<F> {
    async fn build_text_patch(
        ...
        let mut client =
            crate::proto_generated::omega_service_client::OmegaServiceClient::new(channel);
...
impl<F: GrpcInfra> GrpcInfra for OmegaRepo<F> {
...
impl<F: omega_domain::ConsoleWriter> omega_domain::ConsoleWriter for OmegaRepo<F> {
