...
pub struct OmegaContextEngineRepository<I> {
    infra: Arc<I>,
}

impl<I> OmegaContextEngineRepository<I> {
...
#[async_trait]
impl<I: GrpcInfra> WorkspaceIndexRepository for OmegaContextEngineRepository<I> {
    ...
    async fn search(
        &self,
        search_query: &omega_domain::CodeSearchQuery<'_>,
        auth_token: &omega_domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        omega_domain::NodeData::FileChunk(omega_domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        omega_domain::NodeData::File(omega_domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        omega_domain::NodeData::FileRef(omega_domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        omega_domain::NodeData::Note(omega_domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        omega_domain::NodeData::Task(omega_domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &omega_domain::ApiKey,
    ) -> Result<Vec<omega_domain::FileHash>> {
