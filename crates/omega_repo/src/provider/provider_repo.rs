...
use bytes::Bytes;
use omega_app::domain::{ProviderId, ProviderResponse};
use omega_app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra, HttpInfra};
use omega_domain::{
...
enum Models {
    ...
    Hardcoded(Vec<omega_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<omega_domain::AuthMethod>,
    #[serde(default)]
    #[merge(strategy = overwrite)]
...
impl From<omega_config::ProviderUrlParam> for UrlParamVarConfig {
    fn from(param: omega_config::ProviderUrlParam) -> Self {
...
impl From<omega_config::ProviderEntry> for ProviderConfig {
    fn from(entry: omega_config::ProviderEntry) -> Self {
        let provider_type = match entry.provider_type {
            Some(omega_config::ProviderTypeEntry::ContextEngine) => {
                omega_domain::ProviderType::ContextEngine
            }
            Some(omega_config::ProviderTypeEntry::Llm) | None => omega_domain::ProviderType::Llm,
        };

        let auth_methods = if entry.auth_methods.is_empty() {
            vec![omega_domain::AuthMethod::ApiKey]
        } else {
            entry
                .auth_methods
                .into_iter()
                .map(|m| match m {
                    omega_config::ProviderAuthMethod::ApiKey => omega_domain::AuthMethod::ApiKey,
                    omega_config::ProviderAuthMethod::GoogleAdc => {
                        omega_domain::AuthMethod::GoogleAdc
                    }
                })
                .collect()
        };
        ...
        let response_type = entry.response_type.map(|r| match r {
            omega_config::ProviderResponseType::OpenAI => ProviderResponse::OpenAI,
            omega_config::ProviderResponseType::OpenAIResponses => {
                ProviderResponse::OpenAIResponses
            }
            omega_config::ProviderResponseType::Anthropic => ProviderResponse::Anthropic,
            omega_config::ProviderResponseType::Bedrock => ProviderResponse::Bedrock,
            omega_config::ProviderResponseType::Google => ProviderResponse::Google,
            omega_config::ProviderResponseType::OpenCode => ProviderResponse::OpenCode,
        });

        let models = entry.models.map(|m| match m {
            omega_config::ModelListConfig::Url(url) => Models::Url(url),
            omega_config::ModelListConfig::Hardcoded(model_list) => Models::Hardcoded(model_list),
        });
...
impl From<&ProviderConfig> for omega_domain::ProviderTemplate {
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => omega_domain::ModelSource::Url(
                omega_domain::Template::<omega_domain::URLParameters>::new(model_url_template),
            ),
            Models::Hardcoded(model_list) => {
                omega_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: omega_domain::Template::<omega_domain::URLParameters>::new(&config.url),
...
pub struct OmegaProviderRepository<F> {
    infra: Arc<F>,
}

impl<F: EnvironmentInfra<Config = omega_config::OmegaConfig> + HttpInfra>
    OmegaProviderRepository<F>
{
...
> OmegaProviderRepository<F>
{
    ...
    /// Converts provider entries from `OmegaConfig` into `ProviderConfig`
    ...
    async fn get_providers(&self) -> Vec<AnyProvider> {
        ...
        for config in configs {
            // Skip Omega provider as it's handled specially
            if config.id == ProviderId::OMEGA {
                continue;
            }
    ...
    pub async fn migrate_env_to_file(&self) -> anyhow::Result<Option<MigrationResult>> {
        ...
        for config in configs {
            // Skip Omega provider and ContextEngine providers - they're not configurable
            // via env like other providers
            if config.id == ProviderId::OMEGA || config.provider_type == ProviderType::ContextEngine
            {
                continue;
            }
    ...
    async fn create_provider(
        &self,
        config: &ProviderConfig,
    ) -> anyhow::Result<omega_domain::ProviderTemplate> {
        ...
        if (credential.id == omega_domain::ProviderId::VERTEX_AI
            || credential.id == omega_domain::ProviderId::VERTEX_AI_ANTHROPIC)
            && let omega_domain::AuthDetails::ApiKey(ref api_key) = credential.auth_details
    ...
    async fn refresh_google_adc_credential(
        &self,
        original_credential: &omega_domain::AuthCredential,
    ) -> anyhow::Result<omega_domain::AuthCredential> {
        ...
        Ok(omega_domain::AuthCredential::new_api_key(
            original_credential.id.clone(),
            omega_domain::ApiKey::from(access_token.token),
        )
    ...
    async fn provider_from_id(
        ...
        if id == ProviderId::OMEGA {
            // Omega provider isn't typically configured via env vars in the registry
            return Err(Error::provider_not_available(ProviderId::OMEGA).into());
        }
    ...
    async fn get_merged_configs(&self) -> Vec<ProviderConfig> {
        ...
        // Merge inline configs from OmegaConfig (omega.toml `providers` field)
...
impl<
    F: EnvironmentInfra<Config = omega_config::OmegaConfig>
        ...
        + HttpInfra
        + Sync,
> ProviderRepository for OmegaProviderRepository<F>
{
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>> {
        Ok(self.get_providers().await)
    }

    async fn get_provider(&self, id: ProviderId) -> anyhow::Result<omega_domain::ProviderTemplate> {
...
#[cfg(test)]
mod tests {
    use omega_app::domain::{AuthMethod, ProviderResponse};
        ...
        match config.models.as_ref().expect("models should be present") {
            Models::Hardcoded(models) => {
                ...
                assert!(
                    model
                        .input_modalities
                        .contains(&omega_app::domain::InputModality::Image),
    ...
    fn test_provider_entry_with_static_models_converts_to_hardcoded() {
        let model = omega_domain::Model::new("Qwen3.6-35B-A3b-q3-mlx")
            ...
            .input_modalities(vec![omega_domain::InputModality::Text]);

        let entry = omega_config::ProviderEntry {
            id: "ollama".to_string(),
            url: "http://127.0.0.1:8000/v1/chat/completions".to_string(),
            response_type: Some(omega_config::ProviderResponseType::OpenAI),
            auth_methods: vec![omega_config::ProviderAuthMethod::ApiKey],
            models: Some(omega_config::ModelListConfig::Hardcoded(vec![
                model.clone(),
            ])),
            ..Default::default()
        };
        ...
        let expected = ProviderConfig {
            ...
            provider_type: omega_domain::ProviderType::Llm,
            api_key_vars: None,
            url_param_vars: vec![],
            response_type: Some(omega_app::domain::ProviderResponse::OpenAI),
            url: "http://127.0.0.1:8000/v1/chat/completions".to_string(),
            models: Some(Models::Hardcoded(vec![model])),
            auth_methods: vec![omega_domain::AuthMethod::ApiKey],
            custom_headers: None,
        };
    ...
    fn test_provider_entry_with_url_models_converts_to_url() {
        let entry = omega_config::ProviderEntry {
            ...
            models: Some(omega_config::ModelListConfig::Url(
...
mod env_tests {
    ...
    use omega_app::domain::{
        ChatCompletionMessage, Context, Environment, Model, ModelId, ResultStream,
    };
    use omega_domain::{AnyProvider, ChatRepository, ProviderTemplate};
    use pretty_assertions::assert_eq;
    use url::Url;
    ...
    impl EnvironmentInfra for MockInfra {
        ...
        async fn update_environment(
            &self,
            _ops: Vec<omega_domain::ConfigOperation>,
        ) -> anyhow::Result<()> {
            Ok(())
        }

        fn get_config(&self) -> anyhow::Result<omega_config::OmegaConfig> {
            Ok(omega_config::OmegaConfig::default())
        }
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, omega_domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl HttpInfra for MockInfra {
        ...
        async fn http_eventsource(
            ...
            _headers: Option<reqwest::header::HeaderMap>,
            _body: bytes::Bytes,
        ) -> anyhow::Result<omega_eventsource::EventSource> {
            Err(anyhow::anyhow!("HTTP not implemented in mock"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn upsert_credential(
            &self,
            _credential: omega_domain::AuthCredential,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn get_credential(
            &self,
            _id: &ProviderId,
        ) -> anyhow::Result<Option<omega_domain::AuthCredential>> {
            Ok(None)
        }
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<omega_domain::MigrationResult>> {
            Ok(None)
        }
    }
    ...
    async fn test_migration_from_env_to_file() {
        ...
        let registry = OmegaProviderRepository::new(infra.clone());
    ...
    #[tokio::test]
    async fn test_migration_should_not_create_omega_services_credential() {
        let mut env_vars = HashMap::new();
        ...
        // Verify omega_services was NOT created during migration
        assert!(
            !credentials
                .iter()
                .any(|c| c.id == ProviderId::OMEGA_SERVICES),
            "Should NOT create omega_services credential during environment migration"
        );
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        let infra = Arc::new(MockInfra::new(env_vars));
        let registry = OmegaProviderRepository::new(infra);
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    omega_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            omega_domain::ModelSource::Url(model_template) => {
                assert_eq!(
                    model_template.template,
                    "https://{{AZURE_RESOURCE_NAME}}.openai.azure.com/openai/models?api-version={{AZURE_API_VERSION}}"
                );
            }
            omega_domain::ModelSource::Hardcoded(_) => panic!("Expected ModelSource::Url variant"),
        }
    }
