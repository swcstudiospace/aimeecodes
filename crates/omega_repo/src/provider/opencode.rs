...
use omega_domain::ChatRepository;
use url::Url;
...
impl<F: HttpInfra + EnvironmentInfra<Config = omega_config::OmegaConfig> + Sync>
    OpenCodeZenResponseRepository<F>
{
    ...
    pub async fn models(&self, provider: Provider<Url>) -> Result<Vec<Model>> {
        ...
        if let Some(models) = provider.models() {
            match models {
                omega_domain::ModelSource::Hardcoded(models) => Ok(models.clone()),
                omega_domain::ModelSource::Url(_) => {
