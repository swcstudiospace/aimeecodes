use anyhow::Context;
use omega_app::domain::ChatCompletionMessage;
use omega_app::dto::openai::Error;
use omega_eventsource::{Event, EventSource};
use reqwest::Url;
...
{
    source
            .take_while(|message| !matches!(message, Err(omega_eventsource::Error::StreamEnded)))
            .then(|event| async {
                match event {
                    ...
                    Err(error) => match error {
                        omega_eventsource::Error::StreamEnded => None,
                        ...
                        omega_eventsource::Error::InvalidContentType(_, ref response) => {
