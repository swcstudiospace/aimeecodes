...
impl BedrockProvider {
    ...
    pub fn new(provider: Provider<Url>) -> Result<Self> {
        ...
        let region_param: omega_domain::URLParam = "AWS_REGION".to_string().into();
    ...
    pub async fn chat(
        ...
        let output = self
            ...
            .map_err(|sdk_error| {
                ...
                if is_retryable {
                    omega_domain::Error::Retryable(anyhow::anyhow!("{}", source)).into()
                } else {
            ...
            match event_stream.recv().await {
                ...
                Err(stream_error) => {
                    ...
                    let error = if is_retryable {
                        omega_domain::Error::Retryable(anyhow::anyhow!(
    ...
    pub async fn models(&self) -> Result<Vec<Model>> {
        ...
        match &self.provider.models {
            Some(omega_domain::ModelSource::Hardcoded(models)) => Ok(models.clone()),
            _ => Ok(vec![]),
        }
    }
}
...
impl IntoDomain for aws_sdk_bedrockruntime::types::ConverseStreamOutput {
    type Domain = omega_domain::ChatCompletionMessage;
        ...
        match self {
                            ...
                            match reasoning {
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Text(
                                    ...
                                    ChatCompletionMessage::default()
                                        ...
                                        .add_reasoning_detail(omega_domain::Reasoning::Part(vec![
                                            omega_domain::ReasoningPart {
                                                text: Some(text),
                                                signature: None,
                                                ..Default::default()
                                            },
                                ...
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Signature(
                                    ...
                                    ChatCompletionMessage::default().add_reasoning_detail(
                                        omega_domain::Reasoning::Part(vec![
                                            omega_domain::ReasoningPart {
                                                text: None,
                                                signature: Some(sig),
                                                ..Default::default()
            ...
            ConverseStreamOutput::Metadata(metadata) => {
                ...
                let usage = metadata.usage.map(|u| {
                    ...
                    omega_domain::Usage {
                        prompt_tokens: omega_domain::TokenCount::Actual(u.input_tokens as usize),
                        completion_tokens: omega_domain::TokenCount::Actual(
                            u.output_tokens as usize,
                        ),
                        total_tokens: omega_domain::TokenCount::Actual(u.total_tokens as usize),
                        cached_tokens: omega_domain::TokenCount::Actual(cached_tokens as usize),
                        ..Default::default()
                    }
                });
...
impl FromDomain<omega_domain::Context>
    for aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput
{
    fn from_domain(context: omega_domain::Context) -> anyhow::Result<Self> {
        ...
        let system: Vec<SystemContentBlock> = context
            ...
            .filter_map(|msg| match &msg.message {
                omega_domain::ContextMessage::Text(text_msg)
                    if text_msg.has_role(omega_domain::Role::System) =>
                {
        ...
        let messages: Vec<Message> = {
            ...
            let mut pending_tool_results: Vec<omega_domain::ContextMessage> = Vec::new();

            for message in context.messages.into_iter() {
                if message.has_role(omega_domain::Role::System) {
                    continue;
                }

                match &message.message {
                    omega_domain::ContextMessage::Tool(_) => {
        ...
        let tool_config = if !context.tools.is_empty() {
            ...
            let choice = context
                .tool_choice
                .filter(|c| !matches!(c, omega_domain::ToolChoice::None))
        ...
        let adjusted_top_p = if emits_legacy_thinking {
            ...
            context.top_p.map(|p| {
                ...
                if value < 0.95 {
                    ...
                    omega_domain::TopP::new(0.95).expect("0.95 is valid TopP")
                } else {
                    p
                }
            })
            ...
            if !reasoning_on {
                ...
                if let Some(effort) = reasoning_config.effort.as_ref() {
                    let effort_str = match effort {
                        omega_domain::Effort::None => None,
                        omega_domain::Effort::Minimal | omega_domain::Effort::Low => Some("low"),
                        omega_domain::Effort::Medium => Some("medium"),
                        omega_domain::Effort::High => Some("high"),
                        omega_domain::Effort::XHigh => Some("xhigh"),
                        omega_domain::Effort::Max => Some("max"),
                    };
...
impl FromDomain<Vec<omega_domain::ContextMessage>> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(tool_results: Vec<omega_domain::ContextMessage>) -> anyhow::Result<Self> {
        ...
        for msg in tool_results {
            match msg {
                omega_domain::ContextMessage::Tool(tool_result) => {
...
impl FromDomain<omega_domain::ContextMessage> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(msg: omega_domain::ContextMessage) -> anyhow::Result<Self> {
        ...
        match msg {
            omega_domain::ContextMessage::Text(text_msg) => {
                ...
                if text_msg.role == omega_domain::Role::Assistant
                ...
                let role = match text_msg.role {
                    omega_domain::Role::User => ConversationRole::User,
                    omega_domain::Role::Assistant => ConversationRole::Assistant,
                    omega_domain::Role::System => {
            ...
            omega_domain::ContextMessage::Image(img) => {
...
impl FromDomain<omega_domain::ToolCallArguments> for aws_smithy_types::Document {
    fn from_domain(args: omega_domain::ToolCallArguments) -> anyhow::Result<Self> {
        use anyhow::Context as _;
...
impl FromDomain<omega_domain::ToolDefinition> for aws_sdk_bedrockruntime::types::Tool {
    fn from_domain(tool: omega_domain::ToolDefinition) -> anyhow::Result<Self> {
...
impl FromDomain<omega_domain::ToolChoice> for aws_sdk_bedrockruntime::types::ToolChoice {
    fn from_domain(choice: omega_domain::ToolChoice) -> anyhow::Result<Self> {
        use aws_sdk_bedrockruntime::types::{
            AnyToolChoice, AutoToolChoice, SpecificToolChoice, ToolChoice,
        };

        let bedrock_choice = match choice {
            omega_domain::ToolChoice::Auto => ToolChoice::Auto(AutoToolChoice::builder().build()),
            omega_domain::ToolChoice::Required => ToolChoice::Any(AnyToolChoice::builder().build()),
            omega_domain::ToolChoice::Call(tool_name) => ToolChoice::Tool(
                SpecificToolChoice::builder()
                    .name(tool_name.to_string())
                    .build()
                    .map_err(|e| anyhow::anyhow!("Failed to build tool choice: {}", e))?,
            ),
            omega_domain::ToolChoice::None => {
...
#[cfg(test)]
mod tests {
    use omega_domain::InputModality;
    ...
    async fn test_models_returns_hardcoded() {
        use omega_domain::{Model, ModelId, ModelSource};
    ...
    fn test_into_domain_content_block_delta_text() {
        ...
        use omega_domain::{ChatCompletionMessage, Content};
    ...
    fn test_into_domain_content_block_start_tool_use() {
        ...
        use omega_domain::{ChatCompletionMessage, Content, ToolCallId, ToolCallPart, ToolName};
    ...
    fn test_into_domain_message_stop_end_turn() {
        ...
        use omega_domain::{ChatCompletionMessage, Content, FinishReason};
    ...
    fn test_into_domain_metadata_with_usage() {
        use aws_sdk_bedrockruntime::types::ConverseStreamOutput;
        use omega_domain::{ChatCompletionMessage, Content, TokenCount};
        ...
        let actual = fixture.into_domain();
        let expected =
            ChatCompletionMessage::assistant(Content::part("")).usage(omega_domain::Usage {
    ...
    fn test_from_domain_tool_choice_auto() {
        ...
        let fixture = omega_domain::ToolChoice::Auto;
    ...
    fn test_from_domain_tool_choice_required() {
        ...
        let fixture = omega_domain::ToolChoice::Required;
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_choice_call() {
        ...
        let fixture = omega_domain::ToolChoice::Call(omega_domain::ToolName::new("my_tool"));
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_definition() {
        ...
        use omega_domain::ToolDefinition;
        ...
        let fixture = ToolDefinition {
            name: omega_domain::ToolName::new("test_tool"),
    ...
    fn test_from_domain_context_message_text_user() {
        ...
        use omega_domain::{ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant() {
        use aws_sdk_bedrockruntime::types::{ContentBlock, ConversationRole, Message};
        use omega_domain::{ContextMessage, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant_reasoning_uses_message_signature() {
        ...
        use omega_domain::{ContextMessage, ReasoningFull, TextMessage};
        use pretty_assertions::assert_eq;
    ...
    fn test_from_domain_context_message_tool_result() {
        ...
        use omega_domain::{ContextMessage, ToolCallId, ToolResult};
    ...
    fn test_from_domain_context_with_system_messages() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use omega_domain::{Context, ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_with_temperature() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use omega_domain::{Context, Temperature};
    ...
    fn test_from_domain_context_with_reasoning_adjusts_top_p() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use omega_domain::{Context, ReasoningConfig, TopP};
    ...
    fn test_from_domain_context_with_reasoning_enabled() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use omega_domain::{Context, ReasoningConfig};
    ...
    fn test_from_domain_context_emits_output_config_effort() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use omega_domain::{Context, Effort, ReasoningConfig};
    ...
    /// call Bedrock. Run with: cargo test -p omega_repo
