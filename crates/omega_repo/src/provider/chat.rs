...
use omega_infra::CacacheStorage;
...
pub struct OmegaChatRepository<F> {
    router: Arc<ProviderRouter<F>>,
    model_cache: Arc<CacacheStorage>,
    bg_refresh: BgRefresh,
}

impl<F: EnvironmentInfra<Config = omega_config::OmegaConfig> + HttpInfra> OmegaChatRepository<F> {
    /// Creates a new OmegaChatRepository with the given infrastructure.
    ///
    /// # Arguments
    ///
...
impl<F: EnvironmentInfra<Config = omega_config::OmegaConfig> + HttpInfra + Sync> ChatRepository
    for OmegaChatRepository<F>
{
    async fn chat(
        &self,
        model_id: &ModelId,
...
impl<F: HttpInfra + EnvironmentInfra<Config = omega_config::OmegaConfig> + Sync> ProviderRouter<F> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
