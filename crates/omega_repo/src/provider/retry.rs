use omega_app::domain::Error as DomainError;
use omega_app::dto::anthropic::Error as AnthropicError;
use omega_app::dto::openai::{Error, ErrorCode, ErrorResponse};
...
fn get_event_req_status_code(error: &anyhow::Error) -> Option<u16> {
    error
        .downcast_ref::<omega_eventsource::Error>()
        .and_then(|error| match error {
            omega_eventsource::Error::InvalidStatusCode(_, response) => {
                Some(response.status().as_u16())
            }
            omega_eventsource::Error::InvalidContentType(_, response) => {
                Some(response.status().as_u16())
            }
            _ => None,
        })
...
fn is_event_transport_error(error: &anyhow::Error) -> bool {
    error
        .downcast_ref::<omega_eventsource::Error>()
        .is_some_and(|e| matches!(e, omega_eventsource::Error::Transport(_)))
}
