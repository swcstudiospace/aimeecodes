...
use omega_app::{AgentRepository, DirectoryReaderInfra, EnvironmentInfra, FileInfoInfra};
use omega_config::OmegaConfig;
use omega_domain::{ModelId, ProviderId, Template, ToolName};
...
/// 2. Global custom agents (from ~/.omega/agents/ directory)
/// 3. Project-local agents (from .omega/agents/ directory in current working
///    directory)
///
/// ## Agent Precedence
...
/// - **Global agents**: `~/omega/agents/*.md`
/// - **CWD agents**: `./.omega/agents/*.md` (relative to current working
///   directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct OmegaAgentRepository<I> {
    infra: Arc<I>,
}

impl<I> OmegaAgentRepository<I> {
    pub fn new(infra: Arc<I>) -> Self {
        Self { infra }
    }
}

impl<I: FileInfoInfra + EnvironmentInfra<Config = OmegaConfig> + DirectoryReaderInfra>
    OmegaAgentRepository<I>
{
    ...
    async fn init_default(&self) -> anyhow::Result<Vec<AgentDefinition>> {
        ...
        parse_agent_iter(
            [
                ("omega", include_str!("agents/omega.md")),
...
impl<F: FileInfoInfra + EnvironmentInfra<Config = OmegaConfig> + DirectoryReaderInfra>
    AgentRepository for OmegaAgentRepository<F>
{
    async fn get_agents(&self) -> anyhow::Result<Vec<omega_domain::Agent>> {
        ...
        let session = config
            .session
            .clone()
            .ok_or(omega_domain::Error::NoDefaultSession)?;
    ...
    async fn get_agent_infos(&self) -> anyhow::Result<Vec<omega_domain::AgentInfo>> {
        ...
        Ok(agent_defs
            .into_iter()
            .map(|def| omega_domain::AgentInfo {
                id: def.id,
                title: def.title,
...
mod tests {
    ...
    async fn test_parse_basic_agent() {
        let content = omega_test_kit::fixture!("/src/fixtures/agents/basic.md").await;
    ...
    async fn test_parse_advanced_agent() {
        let content = omega_test_kit::fixture!("/src/fixtures/agents/advanced.md").await;
...
"#;
        let config = OmegaConfig { subagents: true, ..Default::default() };

        let actual =
            apply_subagent_tool_config(parse_agent_file(fixture).unwrap(), &config).unwrap();

        assert_eq!(actual.id, AgentId::new("omega"));
        assert_eq!(
...
Body keeps {{tool_names.read}} untouched.
"#;
        let config = OmegaConfig { subagents: false, ..Default::default() };
...
"#;
        ...
        assert_yaml_snapshot!(
            ...
            apply_subagent_tool_config(
                actual,
                &OmegaConfig { subagents: true, ..Default::default() }
            )
            .unwrap()
            .tools
        );
