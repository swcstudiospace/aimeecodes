...
//! `omega_domain` counterparts for compile-time safety while keeping the
//! storage layer independent from domain model changes.

use anyhow::Context as _;
use omega_domain::{Context, ConversationId};
...
impl From<&omega_domain::ModelId> for ModelIdRecord {
    fn from(id: &omega_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for omega_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        omega_domain::ModelId::from(record.0)
    }
}
...
impl From<&omega_domain::Image> for ImageRecord {
    fn from(image: &omega_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for omega_domain::Image {
    fn from(record: ImageRecord) -> Self {
        omega_domain::Image::new_base64(
            record
                .url
...
impl From<&omega_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &omega_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for omega_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        omega_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&omega_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &omega_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for omega_domain::ToolCallArguments {
...
impl From<&omega_domain::ToolName> for ToolNameRecord {
    fn from(name: &omega_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for omega_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        omega_domain::ToolName::new(record.0)
    }
}
...
impl From<&omega_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &omega_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for omega_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        omega_domain::ToolCallFull {
...
impl From<&omega_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &omega_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for omega_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        omega_domain::ReasoningFull {
            text: record.text,
...
impl From<&omega_domain::TokenCount> for TokenCountRecord {
    fn from(count: &omega_domain::TokenCount) -> Self {
        match count {
            omega_domain::TokenCount::Actual(n) => Self::Actual(*n),
            omega_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for omega_domain::TokenCount {
...
impl From<&omega_domain::Usage> for UsageRecord {
    fn from(usage: &omega_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for omega_domain::Usage {
...
impl From<&omega_domain::EventValue> for EventValueRecord {
    fn from(event: &omega_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for omega_domain::EventValue {
...
impl From<&omega_domain::Role> for RoleRecord {
    fn from(role: &omega_domain::Role) -> Self {
        match role {
            omega_domain::Role::System => Self::System,
            omega_domain::Role::User => Self::User,
            omega_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for omega_domain::Role {
...
impl From<&omega_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &omega_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for omega_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(omega_domain::TextMessage {
...
impl From<&omega_domain::ToolValue> for ToolValueRecord {
    fn from(value: &omega_domain::ToolValue) -> Self {
        match value {
            omega_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            omega_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            omega_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            omega_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for omega_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&omega_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &omega_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for omega_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(omega_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&omega_domain::ToolResult> for ToolResultRecord {
    fn from(result: &omega_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for omega_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(omega_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&omega_domain::ContextMessage> for ContextMessageValueRecord {
    fn from(value: &omega_domain::ContextMessage) -> Self {
        match value {
            omega_domain::ContextMessage::Text(msg) => Self::Text(TextMessageRecord::from(msg)),
            omega_domain::ContextMessage::Tool(result) => {
                Self::Tool(ToolResultRecord::from(result))
            }
            omega_domain::ContextMessage::Image(img) => Self::Image(ImageRecord::from(img)),
        }
    }
}

impl TryFrom<ContextMessageValueRecord> for omega_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&omega_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &omega_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for omega_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(omega_domain::MessageEntry {
...
impl From<&omega_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &omega_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for omega_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(omega_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&omega_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &omega_domain::ToolChoice) -> Self {
        match choice {
            omega_domain::ToolChoice::None => Self::None,
            omega_domain::ToolChoice::Auto => Self::Auto,
            omega_domain::ToolChoice::Required => Self::Required,
            omega_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for omega_domain::ToolChoice {
...
impl From<&omega_domain::Effort> for EffortRecord {
    fn from(effort: &omega_domain::Effort) -> Self {
        match effort {
            omega_domain::Effort::None => Self::None,
            omega_domain::Effort::Minimal => Self::Minimal,
            omega_domain::Effort::Low => Self::Low,
            omega_domain::Effort::Medium => Self::Medium,
            omega_domain::Effort::High => Self::High,
            omega_domain::Effort::XHigh => Self::XHigh,
            omega_domain::Effort::Max => Self::Max,
        }
    }
}

impl From<EffortRecord> for omega_domain::Effort {
...
impl From<&omega_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &omega_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for omega_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        omega_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(omega_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(omega_domain::TopP::new_unchecked),
            top_k: record.top_k.map(omega_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<omega_domain::ToolKind>,
}

impl From<&omega_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &omega_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for omega_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(omega_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&omega_domain::Metrics> for MetricsRecord {
    fn from(metrics: &omega_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for omega_domain::Metrics {
    fn from(record: MetricsRecord) -> Self {
        let file_operations: std::collections::HashMap<String, omega_domain::FileOperation> =
            record
                .files_changed
                .into_iter()
        ...
        let files_accessed = if record.files_accessed.is_empty() {
            ...
            file_operations
                .iter()
                .filter(|(_, op)| op.tool == omega_domain::ToolKind::Read)
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: omega_domain::Conversation,
        workspace_id: omega_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for omega_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(omega_domain::Metrics::from)
            .unwrap_or_else(|| {
                omega_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(omega_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                omega_domain::MetaData::new(record.created_at.and_utc())
