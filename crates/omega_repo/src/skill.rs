...
use anyhow::Context;
use omega_app::domain::Skill;
use omega_app::{EnvironmentInfra, FileInfoInfra, FileReaderInfra, Walker, WalkerInfra};
...
/// 2. Global custom skills (from ~/omega/skills/ directory)
/// 3. Agents skills (from ~/.agents/skills/ directory)
/// 4. Project-local skills (from .omega/skills/ directory in current working
///    directory)
///
/// ## Skill Precedence
...
/// - **Global skills**: `~/omega/skills/<skill-name>/SKILL.md`
/// - **Agents skills**: `~/.agents/skills/<skill-name>/SKILL.md`
/// - **CWD skills**: `./.omega/skills/<skill-name>/SKILL.md` (relative to
///   current working directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct OmegaSkillRepository<I> {
    infra: Arc<I>,
}

impl<I> OmegaSkillRepository<I> {
    ...
    fn load_builtin_skills(&self) -> Vec<Skill> {
        let builtin_skills = vec![
            (
                "omega://skills/create-skill/SKILL.md",
                include_str!("skills/create-skill/SKILL.md"),
            ),
            (
                "omega://skills/execute-plan/SKILL.md",
                include_str!("skills/execute-plan/SKILL.md"),
            ),
            (
                "omega://skills/github-pr-description/SKILL.md",
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> OmegaSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &omega_domain::Environment) -> Skill {
...
mod tests {
    ...
    fn fixture_skill_repo() -> (OmegaSkillRepository<OmegaInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/fixtures/skills_with_resources");
        let config = OmegaConfig::read().unwrap_or_default();
        let infra = Arc::new(OmegaInfra::new(std::env::current_dir().unwrap(), config));
        let repo = OmegaSkillRepository::new(infra);
        (repo, skill_dir)
    }
    ...
    fn test_load_builtin_skills() {
        // Fixture
        let repo = OmegaSkillRepository { infra: Arc::new(()) };
        ...
        assert_eq!(
            create_skill.path,
            Some(std::path::Path::new("omega://skills/create-skill/SKILL.md").to_path_buf())
        );
        assert_eq!(
        ...
        assert_eq!(
            execute_plan.path,
            Some(std::path::Path::new("omega://skills/execute-plan/SKILL.md").to_path_buf())
        );
        assert!(
            execute_plan
                .description
        ...
        assert_eq!(
            pr_description.path,
            Some(
                std::path::Path::new("omega://skills/github-pr-description/SKILL.md").to_path_buf()
            )
        );
    ...
    async fn test_extract_skill_with_valid_metadata() {
        ...
        let content =
            omega_test_kit::fixture!("/src/fixtures/skills/with_name_and_description.md").await;
    ...
    async fn test_extract_skill_with_incomplete_metadata() {
        // Fixture
        let content = omega_test_kit::fixture!("/src/fixtures/skills/with_name_only.md").await;
