...
//! `aimee_domain` counterparts for compile-time safety while keeping the
//! storage layer independent from domain model changes.

use aimee_domain::{Context, ConversationId};
...
impl From<&aimee_domain::ModelId> for ModelIdRecord {
    fn from(id: &aimee_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for aimee_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        aimee_domain::ModelId::from(record.0)
    }
}
...
impl From<&aimee_domain::Image> for ImageRecord {
    fn from(image: &aimee_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for aimee_domain::Image {
    fn from(record: ImageRecord) -> Self {
        aimee_domain::Image::new_base64(
            record
                .url
...
impl From<&aimee_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &aimee_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for aimee_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        aimee_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&aimee_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &aimee_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for aimee_domain::ToolCallArguments {
...
impl From<&aimee_domain::ToolName> for ToolNameRecord {
    fn from(name: &aimee_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for aimee_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        aimee_domain::ToolName::new(record.0)
    }
}
...
impl From<&aimee_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &aimee_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for aimee_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        aimee_domain::ToolCallFull {
...
impl From<&aimee_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &aimee_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for aimee_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        aimee_domain::ReasoningFull {
            text: record.text,
...
impl From<&aimee_domain::TokenCount> for TokenCountRecord {
    fn from(count: &aimee_domain::TokenCount) -> Self {
        match count {
            aimee_domain::TokenCount::Actual(n) => Self::Actual(*n),
            aimee_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for aimee_domain::TokenCount {
...
impl From<&aimee_domain::Usage> for UsageRecord {
    fn from(usage: &aimee_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for aimee_domain::Usage {
...
impl From<&aimee_domain::EventValue> for EventValueRecord {
    fn from(event: &aimee_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for aimee_domain::EventValue {
...
impl From<&aimee_domain::Role> for RoleRecord {
    fn from(role: &aimee_domain::Role) -> Self {
        match role {
            aimee_domain::Role::System => Self::System,
            aimee_domain::Role::User => Self::User,
            aimee_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for aimee_domain::Role {
...
impl From<&aimee_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &aimee_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for aimee_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(aimee_domain::TextMessage {
...
impl From<&aimee_domain::ToolValue> for ToolValueRecord {
    fn from(value: &aimee_domain::ToolValue) -> Self {
        match value {
            aimee_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            aimee_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            aimee_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            aimee_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for aimee_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&aimee_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &aimee_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for aimee_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(aimee_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&aimee_domain::ToolResult> for ToolResultRecord {
    fn from(result: &aimee_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for aimee_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(aimee_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&aimee_domain::ContextMessage> for ContextMessageValueRecord {
    fn from(value: &aimee_domain::ContextMessage) -> Self {
        match value {
            aimee_domain::ContextMessage::Text(msg) => Self::Text(TextMessageRecord::from(msg)),
            aimee_domain::ContextMessage::Tool(result) => {
                Self::Tool(ToolResultRecord::from(result))
            }
            aimee_domain::ContextMessage::Image(img) => Self::Image(ImageRecord::from(img)),
        }
    }
}

impl TryFrom<ContextMessageValueRecord> for aimee_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&aimee_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &aimee_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for aimee_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(aimee_domain::MessageEntry {
...
impl From<&aimee_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &aimee_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for aimee_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(aimee_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&aimee_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &aimee_domain::ToolChoice) -> Self {
        match choice {
            aimee_domain::ToolChoice::None => Self::None,
            aimee_domain::ToolChoice::Auto => Self::Auto,
            aimee_domain::ToolChoice::Required => Self::Required,
            aimee_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for aimee_domain::ToolChoice {
...
impl From<&aimee_domain::Effort> for EffortRecord {
    fn from(effort: &aimee_domain::Effort) -> Self {
        match effort {
            aimee_domain::Effort::None => Self::None,
            aimee_domain::Effort::Minimal => Self::Minimal,
            aimee_domain::Effort::Low => Self::Low,
            aimee_domain::Effort::Medium => Self::Medium,
            aimee_domain::Effort::High => Self::High,
            aimee_domain::Effort::XHigh => Self::XHigh,
            aimee_domain::Effort::Max => Self::Max,
        }
    }
}

impl From<EffortRecord> for aimee_domain::Effort {
...
impl From<&aimee_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &aimee_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for aimee_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        aimee_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(aimee_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(aimee_domain::TopP::new_unchecked),
            top_k: record.top_k.map(aimee_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<aimee_domain::ToolKind>,
}

impl From<&aimee_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &aimee_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for aimee_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(aimee_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&aimee_domain::Metrics> for MetricsRecord {
    fn from(metrics: &aimee_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for aimee_domain::Metrics {
    fn from(record: MetricsRecord) -> Self {
        let file_operations: std::collections::HashMap<String, aimee_domain::FileOperation> =
            record
                .files_changed
                .into_iter()
        ...
        let files_accessed = if record.files_accessed.is_empty() {
            ...
            file_operations
                .iter()
                .filter(|(_, op)| op.tool == aimee_domain::ToolKind::Read)
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: aimee_domain::Conversation,
        workspace_id: aimee_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for aimee_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(aimee_domain::Metrics::from)
            .unwrap_or_else(|| {
                aimee_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(aimee_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                aimee_domain::MetaData::new(record.created_at.and_utc())
