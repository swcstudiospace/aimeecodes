...
pub struct AimeeContextEngineRepository<I> {
    infra: Arc<I>,
}

impl<I> AimeeContextEngineRepository<I> {
...
#[async_trait]
impl<I: GrpcInfra> WorkspaceIndexRepository for AimeeContextEngineRepository<I> {
    ...
    async fn search(
        &self,
        search_query: &aimee_domain::CodeSearchQuery<'_>,
        auth_token: &aimee_domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        aimee_domain::NodeData::FileChunk(aimee_domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        aimee_domain::NodeData::File(aimee_domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        aimee_domain::NodeData::FileRef(aimee_domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        aimee_domain::NodeData::Note(aimee_domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        aimee_domain::NodeData::Task(aimee_domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &aimee_domain::ApiKey,
    ) -> Result<Vec<aimee_domain::FileHash>> {
