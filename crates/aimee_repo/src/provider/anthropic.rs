...
use aimee_domain::{ChatRepository, Provider, ProviderId};
    ...
    fn get_headers(&self, model: Option<&ModelId>) -> Vec<(String, String)> {
        ...
        let api_key = self
            ...
            .and_then(|c| match &c.auth_details {
                ...
                aimee_domain::AuthDetails::GoogleAdc(api_key) => Some(api_key.as_str()),
...
impl<F: HttpInfra> AnthropicResponseRepository<F> {
    ...
    fn create_client(&self, provider: Provider<Url>) -> anyhow::Result<Anthropic<F>> {
        ...
        let is_oauth = provider
            .credential
            .as_ref()
            .map(|c| matches!(c.auth_details, aimee_domain::AuthDetails::OAuth { .. }))
            .unwrap_or(false);
...
mod tests {
    ...
    fn create_anthropic(base_url: &str) -> anyhow::Result<Anthropic<MockHttpClient>> {
        ...
        let provider = Provider {
            id: aimee_app::domain::ProviderId::ANTHROPIC,
            provider_type: aimee_domain::ProviderType::Llm,
            response: Some(aimee_app::domain::ProviderResponse::Anthropic),
            url: chat_url,
            ...
            url_params: vec![],
            models: Some(aimee_domain::ModelSource::Url(model_url)),
            custom_headers: None,
        };
    ...
    async fn test_url_for_models() {
        ...
        let provider = Provider {
            ...
            auth_methods: vec![aimee_domain::AuthMethod::ApiKey],
            url_params: vec![],
            models: Some(aimee_domain::ModelSource::Url(model_url.clone())),
            custom_headers: None,
        };
    ...
    fn test_vertex_ai_removes_output_format() {
        use aimee_domain::ResponseFormat;
        ...
        let provider = Provider {
            ...
            credential: Some(aimee_domain::AuthCredential {
                id: ProviderId::VERTEX_AI_ANTHROPIC,
                auth_details: aimee_domain::AuthDetails::GoogleAdc(aimee_domain::ApiKey::from(
                    "test-token".to_string(),
                )),
                url_params: std::collections::HashMap::new(),
            }),
            auth_methods: vec![aimee_domain::AuthMethod::GoogleAdc],
            url_params: vec![],
            models: Some(aimee_domain::ModelSource::Hardcoded(vec![])),
            custom_headers: None,
        };
