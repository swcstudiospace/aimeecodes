use aimee_app::domain::ChatCompletionMessage;
use aimee_app::dto::openai::Error;
use aimee_eventsource::{Event, EventSource};
use anyhow::Context;
use reqwest::Url;
...
{
    source
            .take_while(|message| !matches!(message, Err(aimee_eventsource::Error::StreamEnded)))
            .then(|event| async {
                match event {
                    ...
                    Err(error) => match error {
                        aimee_eventsource::Error::StreamEnded => None,
                        ...
                        aimee_eventsource::Error::InvalidContentType(_, ref response) => {
