...
use aimee_app::domain::{ProviderId, ProviderResponse};
use aimee_app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra, HttpInfra};
use aimee_domain::{
...
enum Models {
    ...
    Hardcoded(Vec<aimee_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<aimee_domain::AuthMethod>,
    #[serde(default)]
    #[merge(strategy = overwrite)]
...
fn legacy_env_var_fallback(new_name: &str) -> Option<&'static str> {
    match new_name {
        ...
        "AIMEE_WORKSPACE_SERVER_URL" => Some("OMEGA_WORKSPACE_SERVER_URL"),
        _ => None,
    }
}
...
impl From<aimee_config::ProviderUrlParam> for UrlParamVarConfig {
    fn from(param: aimee_config::ProviderUrlParam) -> Self {
...
impl From<aimee_config::ProviderEntry> for ProviderConfig {
    fn from(entry: aimee_config::ProviderEntry) -> Self {
        let provider_type = match entry.provider_type {
            Some(aimee_config::ProviderTypeEntry::ContextEngine) => {
                aimee_domain::ProviderType::ContextEngine
            }
            Some(aimee_config::ProviderTypeEntry::Llm) | None => aimee_domain::ProviderType::Llm,
        };

        let auth_methods = if entry.auth_methods.is_empty() {
            vec![aimee_domain::AuthMethod::ApiKey]
        } else {
            entry
                .auth_methods
                .into_iter()
                .map(|m| match m {
                    aimee_config::ProviderAuthMethod::ApiKey => aimee_domain::AuthMethod::ApiKey,
                    aimee_config::ProviderAuthMethod::GoogleAdc => {
                        aimee_domain::AuthMethod::GoogleAdc
                    }
                })
                .collect()
        };
        ...
        let response_type = entry.response_type.map(|r| match r {
            aimee_config::ProviderResponseType::OpenAI => ProviderResponse::OpenAI,
            aimee_config::ProviderResponseType::OpenAIResponses => {
                ProviderResponse::OpenAIResponses
            }
            aimee_config::ProviderResponseType::Anthropic => ProviderResponse::Anthropic,
            aimee_config::ProviderResponseType::Bedrock => ProviderResponse::Bedrock,
            aimee_config::ProviderResponseType::Google => ProviderResponse::Google,
            aimee_config::ProviderResponseType::OpenCode => ProviderResponse::OpenCode,
        });

        let models = entry.models.map(|m| match m {
            aimee_config::ModelListConfig::Url(url) => Models::Url(url),
            aimee_config::ModelListConfig::Hardcoded(model_list) => Models::Hardcoded(model_list),
        });
...
impl From<&ProviderConfig> for aimee_domain::ProviderTemplate {
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => aimee_domain::ModelSource::Url(
                aimee_domain::Template::<aimee_domain::URLParameters>::new(model_url_template),
            ),
            Models::Hardcoded(model_list) => {
                aimee_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: aimee_domain::Template::<aimee_domain::URLParameters>::new(&config.url),
...
pub struct AimeeProviderRepository<F> {
    infra: Arc<F>,
}

impl<F: EnvironmentInfra<Config = aimee_config::AimeeConfig> + HttpInfra>
    AimeeProviderRepository<F>
{
...
> AimeeProviderRepository<F>
{
    ...
    /// Converts provider entries from `AimeeConfig` into `ProviderConfig`
    ...
    async fn get_providers(&self) -> Vec<AnyProvider> {
        ...
        for config in configs {
            // Skip Aimee provider as it's handled specially
            if config.id == ProviderId::AIMEE {
                continue;
            }
    ...
    pub async fn migrate_env_to_file(&self) -> anyhow::Result<Option<MigrationResult>> {
        ...
        for config in configs {
            // Skip Aimee provider and ContextEngine providers - they're not configurable
            // via env like other providers
            if config.id == ProviderId::AIMEE || config.provider_type == ProviderType::ContextEngine
            {
                continue;
            }
    ...
    fn create_credential_from_env(
        ...
        let api_key = if let Some(api_key_var) = &config.api_key_vars {
            self.infra
                ...
                .or_else(|| {
                    if api_key_var == "AIMEE_API_KEY" {
                        self.infra.get_env_var("OMEGA_API_KEY")
                    } else {
                        None
                    }
                })
    ...
    async fn create_provider(
        &self,
        config: &ProviderConfig,
    ) -> anyhow::Result<aimee_domain::ProviderTemplate> {
        ...
        if (credential.id == aimee_domain::ProviderId::VERTEX_AI
            || credential.id == aimee_domain::ProviderId::VERTEX_AI_ANTHROPIC)
            && let aimee_domain::AuthDetails::ApiKey(ref api_key) = credential.auth_details
    ...
    async fn refresh_google_adc_credential(
        &self,
        original_credential: &aimee_domain::AuthCredential,
    ) -> anyhow::Result<aimee_domain::AuthCredential> {
        ...
        Ok(aimee_domain::AuthCredential::new_api_key(
            original_credential.id.clone(),
            aimee_domain::ApiKey::from(access_token.token),
        )
    ...
    async fn provider_from_id(
        ...
        if id == ProviderId::AIMEE {
            // Aimee provider isn't typically configured via env vars in the registry
            return Err(Error::provider_not_available(ProviderId::AIMEE).into());
        }
    ...
    async fn get_merged_configs(&self) -> Vec<ProviderConfig> {
        ...
        // Merge inline configs from AimeeConfig (aimee.toml `providers` field)
...
impl<
    F: EnvironmentInfra<Config = aimee_config::AimeeConfig>
        ...
        + HttpInfra
        + Sync,
> ProviderRepository for AimeeProviderRepository<F>
{
...
#[cfg(test)]
mod tests {
    use aimee_app::domain::{AuthMethod, ProviderResponse};
        ...
        match config.models.as_ref().expect("models should be present") {
            Models::Hardcoded(models) => {
                ...
                assert!(
                    model
                        .input_modalities
                        .contains(&aimee_app::domain::InputModality::Image),
    ...
    fn test_provider_entry_with_static_models_converts_to_hardcoded() {
        let model = aimee_domain::Model::new("Qwen3.6-35B-A3b-q3-mlx")
            ...
            .input_modalities(vec![aimee_domain::InputModality::Text]);

        let entry = aimee_config::ProviderEntry {
            id: "ollama".to_string(),
            url: "http://127.0.0.1:8000/v1/chat/completions".to_string(),
            response_type: Some(aimee_config::ProviderResponseType::OpenAI),
            auth_methods: vec![aimee_config::ProviderAuthMethod::ApiKey],
            models: Some(aimee_config::ModelListConfig::Hardcoded(vec![
                model.clone(),
            ])),
            ..Default::default()
        };
        ...
        let expected = ProviderConfig {
            ...
            provider_type: aimee_domain::ProviderType::Llm,
            api_key_vars: None,
            url_param_vars: vec![],
            response_type: Some(aimee_app::domain::ProviderResponse::OpenAI),
            url: "http://127.0.0.1:8000/v1/chat/completions".to_string(),
            models: Some(Models::Hardcoded(vec![model])),
            auth_methods: vec![aimee_domain::AuthMethod::ApiKey],
            custom_headers: None,
        };
    ...
    fn test_provider_entry_with_url_models_converts_to_url() {
        let entry = aimee_config::ProviderEntry {
            ...
            models: Some(aimee_config::ModelListConfig::Url(
...
mod env_tests {
    ...
    use aimee_app::domain::{
        ChatCompletionMessage, Context, Environment, Model, ModelId, ResultStream,
    };
    use aimee_domain::{AnyProvider, ChatRepository, ProviderTemplate};
    use pretty_assertions::assert_eq;
    use url::Url;
    ...
    impl EnvironmentInfra for MockInfra {
        ...
        async fn update_environment(
            &self,
            _ops: Vec<aimee_domain::ConfigOperation>,
        ) -> anyhow::Result<()> {
            Ok(())
        }

        fn get_config(&self) -> anyhow::Result<aimee_config::AimeeConfig> {
            Ok(aimee_config::AimeeConfig::default())
        }
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, aimee_domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl HttpInfra for MockInfra {
        ...
        async fn http_eventsource(
            ...
            _headers: Option<reqwest::header::HeaderMap>,
            _body: bytes::Bytes,
        ) -> anyhow::Result<aimee_eventsource::EventSource> {
            Err(anyhow::anyhow!("HTTP not implemented in mock"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn upsert_credential(
            &self,
            _credential: aimee_domain::AuthCredential,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn get_credential(
            &self,
            _id: &ProviderId,
        ) -> anyhow::Result<Option<aimee_domain::AuthCredential>> {
            Ok(None)
        }
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<aimee_domain::MigrationResult>> {
            Ok(None)
        }
    }
    ...
    async fn test_migration_from_env_to_file() {
        ...
        let registry = AimeeProviderRepository::new(infra.clone());
    ...
    #[tokio::test]
    async fn test_migration_should_not_create_aimee_services_credential() {
        let mut env_vars = HashMap::new();
        ...
        // Verify aimee_services was NOT created during migration
        assert!(
            !credentials
                .iter()
                .any(|c| c.id == ProviderId::AIMEE_SERVICES),
            "Should NOT create aimee_services credential during environment migration"
        );
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        let infra = Arc::new(MockInfra::new(env_vars));
        let registry = AimeeProviderRepository::new(infra);
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    aimee_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            aimee_domain::ModelSource::Url(model_template) => {
                assert_eq!(
                    model_template.template,
                    "https://{{AZURE_RESOURCE_NAME}}.openai.azure.com/openai/models?api-version={{AZURE_API_VERSION}}"
                );
            }
            aimee_domain::ModelSource::Hardcoded(_) => panic!("Expected ModelSource::Url variant"),
        }
    }
