...
use aimee_infra::CacacheStorage;
...
pub struct AimeeChatRepository<F> {
    router: Arc<ProviderRouter<F>>,
    model_cache: Arc<CacacheStorage>,
    bg_refresh: BgRefresh,
}

impl<F: EnvironmentInfra<Config = aimee_config::AimeeConfig> + HttpInfra> AimeeChatRepository<F> {
    /// Creates a new AimeeChatRepository with the given infrastructure.
    ///
    /// # Arguments
    ///
...
impl<F: EnvironmentInfra<Config = aimee_config::AimeeConfig> + HttpInfra + Sync> ChatRepository
    for AimeeChatRepository<F>
{
    async fn chat(
        &self,
        model_id: &ModelId,
...
impl<F: HttpInfra + EnvironmentInfra<Config = aimee_config::AimeeConfig> + Sync> ProviderRouter<F> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
