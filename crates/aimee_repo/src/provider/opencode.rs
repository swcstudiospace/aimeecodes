...
use aimee_domain::ChatRepository;
use anyhow::Result;
use url::Url;
...
impl<F: HttpInfra + EnvironmentInfra<Config = aimee_config::AimeeConfig> + Sync>
    OpenCodeZenResponseRepository<F>
{
    ...
    pub async fn models(&self, provider: Provider<Url>) -> Result<Vec<Model>> {
        ...
        if let Some(models) = provider.models() {
            match models {
                aimee_domain::ModelSource::Hardcoded(models) => Ok(models.clone()),
                aimee_domain::ModelSource::Url(_) => {
