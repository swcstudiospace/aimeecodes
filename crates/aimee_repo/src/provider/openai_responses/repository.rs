...
use aimee_app::{EnvironmentInfra, HttpInfra};
use aimee_domain::{BoxStream, ChatRepository, Provider};
use aimee_eventsource_stream::Eventsource;
use aimee_infra::sanitize_headers;
    ...
    fn get_headers_for_conversation(&self, conversation_id: Option<&str>) -> Vec<(String, String)> {
        ...
        if let Some(api_key) =
            self.provider
                ...
                .and_then(|c| match &c.auth_details {
                    aimee_domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                    aimee_domain::AuthDetails::OAuthWithApiKey { api_key, .. } => {
                        Some(api_key.as_str())
                    }
                    aimee_domain::AuthDetails::OAuth { tokens, .. } => {
                        Some(tokens.access_token.as_str())
                    }
                    aimee_domain::AuthDetails::GoogleAdc(token) => Some(token.as_str()),
                    aimee_domain::AuthDetails::AwsProfile(_) => None,
                })
        {
        ...
        self.provider
            ...
            .for_each(|method| match method {
                aimee_domain::AuthMethod::ApiKey => {}
                aimee_domain::AuthMethod::OAuthDevice(oauth_config) => {
                ...
                aimee_domain::AuthMethod::OAuthCode(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
                ...
                aimee_domain::AuthMethod::CodexDevice(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
                ...
                aimee_domain::AuthMethod::AwsProfile => {}
            });
        ...
        if self.provider.id == aimee_domain::ProviderId::CODEX {
            ...
            if let Some(account_id) = self.provider.credential.as_ref().and_then(|c| {
                let key: aimee_domain::URLParam = "chatgpt_account_id".to_string().into();
...
impl<T: HttpInfra> OpenAIResponsesProvider<T> {
    pub async fn chat(
        ...
        use aimee_eventsource::Event;
        let url = self.responses_url.clone();
        let event_stream = source
            .take_while(|message| {
                let should_continue =
                    !matches!(message, Err(aimee_eventsource::Error::StreamEnded));
            ...
            .filter_map(move |event_result| {
                ...
                async move {
                    match event_result {
                        ...
                        Err(aimee_eventsource::Error::StreamEnded) => None,
                        Err(aimee_eventsource::Error::InvalidStatusCode(status, response)) => {
                            let (_, reason) = read_http_error_reason(*response).await;
                            Some(Err(anyhow::Error::from(
                                aimee_app::dto::openai::Error::InvalidStatusCode(status.as_u16()),
                            )
                            .context(reason)
                            .context(format_http_context(None, "POST", &url))))
                        }
                        Err(aimee_eventsource::Error::InvalidContentType(_, response)) => {
                ...
                match event_result {
                    ...
                    Ok(event) => {
                        ...
                        match result {
                            ...
                            Ok(super::response::ResponsesStreamEvent::Ping { cost }) => {
                                let usage =
                                    aimee_domain::Usage { cost: Some(cost), ..Default::default() };
                                Some(Ok(super::response::StreamItem::Message(Box::new(
                                    ChatCompletionMessage::assistant(aimee_domain::Content::part(
                                        "",
                                    ))
                                    .usage(usage),
                                ))))
...
fn status_code_error(status: StatusCode, body: String) -> anyhow::Error {
    anyhow::Error::from(aimee_app::dto::openai::Error::InvalidStatusCode(
        status.as_u16(),
    ))
    .context(body)
}

fn into_sse_parse_error<E>(error: aimee_eventsource_stream::EventStreamError<E>) -> anyhow::Error
where
    E: std::fmt::Debug + std::fmt::Display + Send + Sync + 'static,
{
    let is_retryable = matches!(
        &error,
        aimee_eventsource_stream::EventStreamError::Transport(_)
    );
    let error = anyhow::anyhow!("SSE parse error: {}", error);

    if is_retryable {
        aimee_domain::Error::Retryable(error).into()
    } else {
        error
    }
}
...
fn is_codex_responses_lite(provider: &Provider<Url>, model: &ModelId) -> bool {
    provider.id == aimee_domain::ProviderId::CODEX && model.as_str() == "gpt-5.6-luna"
}
...
impl<F: HttpInfra + EnvironmentInfra<Config = aimee_config::AimeeConfig> + 'static> ChatRepository
    for OpenAIResponsesResponseRepository<F>
{
    ...
    async fn models(&self, provider: Provider<Url>) -> anyhow::Result<Vec<Model>> {
        match provider.models().cloned() {
            Some(aimee_domain::ModelSource::Hardcoded(models)) => Ok(models),
            Some(aimee_domain::ModelSource::Url(url)) => {
                ...
                let data: aimee_app::dto::openai::ListModelResponse =
...
mod tests {
    ...
    fn is_retryable(error: &anyhow::Error) -> bool {
        error
            .downcast_ref::<aimee_domain::Error>()
            .is_some_and(|error| matches!(error, aimee_domain::Error::Retryable(_)))
    }

    fn make_credential(provider_id: ProviderId, key: &str) -> Option<aimee_domain::AuthCredential> {
        Some(aimee_domain::AuthCredential {
            id: provider_id,
            auth_details: aimee_domain::AuthDetails::ApiKey(aimee_domain::ApiKey::from(
                key.to_string(),
            )),
    ...
    impl HttpInfra for MockHttpClient {
        ...
        async fn http_eventsource(
            ...
            Ok(aimee_eventsource::EventSource::new(request)?)
        }
    }

    impl aimee_app::EnvironmentInfra for MockHttpClient {
    ...
    fn test_openai_responses_provider_new_with_oauth_with_api_key() {
        let provider = Provider {
            ...
            credential: Some(aimee_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: aimee_domain::AuthDetails::OAuthWithApiKey {
                    tokens: aimee_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
                    ),
                    api_key: aimee_domain::ApiKey::from("oauth-key".to_string()),
                    config: aimee_domain::OAuthConfig {
                        auth_url: Url::parse("https://example.com/auth").unwrap(),
                        token_url: Url::parse("https://example.com/token").unwrap(),
                        client_id: aimee_domain::ClientId::from("client-id".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
    ...
    fn test_openai_responses_provider_new_with_oauth() {
        let provider = Provider {
            ...
            credential: Some(aimee_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: aimee_domain::AuthDetails::OAuth {
                    tokens: aimee_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
    ...
    fn test_get_headers_with_oauth_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![aimee_domain::AuthMethod::OAuthDevice(
                aimee_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: aimee_domain::ClientId::from("client-id".to_string()),
                    scopes: vec![],
    ...
    fn test_get_headers_with_oauth_code_custom_headers() {
        let provider = Provider {
            ...
            credential: make_credential(ProviderId::OPENAI, "test-key"),
            custom_headers: None,
            auth_methods: vec![aimee_domain::AuthMethod::OAuthCode(
                aimee_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: aimee_domain::ClientId::from("client-id".to_string()),
    ...
    fn test_into_sse_parse_error_marks_transport_errors_retryable() {
        let error = into_sse_parse_error(aimee_eventsource_stream::EventStreamError::Transport(
    ...
    fn test_into_sse_parse_error_keeps_utf8_errors_non_retryable() {
        let error = into_sse_parse_error(
            aimee_eventsource_stream::EventStreamError::<anyhow::Error>::Utf8(
    ...
    fn test_get_headers_with_codex_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![aimee_domain::AuthMethod::CodexDevice(
                aimee_domain::OAuthConfig {
                    auth_url: Url::parse(
                    ...
                    client_id: aimee_domain::ClientId::from(
                    ...
                    token_refresh_url: None,
                    custom_headers: Some(
                        [("originator".to_string(), "aimee".to_string())]
                            .into_iter()
                            .collect(),
                    ),
                    extra_auth_params: None,
    ...
    fn test_get_headers_codex_includes_chatgpt_account_id() {
        ...
        url_params.insert(
            aimee_domain::URLParam::from("chatgpt_account_id".to_string()),
            aimee_domain::URLParamValue::from("acct_test_123".to_string()),
        );
            ...
            credential: Some(aimee_domain::AuthCredential {
                ...
                auth_details: aimee_domain::AuthDetails::OAuth {
                    ...
                    config: aimee_domain::OAuthConfig {
                        ...
                        .unwrap(),
                        token_url: Url::parse("https://auth.openai.com/oauth/token").unwrap(),
                        client_id: aimee_domain::ClientId::from("app_test".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
                        token_refresh_url: None,
    ...
    fn test_get_headers_non_codex_does_not_include_chatgpt_account_id() {
        ...
        url_params.insert(
            aimee_domain::URLParam::from("chatgpt_account_id".to_string()),
            aimee_domain::URLParamValue::from("acct_should_not_appear".to_string()),
        );
    ...
    async fn test_stream_503_error_is_retryable() -> anyhow::Result<()> {
        ...
        let retry_config =
            aimee_config::RetryConfig::default().status_codes(vec![429, 500, 502, 503, 504]);
        let retry_error = retry::into_retry(error, &retry_config);
        assert!(
            retry_error
                .downcast_ref::<aimee_domain::Error>()
                .is_some_and(|e| { matches!(e, aimee_domain::Error::Retryable(_)) }),
    ...
    async fn test_503_error_triggers_retry() -> anyhow::Result<()> {
        ...
        let infra = Arc::new(MockHttpClient { client: reqwest::Client::new() });
        let provider_impl = OpenAIResponsesProvider::new(provider, infra);
        let retry_config = aimee_config::RetryConfig::default()
        ...
        let result: anyhow::Result<()> = aimee_app::retry::retry_with_config(
            &retry_config,
            || {
