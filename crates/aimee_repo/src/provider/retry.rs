use aimee_app::domain::Error as DomainError;
use aimee_app::dto::anthropic::Error as AnthropicError;
use aimee_app::dto::openai::{Error, ErrorCode, ErrorResponse};
...
fn get_event_req_status_code(error: &anyhow::Error) -> Option<u16> {
    error
        .downcast_ref::<aimee_eventsource::Error>()
        .and_then(|error| match error {
            aimee_eventsource::Error::InvalidStatusCode(_, response) => {
                Some(response.status().as_u16())
            }
            aimee_eventsource::Error::InvalidContentType(_, response) => {
                Some(response.status().as_u16())
            }
            _ => None,
        })
...
fn is_event_transport_error(error: &anyhow::Error) -> bool {
    error
        .downcast_ref::<aimee_eventsource::Error>()
        .is_some_and(|e| matches!(e, aimee_eventsource::Error::Transport(_)))
}
