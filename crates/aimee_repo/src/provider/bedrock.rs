...
impl BedrockProvider {
    ...
    pub fn new(provider: Provider<Url>) -> Result<Self> {
        ...
        let region_param: aimee_domain::URLParam = "AWS_REGION".to_string().into();
    ...
    pub async fn chat(
        ...
        let output = self
            ...
            .map_err(|sdk_error| {
                ...
                if is_retryable {
                    aimee_domain::Error::Retryable(anyhow::anyhow!("{}", source)).into()
                } else {
            ...
            match event_stream.recv().await {
                ...
                Err(stream_error) => {
                    ...
                    let error = if is_retryable {
                        aimee_domain::Error::Retryable(anyhow::anyhow!(
    ...
    pub async fn models(&self) -> Result<Vec<Model>> {
        ...
        match &self.provider.models {
            Some(aimee_domain::ModelSource::Hardcoded(models)) => Ok(models.clone()),
            _ => Ok(vec![]),
        }
    }
}
...
impl IntoDomain for aws_sdk_bedrockruntime::types::ConverseStreamOutput {
    type Domain = aimee_domain::ChatCompletionMessage;
        ...
        match self {
                            ...
                            match reasoning {
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Text(
                                    ...
                                    ChatCompletionMessage::default()
                                        ...
                                        .add_reasoning_detail(aimee_domain::Reasoning::Part(vec![
                                            aimee_domain::ReasoningPart {
                                                text: Some(text),
                                                signature: None,
                                                ..Default::default()
                                            },
                                ...
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Signature(
                                    ...
                                    ChatCompletionMessage::default().add_reasoning_detail(
                                        aimee_domain::Reasoning::Part(vec![
                                            aimee_domain::ReasoningPart {
                                                text: None,
                                                signature: Some(sig),
                                                ..Default::default()
            ...
            ConverseStreamOutput::Metadata(metadata) => {
                ...
                let usage = metadata.usage.map(|u| {
                    ...
                    aimee_domain::Usage {
                        prompt_tokens: aimee_domain::TokenCount::Actual(u.input_tokens as usize),
                        completion_tokens: aimee_domain::TokenCount::Actual(
                            u.output_tokens as usize,
                        ),
                        total_tokens: aimee_domain::TokenCount::Actual(u.total_tokens as usize),
                        cached_tokens: aimee_domain::TokenCount::Actual(cached_tokens as usize),
                        ..Default::default()
                    }
                });
...
impl FromDomain<aimee_domain::Context>
    for aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput
{
    fn from_domain(context: aimee_domain::Context) -> anyhow::Result<Self> {
        ...
        let system: Vec<SystemContentBlock> = context
            ...
            .filter_map(|msg| match &msg.message {
                aimee_domain::ContextMessage::Text(text_msg)
                    if text_msg.has_role(aimee_domain::Role::System) =>
                {
        ...
        let messages: Vec<Message> = {
            ...
            let mut pending_tool_results: Vec<aimee_domain::ContextMessage> = Vec::new();

            for message in context.messages.into_iter() {
                if message.has_role(aimee_domain::Role::System) {
                    continue;
                }

                match &message.message {
                    aimee_domain::ContextMessage::Tool(_) => {
        ...
        let tool_config = if !context.tools.is_empty() {
            ...
            let choice = context
                .tool_choice
                .filter(|c| !matches!(c, aimee_domain::ToolChoice::None))
        ...
        let adjusted_top_p = if emits_legacy_thinking {
            ...
            context.top_p.map(|p| {
                ...
                if value < 0.95 {
                    ...
                    aimee_domain::TopP::new(0.95).expect("0.95 is valid TopP")
                } else {
                    p
                }
            })
            ...
            if !reasoning_on {
                ...
                if let Some(effort) = reasoning_config.effort.as_ref() {
                    let effort_str = match effort {
                        aimee_domain::Effort::None => None,
                        aimee_domain::Effort::Minimal | aimee_domain::Effort::Low => Some("low"),
                        aimee_domain::Effort::Medium => Some("medium"),
                        aimee_domain::Effort::High => Some("high"),
                        aimee_domain::Effort::XHigh => Some("xhigh"),
                        aimee_domain::Effort::Max => Some("max"),
                    };
...
impl FromDomain<Vec<aimee_domain::ContextMessage>> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(tool_results: Vec<aimee_domain::ContextMessage>) -> anyhow::Result<Self> {
        ...
        for msg in tool_results {
            match msg {
                aimee_domain::ContextMessage::Tool(tool_result) => {
...
impl FromDomain<aimee_domain::ContextMessage> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(msg: aimee_domain::ContextMessage) -> anyhow::Result<Self> {
        ...
        match msg {
            aimee_domain::ContextMessage::Text(text_msg) => {
                ...
                if text_msg.role == aimee_domain::Role::Assistant
                ...
                let role = match text_msg.role {
                    aimee_domain::Role::User => ConversationRole::User,
                    aimee_domain::Role::Assistant => ConversationRole::Assistant,
                    aimee_domain::Role::System => {
            ...
            aimee_domain::ContextMessage::Image(img) => {
...
impl FromDomain<aimee_domain::ToolCallArguments> for aws_smithy_types::Document {
    fn from_domain(args: aimee_domain::ToolCallArguments) -> anyhow::Result<Self> {
        use anyhow::Context as _;
...
impl FromDomain<aimee_domain::ToolDefinition> for aws_sdk_bedrockruntime::types::Tool {
    fn from_domain(tool: aimee_domain::ToolDefinition) -> anyhow::Result<Self> {
...
impl FromDomain<aimee_domain::ToolChoice> for aws_sdk_bedrockruntime::types::ToolChoice {
    fn from_domain(choice: aimee_domain::ToolChoice) -> anyhow::Result<Self> {
        use aws_sdk_bedrockruntime::types::{
            AnyToolChoice, AutoToolChoice, SpecificToolChoice, ToolChoice,
        };

        let bedrock_choice = match choice {
            aimee_domain::ToolChoice::Auto => ToolChoice::Auto(AutoToolChoice::builder().build()),
            aimee_domain::ToolChoice::Required => ToolChoice::Any(AnyToolChoice::builder().build()),
            aimee_domain::ToolChoice::Call(tool_name) => ToolChoice::Tool(
                SpecificToolChoice::builder()
                    .name(tool_name.to_string())
                    .build()
                    .map_err(|e| anyhow::anyhow!("Failed to build tool choice: {}", e))?,
            ),
            aimee_domain::ToolChoice::None => {
...
#[cfg(test)]
mod tests {
    use aimee_domain::InputModality;
    ...
    async fn test_models_returns_hardcoded() {
        use aimee_domain::{Model, ModelId, ModelSource};
    ...
    fn test_into_domain_content_block_delta_text() {
        use aimee_domain::{ChatCompletionMessage, Content};
    ...
    fn test_into_domain_content_block_start_tool_use() {
        use aimee_domain::{ChatCompletionMessage, Content, ToolCallId, ToolCallPart, ToolName};
        use aws_sdk_bedrockruntime::types::{
    ...
    fn test_into_domain_message_stop_end_turn() {
        use aimee_domain::{ChatCompletionMessage, Content, FinishReason};
    ...
    fn test_into_domain_metadata_with_usage() {
        use aimee_domain::{ChatCompletionMessage, Content, TokenCount};
        use aws_sdk_bedrockruntime::types::ConverseStreamOutput;
        ...
        let actual = fixture.into_domain();
        let expected =
            ChatCompletionMessage::assistant(Content::part("")).usage(aimee_domain::Usage {
    ...
    fn test_from_domain_tool_choice_auto() {
        ...
        let fixture = aimee_domain::ToolChoice::Auto;
    ...
    fn test_from_domain_tool_choice_required() {
        ...
        let fixture = aimee_domain::ToolChoice::Required;
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_choice_call() {
        ...
        let fixture = aimee_domain::ToolChoice::Call(aimee_domain::ToolName::new("my_tool"));
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_definition() {
        use aimee_domain::ToolDefinition;
        ...
        let fixture = ToolDefinition {
            name: aimee_domain::ToolName::new("test_tool"),
    ...
    fn test_from_domain_context_message_text_user() {
        use aimee_domain::{ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant() {
        use aimee_domain::{ContextMessage, TextMessage};
        use aws_sdk_bedrockruntime::types::{ContentBlock, ConversationRole, Message};
    ...
    fn test_from_domain_context_message_text_assistant_reasoning_uses_message_signature() {
        use aimee_domain::{ContextMessage, ReasoningFull, TextMessage};
        use aws_sdk_bedrockruntime::types::{
    ...
    fn test_from_domain_context_message_tool_result() {
        use aimee_domain::{ContextMessage, ToolCallId, ToolResult};
        use aws_sdk_bedrockruntime::types::{
    ...
    fn test_from_domain_context_with_system_messages() {
        use aimee_domain::{Context, ContextMessage, Role, TextMessage};
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
    ...
    fn test_from_domain_context_with_temperature() {
        use aimee_domain::{Context, Temperature};
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
    ...
    fn test_from_domain_context_with_reasoning_adjusts_top_p() {
        use aimee_domain::{Context, ReasoningConfig, TopP};
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
    ...
    fn test_from_domain_context_with_reasoning_enabled() {
        use aimee_domain::{Context, ReasoningConfig};
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
    ...
    fn test_from_domain_context_emits_output_config_effort() {
        use aimee_domain::{Context, Effort, ReasoningConfig};
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
    ...
    /// call Bedrock. Run with: cargo test -p aimee_repo
