...
use aimee_app::domain::Skill;
use aimee_app::{EnvironmentInfra, FileInfoInfra, FileReaderInfra, Walker, WalkerInfra};
...
/// 2. Global custom skills (from ~/aimee/skills/ directory)
/// 3. Agents skills (from ~/.agents/skills/ directory)
/// 4. Project-local skills (from .aimee/skills/ directory in current working
///    directory)
///
/// ## Skill Precedence
...
/// - **Global skills**: `~/aimee/skills/<skill-name>/SKILL.md`
/// - **Agents skills**: `~/.agents/skills/<skill-name>/SKILL.md`
/// - **CWD skills**: `./.aimee/skills/<skill-name>/SKILL.md` (relative to
///   current working directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct AimeeSkillRepository<I> {
    infra: Arc<I>,
}

impl<I> AimeeSkillRepository<I> {
    ...
    fn load_builtin_skills(&self) -> Vec<Skill> {
        let builtin_skills = vec![
            (
                "aimee://skills/create-skill/SKILL.md",
                include_str!("skills/create-skill/SKILL.md"),
            ),
            (
                "aimee://skills/execute-plan/SKILL.md",
                include_str!("skills/execute-plan/SKILL.md"),
            ),
            (
                "aimee://skills/github-pr-description/SKILL.md",
    ...
    for AimeeSkillRepository<I>
{
    ...
    async fn load_skills(&self) -> anyhow::Result<Vec<Skill>> {
        ...
        let global_skills = self.load_skills_from_dir(&global_dir, 1).await?;
        skills.extend(global_skills);

        // Load agents skills (~/.agents/skills)
        if let Some(agents_dir) = env.agents_skills_path() {
            let agents_skills = self.load_skills_from_dir(&agents_dir, 1).await?;
            skills.extend(agents_skills);
        }

        // Load project-local skills
        let cwd_dir = env.local_skills_path();
        let cwd_skills = self.load_skills_from_dir(&cwd_dir, 1).await?;
        skills.extend(cwd_skills);

        if let Some(hermes_dir) = env.hermes_skills_path() {
            let hermes_skills = self.load_skills_from_dir(&hermes_dir, 3).await?;
            skills.extend(hermes_skills);
        }
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> AimeeSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &aimee_domain::Environment) -> Skill {
...
mod tests {
    ...
    fn fixture_skill_repo() -> (AimeeSkillRepository<AimeeInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/fixtures/skills_with_resources");
        let config = AimeeConfig::read().unwrap_or_default();
        let infra = Arc::new(AimeeInfra::new(std::env::current_dir().unwrap(), config));
        let repo = AimeeSkillRepository::new(infra);
        (repo, skill_dir)
    }
    ...
    fn test_load_builtin_skills() {
        // Fixture
        let repo = AimeeSkillRepository { infra: Arc::new(()) };
        ...
        assert_eq!(
            create_skill.path,
            Some(std::path::Path::new("aimee://skills/create-skill/SKILL.md").to_path_buf())
        );
        assert_eq!(
        ...
        assert_eq!(
            execute_plan.path,
            Some(std::path::Path::new("aimee://skills/execute-plan/SKILL.md").to_path_buf())
        );
        assert!(
            execute_plan
                .description
        ...
        assert_eq!(
            pr_description.path,
            Some(
                std::path::Path::new("aimee://skills/github-pr-description/SKILL.md").to_path_buf()
            )
        );
    ...
    async fn test_extract_skill_with_valid_metadata() {
        ...
        let content =
            aimee_test_kit::fixture!("/src/fixtures/skills/with_name_and_description.md").await;
    ...
    async fn test_extract_skill_with_incomplete_metadata() {
        // Fixture
        let content = aimee_test_kit::fixture!("/src/fixtures/skills/with_name_only.md").await;
    ...
    async fn test_load_skills_from_dir() {
        ...
        // Act
        let actual = repo.load_skills_from_dir(&skill_dir, 1).await.unwrap();
