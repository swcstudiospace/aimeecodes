...
use aimee_app::domain::File;
use aimee_app::{
    DirectoryReaderInfra, EnvironmentInfra, FileDiscoveryService, Walker, WalkerInfra,
};
use anyhow::Result;

pub struct AimeeDiscoveryService<F> {
    service: Arc<F>,
}

impl<F> AimeeDiscoveryService<F> {
    pub fn new(service: Arc<F>) -> Self {
        Self { service }
    }
}

impl<F: EnvironmentInfra + WalkerInfra> AimeeDiscoveryService<F> {
...
mod tests {
    ...
    use aimee_app::{EnvironmentInfra, WalkedFile};
    ...
    async fn test_list_current_directory_sorts_dirs_first() {
        ...
        let service = AimeeDiscoveryService::new(Arc::new(fixture));
