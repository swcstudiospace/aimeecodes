...
use aimee_app::{AppConfigService, EnvironmentInfra};
use aimee_domain::{ConfigOperation, Effort, ModelConfig, ModelId, ProviderId, ProviderRepository};
use tracing::debug;
...
pub struct AimeeAppConfigService<F> {
    infra: Arc<F>,
}

impl<F> AimeeAppConfigService<F> {
...
impl<F: ProviderRepository + EnvironmentInfra<Config = aimee_config::AimeeConfig> + Send + Sync>
    AppConfigService for AimeeAppConfigService<F>
{
    ...
    async fn get_commit_config(&self) -> anyhow::Result<Option<aimee_domain::ModelConfig>> {
    ...
    async fn get_suggest_config(&self) -> anyhow::Result<Option<aimee_domain::ModelConfig>> {
        let config = self.infra.get_config()?;
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<Effort>> {
        ...
        Ok(config
            ...
            .and_then(|r| r.effort)
            .map(|e| match e {
                aimee_config::Effort::None => Effort::None,
                aimee_config::Effort::Minimal => Effort::Minimal,
                aimee_config::Effort::Low => Effort::Low,
                aimee_config::Effort::Medium => Effort::Medium,
                aimee_config::Effort::High => Effort::High,
                aimee_config::Effort::XHigh => Effort::XHigh,
                aimee_config::Effort::Max => Effort::Max,
            }))
    }
...
mod tests {
    ...
    use aimee_config::{AimeeConfig, ModelConfig};
    // Alias to avoid collision with aimee_config::ModelConfig used in test fixtures
    use aimee_domain::ModelConfig as DomainModelConfig;
    use aimee_domain::{
    ...
    impl MockInfra {
        fn new() -> Self {
            Self {
                config: Arc::new(Mutex::new(AimeeConfig::default())),
                providers: vec![
                    Provider {
                        ...
                        credential: Some(aimee_domain::AuthCredential {
                            id: ProviderId::OPENAI,
                            auth_details: aimee_domain::AuthDetails::ApiKey(
                                aimee_domain::ApiKey::from("test-key".to_string()),
                            ),
    ...
    impl ChatRepository for MockInfra {
        async fn chat(
            &self,
            _model_id: &aimee_app::domain::ModelId,
            _context: aimee_app::domain::Context,
            _provider: Provider<Url>,
        ) -> aimee_app::domain::ResultStream<aimee_app::domain::ChatCompletionMessage, anyhow::Error>
        {
        ...
        async fn models(
            &self,
            _provider: Provider<Url>,
        ) -> anyhow::Result<Vec<aimee_app::domain::Model>> {
            Ok(vec![])
        }
    }
        ...
        async fn get_provider(&self, id: ProviderId) -> anyhow::Result<ProviderTemplate> {
            ...
            self.providers
                ...
                .map(|p| Provider {
                    ...
                    url: aimee_domain::Template::<aimee_domain::URLParameters>::new(p.url.as_str()),
                    models: p.models.as_ref().map(|m| match m {
                        ModelSource::Url(url) => ModelSource::Url(aimee_domain::Template::<
                            aimee_domain::URLParameters,
                        >::new(
                            url.as_str()
                        )),
    ...
    async fn test_get_session_config_when_none_set() -> anyhow::Result<()> {
        ...
        let service = AimeeAppConfigService::new(Arc::new(fixture));
    ...
    async fn test_get_session_config_when_set() -> anyhow::Result<()> {
        let fixture = MockInfra::new();
        let service = AimeeAppConfigService::new(Arc::new(fixture.clone()));
    ...
    async fn test_set_multiple_default_models() -> anyhow::Result<()> {
        ...
        // AimeeConfig only tracks a single active session, so the last
