...
use aimee_app::{EnvironmentInfra, FileReaderInfra, TemplateService};
use aimee_domain::Template;
use anyhow::Context;
use futures::future;
...
#[derive(Clone)]
pub struct AimeeTemplateService<F> {
    hb: Arc<OnceCell<RwLock<Handlebars<'static>>>>,
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + FileReaderInfra> AimeeTemplateService<F> {
    ...
    async fn get_hb(&self) -> &RwLock<Handlebars<'static>> {
        self.hb
            .get_or_init(|| async { RwLock::new(aimee_app::TemplateEngine::handlebar_instance()) })
            .await
    }
...
impl<F: EnvironmentInfra + FileReaderInfra> TemplateService for AimeeTemplateService<F> {
...
mod tests {
    ...
    async fn test_render_simple_template() {
        ...
        let service = AimeeTemplateService::new(Arc::new(MockCompositeService::new()));
        let data = json!({
            "name": "Aimee",
            "version": "1.0",
        ...
        let expected = "App: Aimee v1.0 - Features: templates, rendering, handlebars";
    ...
    async fn test_render_partial_system_info() {
        ...
        let actual = service
            .render_template(Template::new("{{> aimee-partial-system-info.md }}"), &data)
            .await
            .unwrap();
