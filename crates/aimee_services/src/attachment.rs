...
use aimee_app::utils::format_display_path;
use aimee_app::{
...
impl<
    F: FileReaderInfra
        ...
        + FileInfoInfra
        + DirectoryReaderInfra,
> AttachmentService for AimeeChatRequest<F>
{
...
pub mod tests {
    ...
    use aimee_app::domain::{AttachmentContent, Environment};
    use aimee_app::utils::compute_hash;
    use aimee_app::{
    ...
    use aimee_domain::{ConfigOperation, FileInfo};
    use base64::Engine;
    use bytes::Bytes;
    use futures::stream;
    ...
    impl EnvironmentInfra for MockEnvironmentInfra {
        ...
        fn get_config(&self) -> anyhow::Result<aimee_config::AimeeConfig> {
            Ok(aimee_config::AimeeConfig { max_read_lines: 2000, ..Default::default() })
        }
    ...
    async fn test_add_url_with_text_file() {
        ...
        let chat_request = AimeeChatRequest::new(infra.clone());
