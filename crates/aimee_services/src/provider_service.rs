...
pub struct AimeeProviderService<R> {
    repository: Arc<R>,
}

impl<R> AimeeProviderService<R> {
    /// Creates a new AimeeProviderService instance
    ...
    fn render_url_template(
        &self,
        template: &str,
        params: &HashMap<aimee_domain::URLParam, aimee_domain::URLParamValue>,
        specs: &[aimee_domain::URLParamSpec],
    ) -> Result<Url> {
        ...
        let handlebars = aimee_app::TemplateEngine::handlebar_instance();
...
impl<R: ChatRepository + ProviderRepository> ProviderService for AimeeProviderService<R> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
...
#[cfg(test)]
mod tests {
    use aimee_app::domain::ProviderId;
    use aimee_domain::{
    ...
    fn test_provider() -> Provider<Url> {
        Provider {
            ...
            response: Some(aimee_app::domain::ProviderResponse::OpenAI),
            url: Url::parse("https://api.openai.com/v1/chat/completions").unwrap(),
            auth_methods: vec![AuthMethod::ApiKey],
            url_params: vec![],
            credential: Some(AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: AuthDetails::ApiKey(aimee_domain::ApiKey::from(
                    "test-key".to_string(),
                )),
    ...
    fn test_template_provider() -> ProviderTemplate {
        Provider {
            ...
            provider_type: ProviderType::Llm,
            response: Some(aimee_app::domain::ProviderResponse::OpenAI),
            url: Template::<aimee_domain::URLParameters>::new(
            ...
            models: Some(ModelSource::Url(
                Template::<aimee_domain::URLParameters>::new("https://api.openai.com/v1/models"),
            )),
            custom_headers: None,
        }
    }
    ...
    async fn test_models_delegates_to_repository() {
        ...
        let service = AimeeProviderService::new(repository);
    ...
    fn test_render_url_template_optional_port_absent() {
        ...
        let service = AimeeProviderService::new(Arc::new(MockProviderRepository::new(vec![])));
        let template = "{{VLLM_SSL_SCHEME}}://{{VLLM_HOST}}{{#if VLLM_PORT}}:{{VLLM_PORT}}{{/if}}/v1/chat/completions";

        let mut params = HashMap::new();
        params.insert(
            aimee_domain::URLParam::from("VLLM_SSL_SCHEME".to_string()),
            aimee_domain::URLParamValue::from("https".to_string()),
        );
        params.insert(
            aimee_domain::URLParam::from("VLLM_HOST".to_string()),
            aimee_domain::URLParamValue::from("my.server.url".to_string()),
        );
        // VLLM_PORT intentionally absent — not in the credential map at all.

        let specs = vec![aimee_domain::URLParamSpec::optional(
            aimee_domain::URLParam::from("VLLM_PORT".to_string()),
        )];
    ...
    fn test_render_url_template_optional_port_with_value() {
        ...
        params.insert(
            aimee_domain::URLParam::from("VLLM_PORT".to_string()),
            aimee_domain::URLParamValue::from("8000".to_string()),
        );
