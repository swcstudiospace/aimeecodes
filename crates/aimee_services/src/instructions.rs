...
use aimee_app::{CommandInfra, CustomInstructionsService, EnvironmentInfra, FileReaderInfra};
...
#[derive(Clone)]
pub struct AimeeCustomInstructionsService<F> {
    infra: Arc<F>,
    cache: tokio::sync::OnceCell<Vec<String>>,
}

impl<F: EnvironmentInfra + FileReaderInfra + CommandInfra> AimeeCustomInstructionsService<F> {
    ...
    async fn discover_agents_files(&self) -> Vec<PathBuf> {
        ...
        for (soul_path, _) in
            aimee_domain::SoulDocument::candidate_paths(&environment.cwd, &environment.base_path)
        {
            if !paths.contains(&soul_path) {
                paths.push(soul_path);
            }
        }
...
impl<F: EnvironmentInfra + FileReaderInfra + CommandInfra> CustomInstructionsService
    for AimeeCustomInstructionsService<F>
{
