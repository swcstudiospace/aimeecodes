...
use aimee_app::{AuthStrategy, ProviderAuthService, StrategyFactory};
use aimee_domain::{
    AuthContextRequest, AuthContextResponse, AuthMethod, Provider, ProviderId, ProviderRepository,
};

/// Aimee Provider Authentication Service
#[derive(Clone)]
pub struct AimeeProviderAuthService<I> {
    infra: Arc<I>,
}

impl<I> AimeeProviderAuthService<I> {
...
impl<I> ProviderAuthService for AimeeProviderAuthService<I>
where
    ...
    async fn init_provider_auth(
        ...
        let required_params = if matches!(
            ...
            let provider = providers
                ...
                .ok_or_else(|| aimee_domain::Error::provider_not_available(provider_id.clone()))?;
    ...
    async fn complete_provider_auth(
        ...
        let auth_method = match &auth_context_response {
            AuthContextResponse::ApiKey(response) => {
                ...
                let is_vertex_provider = provider_id == aimee_domain::ProviderId::VERTEX_AI
                    || provider_id == aimee_domain::ProviderId::VERTEX_AI_ANTHROPIC;
                if is_vertex_provider && response.response.api_key.as_ref() == "google_adc_marker" {
                    // Vertex AI uses Google ADC
                    aimee_domain::AuthMethod::google_adc()
            ...
            AuthContextResponse::DeviceCode(ctx) => {
                if provider_id == aimee_domain::ProviderId::CODEX {
                        ...
                        | AuthMethod::GoogleAdc => {
                            ...
                            let existing_credential =
                                self.infra.get_credential(&provider.id).await?.ok_or_else(
                                    || aimee_domain::Error::ProviderNotAvailable {
