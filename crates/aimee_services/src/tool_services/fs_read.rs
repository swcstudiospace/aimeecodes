...
use aimee_domain::{FileInfo, Image};
use anyhow::Context;
...
pub struct AimeeFsRead<F> {
    infra: Arc<F>,
}

impl<F> AimeeFsRead<F> {
...
impl<F: FileInfoInfra + EnvironmentInfra<Config = aimee_config::AimeeConfig> + InfraFsReadService>
    FsReadService for AimeeFsRead<F>
{
    async fn read(
        &self,
        path: String,
            ...
            assert_file_size(&*self.infra, path, config.max_image_size_bytes)
                ...
                .with_context(|| {
                    if mime_type == "application/pdf" {
                        "PDF exceeds size limit. Use a smaller PDF or increase AIMEE_MAX_IMAGE_SIZE."
                    } else {
                        "Image exceeds size limit. Compress the image or increase AIMEE_MAX_IMAGE_SIZE."
                    }
                })?;
