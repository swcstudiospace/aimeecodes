...
use aimee_app::{FileInfoInfra, FileReaderInfra, FsUndoOutput, FsUndoService};
use aimee_domain::SnapshotRepository;
...
#[derive(Default)]
pub struct AimeeFsUndo<F> {
    infra: Arc<F>,
}

impl<F> AimeeFsUndo<F> {
...
impl<F: FileInfoInfra + FileReaderInfra + SnapshotRepository> FsUndoService for AimeeFsUndo<F> {
