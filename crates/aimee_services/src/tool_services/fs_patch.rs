...
use aimee_app::domain::PatchOperation;
use aimee_app::{EnvironmentInfra, FileWriterInfra, FsPatchService, PatchOutput, compute_hash};
...
// Using PatchOperation from aimee_domain

// Using FSPatchInput from aimee_domain
...
pub struct AimeeFsPatch<F> {
    infra: Arc<F>,
}

impl<F> AimeeFsPatch<F> {
...
impl<
    F: EnvironmentInfra<Config = AimeeConfig>
        + FileWriterInfra
        + SnapshotRepository
        + ValidationRepository
        + FuzzySearchRepository
        + TextPatchRepository,
> FsPatchService for AimeeFsPatch<F>
{
    async fn patch(
        &self,
        input_path: String,
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl aimee_domain::FuzzySearchRepository for FallbackRepository {
        async fn fuzzy_search(
            &self,
            _needle: &str,
            _haystack: &str,
            _search_all: bool,
        ) -> anyhow::Result<Vec<aimee_domain::SearchMatch>> {
            let actual = vec![aimee_domain::SearchMatch { start_line: 1, end_line: 1 }];
            Ok(actual)
        }
    }

    #[async_trait::async_trait]
    impl aimee_domain::TextPatchRepository for FallbackRepository {
        async fn build_text_patch(
            &self,
            _haystack: &str,
            _old_string: &str,
            _new_string: &str,
        ) -> anyhow::Result<aimee_domain::TextPatchBlock> {
            let actual = aimee_domain::TextPatchBlock {
                patch: "@@ -1 +1 @@".to_string(),
