...
use aimee_domain::{FSSearch, OutputMode};
...
pub struct AimeeFsSearch<W> {
    infra: Arc<W>,
}

impl<W> AimeeFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> FsSearchService for AimeeFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> AimeeFsSearch<W> {
...
mod test {
    ...
    async fn test_basic_content_search() {
        ...
        let actual = AimeeFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await
            .unwrap();
    ...
    async fn test_path_defaults_to_cwd() {
        ...
        let result = AimeeFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await;
