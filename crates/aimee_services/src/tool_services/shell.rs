...
use aimee_app::domain::Environment;
use aimee_app::{CommandInfra, EnvironmentInfra, ShellOutput, ShellService};
use anyhow::bail;
use bstr::ByteSlice;
...
/// unrestricted access, advise users to run aimee CLI with '-u' flag. Returns
/// complete output including stdout, stderr, and exit code for diagnostic
/// purposes.
pub struct AimeeShell<I> {
    env: Environment,
    infra: Arc<I>,
}

impl<I: EnvironmentInfra> AimeeShell<I> {
...
impl<I: CommandInfra + EnvironmentInfra> ShellService for AimeeShell<I> {
    async fn execute(
        &self,
        command: String,
        cwd: PathBuf,
...
mod tests {
    ...
    use aimee_app::domain::{CommandOutput, Environment};
    use aimee_app::{CommandInfra, EnvironmentInfra, ShellService};
    use aimee_domain::ConfigOperation;
    ...
    async fn test_shell_service_forwards_env_vars() {
        let fixture = AimeeShell::new(Arc::new(MockCommandInfra {
    ...
    async fn test_shell_service_forwards_no_env_vars() {
        let fixture = AimeeShell::new(Arc::new(MockCommandInfra { expected_env_vars: None }));
