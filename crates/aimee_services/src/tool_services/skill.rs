...
use aimee_app::SkillFetchService;
use aimee_domain::Skill;
...
pub struct AimeeSkillFetch<R> {
    repository: Arc<R>,
    cache: OnceCell<Vec<Skill>>,
}

impl<R> AimeeSkillFetch<R> {
...
impl<R: aimee_domain::SkillRepository> SkillFetchService for AimeeSkillFetch<R> {
...
impl<R: aimee_domain::SkillRepository> AimeeSkillFetch<R> {
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl aimee_domain::SkillRepository for MockSkillRepository {
    ...
    async fn test_fetch_skill_found() {
        ...
        let fetch_service = AimeeSkillFetch::new(Arc::new(repo));
