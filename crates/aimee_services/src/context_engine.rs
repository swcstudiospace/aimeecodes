...
use aimee_app::{CommandInfra, EnvironmentInfra, FileReaderInfra, WalkerInfra, WorkspaceService};
use aimee_domain::{
...
pub struct AimeeWorkspaceService<F, D> {
    infra: Arc<F>,
    discovery: Arc<D>,
}

impl<F, D> Clone for AimeeWorkspaceService<F, D> {
    fn clone(&self) -> Self {
        Self {
...
impl<F, D> AimeeWorkspaceService<F, D> {
...
> AimeeWorkspaceService<F, D>
{
    ...
    /// Gets the AimeeCodes services credential and extracts workspace auth
    /// components
    ///
    /// # Errors
    /// Returns an error if the credential is not found, if there's a database
    /// error, or if the credential format is invalid
    async fn get_workspace_credentials(&self) -> Result<(aimee_domain::ApiKey, UserId)> {
        let credential = self
            .infra
            .get_credential(&ProviderId::AIMEE_SERVICES)
            .await?
        ...
        match &credential.auth_details {
            AuthDetails::ApiKey(token) => {
                ...
                let user_id_str = credential
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!("Missing user_id in AimeeServices credential")
                    })?;
                let user_id = UserId::from_string(user_id_str.as_str())?;

                Ok((token.clone(), user_id))
            }
            _ => anyhow::bail!("AimeeServices credential must be an API key"),
        }
    }
    ...
    async fn find_workspace_by_path(
        ...
        let mut best_match: Option<(&aimee_domain::WorkspaceInfo, usize)> = None;
    ...
    async fn get_workspace_by_path(
        ...
        path: PathBuf,
        token: &aimee_domain::ApiKey,
    ) -> Result<aimee_domain::WorkspaceInfo> {
        self.find_workspace_by_path(path, token)
            .await?
            .context("Workspace not indexed. Please run `aimee workspace init` first.")
    }
...
impl<
    F: ProviderRepository
        ...
        + 'static,
    D: FileDiscovery + 'static,
> WorkspaceService for AimeeWorkspaceService<F, D>
{
    ...
    async fn query_workspace(
        ...
        let workspace = self
            ...
            .await?
            .ok_or(aimee_domain::Error::WorkspaceNotFound)?;

        let search_query =
            aimee_domain::CodeBase::new(user_id, workspace.workspace_id.clone(), params);
    ...
    async fn delete_workspace(&self, workspace_id: &aimee_domain::WorkspaceId) -> Result<()> {
        let (token, _) = self.get_workspace_credentials().await?;
    ...
    /// database.
    async fn delete_workspaces(&self, workspace_ids: &[aimee_domain::WorkspaceId]) -> Result<()> {
    ...
    async fn init_auth_credentials(&self) -> Result<aimee_domain::WorkspaceAuth> {
    ...
    async fn init_workspace(&self, path: PathBuf) -> Result<WorkspaceId> {
        ...
        if is_new {
            ...
            Err(aimee_domain::Error::WorkspaceAlreadyInitialized(workspace_id).into())
        }
    }
}
