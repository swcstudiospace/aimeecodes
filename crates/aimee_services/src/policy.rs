...
#[derive(Clone)]
pub struct AimeePolicyService<I> {
    infra: Arc<I>,
}
    ...
    I: FileReaderInfra + FileWriterInfra + FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra,
{
    ...
    /// Load all policy definitions from the aimee/policies directory
...
impl<I> PolicyService for AimeePolicyService<I>
where
    I: FileReaderInfra
        + FileWriterInfra
        + FileInfoInfra
        ...
        match permission {
            ...
            Permission::Confirm => {
                if aimee_domain::ApprovalMode::current().skips_prompts() {
                    return Ok(PolicyDecision { allowed: true, path });
                }
