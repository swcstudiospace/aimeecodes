...
This crate provides a unified interface for terminal user interactions across the omega codebase. It encapsulates all direct dependencies on `dialoguer`, ensuring no other crates need to depend on it directly.
...
```rust
use omega_select::OmegaSelect;

let options = vec!["Option 1", "Option 2", "Option 3"];
let selected = OmegaSelect::select("Choose an option:", options)
...
let confirmed = OmegaSelect::confirm("Are you sure?")
    .with_default(true)
    .prompt()?;
```
...
let name = OmegaSelect::input("Enter your name:")
    .allow_empty(false)
    .with_default("John")
    .prompt()?;
```
...
let selected = OmegaSelect::multi_select("Choose colors:", options)
    .prompt()?;
```
...
- Create the builder with `OmegaSelect::select()`, `OmegaSelect::confirm()`, etc.
...
- `omega_main`: For CLI user interactions
- `omega_infra`: For implementing the `UserInfra` trait
