...
# Core utility functions for aimee plugin

# Lazy loader for commands cache
# Loads the commands list only when first needed, avoiding startup cost
function _aimee_get_commands() {
    if [[ -z "$_AIMEE_COMMANDS" ]]; then
        _AIMEE_COMMANDS="$(CLICOLOR_FORCE=0 $_AIMEE_BIN list commands --porcelain 2>/dev/null)"
    fi
    echo "$_AIMEE_COMMANDS"
}

# Helper function to execute aimee commands consistently
# This ensures proper handling of special characters and consistent output
function _aimee_exec() {
    local agent_id="${_AIMEE_ACTIVE_AGENT:-aimee}"
    local -a cmd
    cmd=($_AIMEE_BIN --agent "$agent_id")
    ...
    # Use `local -x` so the variables are exported only to the child aimee
    # process and do not leak into the caller's shell environment.
    if [[ "$_AIMEE_TERM" == "true" && ${#_AIMEE_TERM_COMMANDS} -gt 0 ]]; then
        ...
        IFS="$_sep"
        local -x _AIMEE_TERM_COMMANDS="${_AIMEE_TERM_COMMANDS[*]}"
        local -x _AIMEE_TERM_EXIT_CODES="${_AIMEE_TERM_EXIT_CODES[*]}"
        local -x _AIMEE_TERM_TIMESTAMPS="${_AIMEE_TERM_TIMESTAMPS[*]}"
        IFS="$_old_ifs"
    fi
...
# Like _aimee_exec but connects stdin/stdout to /dev/tty so that interactive
# prompts (rustyline, nucleo-picker, etc.) work correctly when aimee is launched as a
...
# Do NOT use inside $(...) command substitutions - use _aimee_exec instead.
function _aimee_exec_interactive() {
    local agent_id="${_AIMEE_ACTIVE_AGENT:-aimee}"
    local -a cmd
    cmd=($_AIMEE_BIN --agent "$agent_id")
    ...
    # this function call (i.e. inherited by the child aimee process) and do
...
function _aimee_select() {
    [[ -n "$_AIMEE_SESSION_MODEL" ]] && local -x AIMEE_SESSION__MODEL_ID="$_AIMEE_SESSION_MODEL"
    [[ -n "$_AIMEE_SESSION_PROVIDER" ]] && local -x AIMEE_SESSION__PROVIDER_ID="$_AIMEE_SESSION_PROVIDER"
    [[ -n "$_AIMEE_SESSION_REASONING_EFFORT" ]] && local -x AIMEE_REASONING__EFFORT="$_AIMEE_SESSION_REASONING_EFFORT"
    CLICOLOR_FORCE=0 $_AIMEE_BIN select "$@" </dev/tty 2>/dev/tty
}

function _aimee_select_global() {
    CLICOLOR_FORCE=0 $_AIMEE_BIN select "$@" </dev/tty 2>/dev/tty
}

function _aimee_select_with_query() {
    local query="$1"
    shift

    if [[ -n "$query" ]]; then
        _aimee_select "$@" --query "$query"
    else
        _aimee_select "$@"
    fi
}

function _aimee_select_with_query_global() {
    local query="$1"
    shift

    if [[ -n "$query" ]]; then
        _aimee_select_global "$@" --query "$query"
    else
        _aimee_select_global "$@"
    fi
}

function _aimee_select_model_pair() {
    local result
    result=$(_aimee_select_with_query "$1" model)
...
function _aimee_select_model_pair_global() {
    local result
    result=$(_aimee_select_with_query_global "$1" model)
...
function _aimee_reset() {
  ...
  # rather than the aimee conversation output that precedes them.
...
# Usage: _aimee_log <level> <message>
# Levels: error, info, success, warning, debug
# Color scheme matches crates/aimee_main/src/title_display.rs
function _aimee_log() {
    local level="$1"
    local message="$2"
...
# Usage: _aimee_is_workspace_indexed <workspace_path>
# Returns: 0 if workspace is indexed, 1 otherwise
function _aimee_is_workspace_indexed() {
    local workspace_path="$1"
    $_AIMEE_BIN workspace info "$workspace_path" >/dev/null 2>&1
    return $?
}

# Start background sync job for current workspace if not already running
# Uses canonical path hash to identify workspace
function _aimee_start_background_sync() {
    # Check if sync is enabled (default to true if not set)
    local sync_enabled="${AIMEE_SYNC_ENABLED:-true}"
    ...
    # Check if workspace is indexed before attempting sync
    {
        ...
        if ! _aimee_is_workspace_indexed "$workspace_path"; then
            return 0
        fi
        # Should fail if sync-init or sync --init has not been performed even once
        $_AIMEE_BIN workspace sync "$workspace_path"
    } &!
}
...
function _aimee_start_background_update() {
    {
