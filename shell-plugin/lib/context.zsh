...
# Terminal context capture for aimee plugin
#
...
# The detection result is cached per session in _AIMEE_TERM_OSC133_CACHED
# ("1" = emit, "0" = don't emit) to avoid repeated detection overhead.
typeset -g _AIMEE_TERM_OSC133_CACHED=""
function _aimee_osc133_should_emit() {
    if [[ -n "$_AIMEE_TERM_OSC133_CACHED" ]]; then
        [[ "$_AIMEE_TERM_OSC133_CACHED" == "1" ]] && return 0 || return 1
    fi
    case "$_AIMEE_TERM_OSC133" in
        on)  _AIMEE_TERM_OSC133_CACHED="1"; return 0 ;;
        off) _AIMEE_TERM_OSC133_CACHED="0"; return 1 ;;
        auto)
            # Kitty sets KITTY_PID
            if [[ -n "${KITTY_PID:-}" ]]; then _AIMEE_TERM_OSC133_CACHED="1"; return 0; fi
            # Detect by TERM_PROGRAM
            case "${TERM_PROGRAM:-}" in
                WezTerm|iTerm.app|vscode|WarpTerminal) _AIMEE_TERM_OSC133_CACHED="1"; return 0 ;;
            esac
            # Foot terminal
            if [[ "${TERM:-}" == "foot"* ]]; then _AIMEE_TERM_OSC133_CACHED="1"; return 0; fi
            # Ghostty
            if [[ "${TERM_PROGRAM:-}" == "ghostty" ]]; then _AIMEE_TERM_OSC133_CACHED="1"; return 0; fi
            # Unknown terminal: don't emit
            _AIMEE_TERM_OSC133_CACHED="0"
            return 1
            ;;
        *)   _AIMEE_TERM_OSC133_CACHED="0"; return 1 ;;
    esac
}

# Emits an OSC 133 marker if the terminal supports it.
# Usage: _aimee_osc133_emit "A"  or  _aimee_osc133_emit "D;0"
function _aimee_osc133_emit() {
    _aimee_osc133_should_emit || return 0
    printf '\e]133;%s\a' "$1"
}
...
#   _AIMEE_TERM_COMMANDS, _AIMEE_TERM_EXIT_CODES, _AIMEE_TERM_TIMESTAMPS
# Pending command state:
typeset -g _AIMEE_TERM_PENDING_CMD=""
typeset -g _AIMEE_TERM_PENDING_TS=""

# Called before each command executes.
# Records the command text and timestamp, emits OSC 133 B+C markers.
function _aimee_context_preexec() {
    [[ "$_AIMEE_TERM" != "true" ]] && return
    _AIMEE_TERM_PENDING_CMD="$1"
    _AIMEE_TERM_PENDING_TS="$(date +%s)"
...
function _aimee_context_precmd() {
    ...
    _aimee_osc133_emit "D;$last_exit"

    [[ "$_AIMEE_TERM" != "true" ]] && return

    # Only record if we have a pending command from preexec
    if [[ -n "$_AIMEE_TERM_PENDING_CMD" ]]; then
        _AIMEE_TERM_COMMANDS+=("$_AIMEE_TERM_PENDING_CMD")
        _AIMEE_TERM_EXIT_CODES+=("$last_exit")
        _AIMEE_TERM_TIMESTAMPS+=("$_AIMEE_TERM_PENDING_TS")

        # Trim ring buffer to max size
        while (( ${#_AIMEE_TERM_COMMANDS} > _AIMEE_TERM_MAX_COMMANDS )); do
            shift _AIMEE_TERM_COMMANDS
            shift _AIMEE_TERM_EXIT_CODES
            shift _AIMEE_TERM_TIMESTAMPS
        done

        _AIMEE_TERM_PENDING_CMD=""
        _AIMEE_TERM_PENDING_TS=""
    fi
...
if [[ "$_AIMEE_TERM" == "true" ]]; then
    preexec_functions+=(_aimee_context_preexec)
    precmd_functions=(_aimee_context_precmd "${precmd_functions[@]}")
fi
