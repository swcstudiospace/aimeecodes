...
## Objective
Add a `/model` command to the Omega CLI that uses the `inquire` library to display a list of available models and update the project's `omega.yaml` file with the selected model.

## Implementation Plan
- [x] Add `inquire` as a dependency to the workspace and omega_main crate
...
- After selecting a model, the omega.yaml file should be updated with the selected model
- The standard_model anchor in the omega.yaml file should be created if it doesn't exist
...
- Risk: omega.yaml file might not exist in the current directory
  Mitigation: Added error handling for missing files
- Risk: omega.yaml file might be in an unexpected format
  Mitigation: Added error handling for parsing errors

## Future Improvements
- Add support for updating advanced_model in addition to standard_model
- Add support for creating omega.yaml if it doesn't exist
