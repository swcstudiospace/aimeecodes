...
## Objective
Add a `/model` command to the Aimee CLI that uses the `inquire` library to display a list of available models and update the project's `aimee.yaml` file with the selected model.

## Implementation Plan
- [x] Add `inquire` as a dependency to the workspace and aimee_main crate
...
- After selecting a model, the aimee.yaml file should be updated with the selected model
- The standard_model anchor in the aimee.yaml file should be created if it doesn't exist
...
- Risk: aimee.yaml file might not exist in the current directory
  Mitigation: Added error handling for missing files
- Risk: aimee.yaml file might be in an unexpected format
  Mitigation: Added error handling for parsing errors

## Future Improvements
- Add support for updating advanced_model in addition to standard_model
- Add support for creating aimee.yaml if it doesn't exist
