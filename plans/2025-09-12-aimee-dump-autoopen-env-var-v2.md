...
**Current Implementation Location**: `crates/aimee_main/src/ui.rs:721-740`

**Environment Service Architecture**: The codebase uses a layered service architecture:
- `Environment` struct in `crates/aimee_domain/src/env.rs:18` contains configuration fields
- `EnvironmentInfra` trait in `crates/aimee_infra/src/env.rs:107-108` handles environment variable access
- Environment variables are parsed during initialization in `crates/aimee_infra/src/env.rs:39-71`
...
- [x] **Task 1: Add auto_open_dump field to Environment struct**
  - Location: `crates/aimee_domain/src/env.rs:54` (add field after existing configuration fields)
...
- [x] **Task 2: Add environment variable parsing in infrastructure layer**
  - Location: `crates/aimee_infra/src/env.rs:39-71` (in the `get()` method)
  - Parse `AIMEE_DUMP_AUTO_OPEN` environment variable using existing `parse_env` helper
...
- [x] **Task 3: Update UI dump method to use Environment service**
  - Location: `crates/aimee_main/src/ui.rs:721-740`
...
- [x] **Task 6: Create comprehensive test coverage**
  - Location: `crates/aimee_infra/src/env.rs:269-511` (following existing test patterns)
...
- Environment variable `AIMEE_DUMP_AUTO_OPEN` is integrated into Environment service
...
3. **Workflow-level Configuration**
   - Alternative: Add configuration to aimee.yaml workflow files
...
```rust
// In crates/aimee_domain/src/env.rs
...
```rust
// In crates/aimee_infra/src/env.rs get() method
let auto_open_dump = parse_env::<bool>("AIMEE_DUMP_AUTO_OPEN").unwrap_or(false);
...
**UI Integration:**
```rust
// In crates/aimee_main/src/ui.rs dump methods
...
```bash
# .env
AIMEE_TOOL_TIMEOUT=300         # Maximum execution time in seconds for a tool (default: 300)
AIMEE_DUMP_AUTO_OPEN=false     # Automatically open dump files in browser (default: false)
```

</details>
```

This plan follows established architectural patterns in the Aimee codebase while providing the requested `self.api.environment().auto_open_dump` access pattern and comprehensive documentation.