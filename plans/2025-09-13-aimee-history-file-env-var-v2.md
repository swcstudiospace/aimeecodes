...
**History File Location**: Currently determined by `Environment::history_path()` method in `crates/aimee_domain/src/env.rs:68-70`, which returns `self.base_path.join(".aimee_history")` - a single global file.

**History Usage**: History is initialized in `crates/aimee_main/src/editor.rs:68-71` using `FileBackedHistory::with_file()` with the path from `env.history_path()`.

**Environment Configuration**: Environment variables are parsed in `AimeeEnvironmentInfra::get()` method in `crates/aimee_infra/src/env.rs:39-78`, which already supports parsing custom environment variables like `AIMEE_DUMP_AUTO_OPEN`, `AIMEE_TOOL_TIMEOUT`, etc.
...
- [ ] **Task 1**: Add `AIMEE_HISTORY_FILE` environment variable support to `AimeeEnvironmentInfra::get()` method
  - **Location**: `crates/aimee_infra/src/env.rs:39-78`
  - **Rationale**: This is where all environment variable parsing occurs, following the established pattern of other AIMEE_* environment variables
...
- [ ] **Task 2**: Add `custom_history_path` field to `Environment` struct to store the resolved history file path
  - **Location**: `crates/aimee_domain/src/env.rs:16-61`
...
- [ ] **Task 3**: Modify `Environment::history_path()` to support custom path resolution with relative path handling
  - **Location**: `crates/aimee_domain/src/env.rs:68-70`
...
<task_status>
[x]: DONE - Task 1: Add AIMEE_HISTORY_FILE environment variable support
...
- Environment variable `AIMEE_HISTORY_FILE` successfully overrides default history location only when set
- Absolute paths specified in environment variable are used directly without modification
- Relative paths are resolved relative to current working directory where aimee is invoked
...
### 3. **Relative Path Confusion**
   ...
   **Mitigation**: Clear documentation explaining resolution is relative to working directory, not aimee binary location
...
### 1. **Always Use Current Directory**: Default to `./.aimee_history` instead of global file
   **Trade-offs**: More intuitive per-project behavior but breaks existing user expectations

### 2. **Auto-Detection**: Automatically look for `.aimee_history` in current directory first
   **Trade-offs**: Zero configuration but might create unexpected behavior changes

### 3. **Aimee.yaml Integration**: Add history_file field to project configuration
...
### Functional Success
- Users can set `AIMEE_HISTORY_FILE=./project-history` for relative paths
- Users can set `AIMEE_HISTORY_FILE=/absolute/path/history` for absolute paths
- Windows users can use `AIMEE_HISTORY_FILE=C:\Users\Name\history` successfully
...
AIMEE_HISTORY_FILE=./project-history
AIMEE_HISTORY_FILE=../shared/team-history
AIMEE_HISTORY_FILE=/home/user/aimee-histories/project1

# Windows examples
AIMEE_HISTORY_FILE=.\project-history
AIMEE_HISTORY_FILE=..\shared\team-history
AIMEE_HISTORY_FILE=C:\Users\Name\AimeeHistories\project1
AIMEE_HISTORY_FILE=\\server\share\team-histories\project1
```