...
```bash
./.omega/skills/create-plan/validate-plan.sh plans/{YYYY-MM-DD}-{task-name}-v{N}.md
```
...
- **ALWAYS validate the plan** using `./.omega/skills/create-plan/validate-plan.sh` after creation
...
- Use `filepath:line` format for file references (e.g., `crates/omega_repo/src/provider.rs:45`)
