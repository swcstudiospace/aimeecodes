---
name: create-agent
description: Create new agents for the omega-loops application. Agents are stored as .md files in the <cwd>/.omega/agents directory with YAML frontmatter (id, title, description, reasoning, tools, user_prompt) and markdown body containing agent instructions. Use when users need to add new agents, modify existing agents, or understand the agent file structure.
---
{{{{raw}}}}
# Create Agents

Create and manage agents for the omega-loops application. Agents are specialized AI assistants with specific capabilities, tools, and behaviors.

## File Location

**CRITICAL**: All agent files must be created in the `<cwd>/.omega/agents` directory, where `<cwd>` is the current working directory of your omega-loops project.

- **Directory**: `<cwd>/.omega/agents`
- **File format**: `{agent-id}.md`
- **Example**: If your project is at `/home/user/my-project`, agents go in `/home/user/my-project/.omega/agents/`

This is the only location where omega will discover and load custom agents.
...
You are Omega, an expert software engineering assistant...
...
Create the file in the `<cwd>/.omega/agents` directory with the format: `{agent-id}.md`

**IMPORTANT**: The file MUST be in `<cwd>/.omega/agents` where `<cwd>` is your current working directory. Agents placed anywhere else will not be discovered by omega.
...
**Strictly Advisory**: You cannot perform implementation tasks. If asked, offer to switch to an implementation agent like Omega.
```
...
### File Structure
- [ ] File is in the `<cwd>/.omega/agents` directory (CRITICAL)
- [ ] Filename matches agent ID (e.g., `omega.md` for `id: "omega"`)
...
```markdown
You are Omega, an expert software engineering assistant designed to help users with programming tasks, file operations, and software development processes.
```
...
### File Location
- **Directory**: `<cwd>/.omega/agents` (where `<cwd>` is current working directory)
- **Format**: `{agent-id}.md`
- **CRITICAL**: Agents MUST be in this exact location to be discovered by omega
...
1. **Verify the file location**: Ensure the file is in `<cwd>/.omega/agents` directory (CRITICAL - agents anywhere else will not be found)
