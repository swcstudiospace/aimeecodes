...
# Runs omega with OMEGA_DEBUG_REQUESTS pointing to <outfile>.
...
# Like run_test, but expects omega to exit non-zero and NOT write the request file.
...
# Returns 0 if omega exited non-zero and wrote no file, 1 otherwise.
...
printf "${DIM}Building omega (debug)...${RESET}\n\n"
if ! cargo build 2>&1 | grep -E "^error|Finished|^   Compiling omega_main"; then
...
# Note: the default omega config sets reasoning.enabled=true; it always appears
...
for effort in none minimal low medium high xhigh; do
    ...
    (
        ...
        if run_test "$OUT" open_router "openai/o4-mini" "OMEGA_REASONING__EFFORT=$effort"; then
...
# Note: the default omega config also injects effort="medium" and enabled=true;
...
(
    ...
    if run_test "$OUT" open_router "openai/o4-mini" OMEGA_REASONING__MAX_TOKENS=4000; then
...
(
    ...
    if run_test "$OUT" open_router "openai/o4-mini" \
            OMEGA_REASONING__EFFORT=high OMEGA_REASONING__EXCLUDE=true; then
...
(
    ...
    if run_test "$OUT" open_router "openai/o4-mini" OMEGA_REASONING__ENABLED=true; then
...
(
    ...
    if run_test "$OUT" open_router "anthropic/claude-opus-4-5" OMEGA_REASONING__MAX_TOKENS=4000; then
...
(
    ...
    if run_test "$OUT" open_router "moonshotai/kimi-k2" OMEGA_REASONING__MAX_TOKENS=4000; then
...
(
    ...
    if run_test "$OUT" open_router "moonshotai/kimi-k2" OMEGA_REASONING__EFFORT=high; then
...
(
    ...
    if run_test "$OUT" open_router "minimax/minimax-m2" OMEGA_REASONING__MAX_TOKENS=4000; then
...
(
    ...
    if run_test "$OUT" open_router "minimax/minimax-m2" OMEGA_REASONING__EFFORT=high; then
...
for effort in low medium high max; do
    ...
    (
        ...
        if run_test "$OUT" anthropic "claude-opus-4-6" "OMEGA_REASONING__EFFORT=$effort"; then
...
(
    ...
    if run_test "$OUT" anthropic "claude-3-7-sonnet-20250219" \
            OMEGA_REASONING__ENABLED=true OMEGA_REASONING__MAX_TOKENS=8000; then
...
for effort in none minimal low medium high xhigh; do
    ...
    (
        ...
        if run_test "$OUT" github_copilot "o4-mini" "OMEGA_REASONING__EFFORT=$effort"; then
...
for effort in none minimal low medium high xhigh; do
    ...
    (
        ...
        if run_test "$OUT" codex "gpt-5.1-codex" "OMEGA_REASONING__EFFORT=$effort"; then
...
(
    ...
    if run_test "$OUT" codex "gpt-5.1-codex" \
            OMEGA_REASONING__EFFORT=medium OMEGA_REASONING__EXCLUDE=true; then
...
# "invalid" is not a recognised Effort variant. Omega must reject it at config
...
(
    ...
    do
        ...
        if run_test_expect_failure "$OUT" "$provider" "$model" OMEGA_REASONING__EFFORT=invalid; then
