---
name: debug-cli
description: Use when users need to debug, modify, or extend the aimee-codes application's CLI commands, argument parsing, or CLI behavior. This includes adding new commands, fixing CLI bugs, updating command options, or troubleshooting CLI-related issues.
---

# CLI Debug Skill

This skill provides a systematic workflow for debugging and verifying changes to the aimee CLI application.

## Core Principles

1. **Always get latest docs first**: Run `--help` to see current commands and options
2. **Use `-p` for testing**: Test aimee by giving it tasks with the `-p` flag
...
./target/debug/aimee [COMMAND] --help

# Subcommand help
./target/debug/aimee [COMMAND] [SUBCOMMAND] --help
```

### 3. Test with `-p` Flag

Use the `-p` flag to give aimee a task to complete without interactive mode:

```bash
# Test with a simple prompt
./target/debug/aimee -p "create a hello world rust program"

# Test with specific functionality
./target/debug/aimee -p "read the README.md file and summarize it"

# Test with complex tasks
./target/debug/aimee -p "analyze the code structure and suggest improvements"
```
...
./target/debug/aimee conversation dump <conversation-id>

# Export as HTML for human-readable format (creates: YYYY-MM-DD_HH-MM-SS-dump.html)
./target/debug/aimee conversation dump --html <conversation-id>

# Use dumped JSON to reproduce issues
./target/debug/aimee --conversation 2025-11-23_12-28-52-dump.json
```
...
./target/debug/aimee conversation clone <source-conversation-id>

# This creates a new conversation ID - use that for testing
./target/debug/aimee --conversation-id <new-cloned-id>
...
cargo build
./target/debug/aimee --help  # Verify new command appears
./target/debug/aimee new-command --help  # Check command docs
./target/debug/aimee -p "test the new feature"
```
...
./target/debug/aimee conversation dump <bug-conversation-id>

# 2. Clone it for testing (preserves original)
./target/debug/aimee conversation clone <bug-conversation-id>

# 3. Reproduce with the cloned conversation
./target/debug/aimee --conversation-id <cloned-id> -p "reproduce the issue"

# 4. After fix, verify with new clone
./target/debug/aimee conversation clone <bug-conversation-id>
./target/debug/aimee --conversation-id <new-clone-id> -p "verify fix"
```
...
./target/debug/aimee command

# Test with invalid input
./target/debug/aimee -p "invalid task with special chars: <>|&"

# Test with boundary values
./target/debug/aimee -p "create a file with a very long name..."
```
...
./target/debug/aimee conversation dump <id>
...
./target/debug/aimee conversation dump --html <id>

# 4. Test modified prompts
./target/debug/aimee -p "your optimized prompt here"
```
...
1. **Build**: `cargo build`
2. **Docs**: `./target/debug/aimee --help` (verify documentation)
3. **Test**: `./target/debug/aimee -p "relevant task"`
4. **Verify**: Check output matches expectations

### Debugging a Bug Report

1. **Clone**: `./target/debug/aimee conversation clone <source-id>`
2. **Build**: `cargo build` (with potential fixes)
3. **Test**: `./target/debug/aimee --conversation-id <cloned-id> -p "reproduce"`
...
cargo build
./target/debug/aimee --help  # Always check docs first
./target/debug/aimee -p "your test task"
...
# Clone and test bug
./target/debug/aimee conversation clone <source-id>
./target/debug/aimee --conversation-id <cloned-id> -p "reproduce bug"

# Debug prompts with jq (use actual filename)
cat 2025-11-23_12-28-52-dump.json | jq '.messages[] | {role, content}'

# Test with verbose output
./target/debug/aimee --verbose -p "test task"
```
...
- **Use relative paths**: Binary is at `./target/debug/aimee` from project root
